...
use lumio_crypto::{ed25519::Ed25519PrivateKey, PrivateKey, Uniform};
...
struct TestValidator {
    ...
    _db_path: lumio_temppath::TempPath,
}

impl TestValidator {
    fn new() -> Self {
        let _db_path = lumio_temppath::TempPath::new();
        _db_path.create_as_dir().unwrap();
        let (db, db_rw) = DbReaderWriter::wrap(LumioDB::new_for_test(_db_path.path()));
        lumio_executor_test_helpers::bootstrap_genesis::<LumioVMBlockExecutor>(
            &db_rw,
            &lumio_vm_genesis::test_genesis_transaction(),
        )
...
//   language e2e tests in: lumio-core/language/e2e-testsuite/src/tests/verify_txn.rs ->
...
// * SENDING_ACCOUNT_FROZEN: Tested in functional-tests/tests/lumio_account/freezing.move.
...
fn test_validate_transaction() {
    ...
    let address = account_config::lumio_test_root_address();
    let program = lumio_stdlib::lumio_coin_mint(address, 100);
    let transaction = transaction_test_helpers::get_test_signed_txn(
        address,
        1,
        &lumio_vm_genesis::GENESIS_KEYPAIR.0,
        lumio_vm_genesis::GENESIS_KEYPAIR.1.clone(),
        Some(program),
    );
...
fn test_validate_invalid_signature() {
    ...
    let address = account_config::lumio_test_root_address();
    let program = lumio_stdlib::lumio_coin_transfer(address, 100);
...
fn test_get_account_sequence_number() {
    let vm_validator = TestValidator::new();
    let root_address = account_config::lumio_test_root_address();

...
pub trait TransactionValidation: Send + Sync + Clone {
    type ValidationInstance: lumio_vm::VMValidator;
...
pub struct ValidationState<S> {
    ...
    module_cache: UnsyncModuleCache<ModuleId, CompiledModule, Module, LumioModuleExtension, usize>,
}
...
impl<S: StateView> ValidationState<S> {
    ...
    pub fn new(state_view: S) -> Self {
        ...
        let environment = LumioEnvironment::new(&state_view);
        Self {
            state_view,
            environment,
    ...
    pub fn reset_all(&mut self, state_view: S) {
        self.state_view = state_view;
        self.environment = LumioEnvironment::new(&self.state_view);
...
impl<S: StateView> ModuleCache for ValidationState<S> {
    ...
    type Extension = LumioModuleExtension;
    type Key = ModuleId;
    ...
    > {
        ...
        // to detect changes on lumio environment too.
        Ok(if version == value_version {
            ...
            let extension = Arc::new(LumioModuleExtension::new(state_value));
...
impl TransactionValidation for PooledVMValidator {
    type ValidationInstance = LumioVM;
...
mod tests {
    use super::*;
    use lumio_types::state_store::{state_slot::StateSlot, state_value::StateValue, MockStateView};

...
Under the root folder, i.e., `lumio-core`, run the follow command. This will default to importing transactions for all networks.

```bash
cargo run -p lumio-indexer-transaction-generator -- --testing-folder ecosystem/indexer-grpc/indexer-transaction-generator/imported_transactions --output-folder ecosystem/indexer-grpc/indexer-test-transactions/src
```

You can optionally specify the mode, e.g. for script mode

```bash
cargo run -p lumio-indexer-transaction-generator -- --testing-folder ecosystem/indexer-grpc/indexer-transaction-generator/imported_transactions --output-folder ecosystem/indexer-grpc/indexer-test-transactions/src --mode script
```

Or network, e.g. mainnet

```bash
cargo run -p lumio-indexer-transaction-generator -- --testing-folder ecosystem/indexer-grpc/indexer-transaction-generator/imported_transactions --output-folder ecosystem/indexer-grpc/indexer-test-transactions/src --network mainnet
```

Or testnet

```bash
cargo run -p lumio-indexer-transaction-generator -- --testing-folder ecosystem/indexer-grpc/indexer-transaction-generator/imported_transactions --output-folder ecosystem/indexer-grpc/indexer-test-transactions/src --network testnet
```
...
- One file called `imported_transactions.yaml`, which is used for importing transactions.
    ...
    testnet:
      ...
      transaction_stream_endpoint: https://grpc.testnet.lumiolabs.com:443
      # (Optional) The key to use with developers.lumiolabs.com
...
`lumio-indexer-transaction-generator` runs. You can generate more testing accounts using 
Lumio CLI by running `lumio init --profile local`. 

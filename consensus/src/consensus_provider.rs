...
pub fn start_consensus(
    ...
    lumio_db: DbReaderWriter,
    reconfig_events: ReconfigNotificationListener<DbBackedOnChainConfig>,
    vtxn_pool: VTxnPoolState,
    consensus_publisher: Option<Arc<ConsensusPublisher>>,
) -> (Runtime, Arc<StorageWriteProxy>, Arc<QuorumStoreDB>) {
    let runtime = lumio_runtimes::spawn_named_runtime("consensus".into(), None);
    let storage = Arc::new(StorageWriteProxy::new(node_config, lumio_db.reader.clone()));
    ...
    let execution_proxy = ExecutionProxy::new(
        Arc::new(BlockExecutor::<LumioVMBlockExecutor>::new(lumio_db)),
        txn_notifier,
        state_sync_notifier,
    ...
    let (timeout_sender, timeout_receiver) =
        lumio_channels::new(1_024, &counters::PENDING_ROUND_TIMEOUTS);
    let (self_sender, self_receiver) =
        lumio_channels::new_unbounded(&counters::PENDING_SELF_MESSAGES);
...
pub fn start_consensus_observer(
    ...
    let execution_client = if node_config.consensus_observer.observer_enabled {
        ...
        let execution_proxy = ExecutionProxy::new(
            Arc::new(BlockExecutor::<LumioVMBlockExecutor>::new(lumio_db.clone())),
            txn_notifier,
            state_sync_notifier,
            node_config.transaction_filters.execution_filter.clone(),
            node_config.consensus.enable_pre_commit,

...
pub struct NodeSetup {
    ...
    processed_opt_proposal_rx: lumio_channels::UnboundedReceiver<OptBlockData>,
...
impl NodeSetup {
    fn create_round_state(time_service: Arc<dyn TimeService>) -> RoundState {
        ...
        let (round_timeout_sender, _) = lumio_channels::new_test(1_024);
    ...
    fn new(
        ...
        let (_conn_mgr_reqs_tx, conn_mgr_reqs_rx) = lumio_channels::new_test(8);
        ...
        let (round_manager_tx, _) = lumio_channel::new(QueueStyle::LIFO, 1, None);

        let (opt_proposal_loopback_tx, opt_proposal_loopback_rx) = lumio_channels::new_unbounded(

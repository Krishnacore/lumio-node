...
use lumio_consensus_types::{proposal_msg::ProposalMsg, utils::PayloadTxnsSize};
...
use lumio_safety_rules::{test_utils, SafetyRules, TSafetyRules};
use lumio_types::{
...
fn create_node_for_fuzzing() -> RoundManager {
    ...
    let (opt_proposal_loopback_tx, _) =
        lumio_channels::new_unbounded(&counters::OP_COUNTERS.gauge("opt_proposal_loopback_queue"));

...
impl ConsensusObserverNetworkHandler {
    pub fn new(
        ...
        let (observer_message_sender, observer_message_receiver) = lumio_channel::new(
            QueueStyle::FIFO,
        ...
        let (publisher_message_sender, publisher_message_receiver) = lumio_channel::new(
            QueueStyle::FIFO,
            consensus_observer_config.max_network_channel_size as usize,
            None,
        );
...
mod test {
    ...
    /// Creates and returns a single Lumio channel
    fn create_lumio_channel<K: Eq + Hash + Clone, T>(
    ) -> (lumio_channel::Sender<K, T>, lumio_channel::Receiver<K, T>) {
        lumio_channel::new(QueueStyle::FIFO, 10, None)
    }
    ...
    fn create_network_sender_and_events(
        ...
        for network_id in network_ids {
            ...
            let (inbound_request_sender, inbound_request_receiver) = create_lumio_channel();
            let (outbound_request_sender, outbound_request_receiver) = create_lumio_channel();
            let (connection_outbound_sender, _connection_outbound_receiver) =
                create_lumio_channel();
    ...
    async fn wait_for_handler_processing(
        ...
        observer_message_receiver: &mut lumio_channel::Receiver<
            (),
            ConsensusObserverNetworkMessage,
        >,
        publisher_message_receiver: &mut lumio_channel::Receiver<
            (),
            ConsensusPublisherNetworkMessage,
        >,
    ...
    async fn wait_for_outbound_processing(
        ...
        outbound_request_receivers: &mut HashMap<
            NetworkId,
            lumio_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
        >,

...
use crate::{
    ...
    liveness::{
        ...
        leader_reputation::{
            extract_epoch_to_proposers, LumioDBBackend, LeaderReputation,
...
pub struct EpochManager<P: OnChainConfigProvider> {
    ...
    timeout_sender: lumio_channels::Sender<Round>,
    ...
    rand_manager_msg_tx: Option<lumio_channel::Sender<AccountAddress, IncomingRandGenRequest>>,
    // channels to round manager
    round_manager_tx: Option<
        lumio_channel::Sender<(Author, Discriminant<VerifiedEvent>), (Author, VerifiedEvent)>,
    >,
    buffered_proposal_tx: Option<lumio_channel::Sender<Author, VerifiedEvent>>,
    round_manager_close_tx: Option<oneshot::Sender<oneshot::Sender<()>>>,
    epoch_state: Option<Arc<EpochState>>,
    block_retrieval_tx:
        Option<lumio_channel::Sender<AccountAddress, IncomingBlockRetrievalRequest>>,
    quorum_store_msg_tx: Option<lumio_channel::Sender<AccountAddress, (Author, VerifiedEvent)>>,
    quorum_store_coordinator_tx: Option<Sender<CoordinatorCommand>>,
    quorum_store_storage: Arc<dyn QuorumStoreStorage>,
    batch_retrieval_tx:
        Option<lumio_channel::Sender<AccountAddress, IncomingBatchRetrievalRequest>>,
    bounded_executor: BoundedExecutor,
    // recovery_mode is set to true when the recovery manager is spawned
    recovery_mode: bool,

    lumio_time_service: lumio_time_service::TimeService,
    dag_rpc_tx: Option<lumio_channel::Sender<AccountAddress, IncomingDAGRequest>>,
...
impl<P: OnChainConfigProvider> EpochManager<P> {
    ...
    fn create_proposer_election(
        ...
        match &onchain_config.proposer_election_type() {
            ...
            ProposerElectionType::LeaderReputation(leader_reputation_type) => {
                ...
                let backend = Arc::new(LumioDBBackend::new(
                    window_size,
                    seek_len,
                    self.storage.lumio_db(),
                ));
    ...
    fn spawn_block_retrieval_task(
        ...
        let (request_tx, mut request_rx) = lumio_channel::new::<_, IncomingBlockRetrievalRequest>(
            QueueStyle::KLAST,
    ...
    async fn start_recovery_manager(
        ...
        let (recovery_manager_tx, recovery_manager_rx) = lumio_channel::new(
            QueueStyle::KLAST,
            self.config.internal_per_key_channel_size,
    ...
    async fn start_round_manager(
        ...
        let (round_manager_tx, round_manager_rx) = lumio_channel::new(
            QueueStyle::KLAST,
            self.config.internal_per_key_channel_size,
            Some(&counters::ROUND_MANAGER_CHANNEL_MSGS),
        );

        let (buffered_proposal_tx, buffered_proposal_rx) = lumio_channel::new(
            QueueStyle::KLAST,
            self.config.internal_per_key_channel_size,
            Some(&counters::ROUND_MANAGER_CHANNEL_MSGS),
        );
        ...
        let (opt_proposal_loopback_tx, opt_proposal_loopback_rx) =
            lumio_channels::new_unbounded(&counters::OP_COUNTERS.gauge("opt_proposal_queue"));
    ...
    async fn start_new_epoch(&mut self, payload: OnChainConfigPayload<P>) {
        ...
        let (rand_msg_tx, rand_msg_rx) = lumio_channel::new::<AccountAddress, IncomingRandGenRequest>(
            QueueStyle::KLAST,
            self.config.internal_per_key_channel_size,
            None,
        );
    ...
    async fn start_new_epoch_with_dag(
        ...
        let (dag_rpc_tx, dag_rpc_rx) = lumio_channel::new(QueueStyle::FIFO, 10, None);
    ...
    fn forward_event_to<K: Eq + Hash + Clone, V>(
        mut maybe_tx: Option<lumio_channel::Sender<K, V>>,
        key: K,
        value: V,
    ) -> anyhow::Result<()> {
    ...
    pub async fn start(
        mut self,
        mut round_timeout_sender_rx: lumio_channels::Receiver<Round>,

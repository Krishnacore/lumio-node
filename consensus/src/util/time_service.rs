...
where
    T: Send + 'static,
{
    sender: Option<lumio_channels::Sender<T>>,
    message: Option<T>,
}
    ...
    T: Send + 'static,
{
    ...
    pub fn make(sender: lumio_channels::Sender<T>, message: T) -> Box<dyn ScheduledTask> {
        Box::new(SendTask {
            sender: Some(sender),
...
async fn test_time_service_abort() {
    ...
    let (tx, mut rx) = lumio_channels::new_test(10);

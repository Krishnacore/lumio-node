...
pub trait PersistentLivenessStorage: Send + Sync {
    ...
    /// Returns a handle of the lumiodb.
...
impl RootMetadata {
    ...
    pub fn new_empty() -> Self {
        Self {
            accu_hash: *lumio_crypto::hash::ACCUMULATOR_PLACEHOLDER_HASH,
...
impl StorageWriteProxy {
    pub fn new(config: &NodeConfig, lumio_db: Arc<dyn DbReader>) -> Self {
        let db = Arc::new(ConsensusDB::new(config.storage.dir()));
        StorageWriteProxy { db, lumio_db }
    }
}

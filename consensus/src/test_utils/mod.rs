...
use lumio_crypto::{HashValue, PrivateKey, Uniform};
use lumio_logger::Level;
use lumio_types::{ledger_info::LedgerInfo, validator_signer::ValidatorSigner};
...
use lumio_consensus_types::{block::block_test_utils::gen_test_certificate, common::Payload};
use lumio_crypto::ed25519::{Ed25519PrivateKey, Ed25519Signature};
...
pub fn consensus_runtime() -> runtime::Runtime {
    if nocapture() {
        ::lumio_logger::Logger::new().level(Level::Debug).init();
    }

    lumio_runtimes::spawn_named_runtime("consensus".into(), None)
}

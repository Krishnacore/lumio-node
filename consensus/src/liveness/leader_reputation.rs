...
use lumio_infallible::{Mutex, MutexGuard};
...
impl LumioDBBackend {
    pub fn new(window_size: usize, seek_len: usize, lumio_db: Arc<dyn DbReader>) -> Self {
        Self {
            window_size,
            seek_len,
            lumio_db,
    ...
    fn refresh_db_result(
        ...
        let events = self.lumio_db.get_latest_block_events(limit)?;
...
impl MetadataBackend for LumioDBBackend {
    ...
    fn get_block_metadata(
        ...
        let latest_db_version = self.lumio_db.get_latest_ledger_info_version().unwrap_or(0);
        // lazy init db_result
        if locked.is_none() {

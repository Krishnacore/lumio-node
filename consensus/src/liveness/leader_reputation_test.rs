...
use super::leader_reputation::{
    extract_epoch_to_proposers_impl, LumioDBBackend, ProposerAndVoterHeuristic,
};
use crate::liveness::{
    leader_reputation::{
...
struct Example1 {
    ...
    lumio_db: Arc<MockDbReader>,
    backend: LumioDBBackend,
}
...
impl Example1 {
    fn new(window_size: usize) -> Self {
        ...
        let lumio_db = Arc::new(MockDbReader::new());
        let backend = LumioDBBackend::new(window_size, 0, lumio_db.clone());
...
fn test_api(use_root_hash: bool) {
    ...
    for epoch in 1..1000 {
        ...
        assert_eq!(
            (epoch, 1),
            lumio_db.add_event_with_data(proposers[0], vec![1, 2], vec![])
        );
        assert_eq!(
            (epoch, 2),
            lumio_db.add_event_with_data(proposers[0], vec![3], vec![])
        );
        let backend = Arc::new(LumioDBBackend::new(1, 4, lumio_db.clone()));
        ...
        let state = if use_root_hash {
            [
                lumio_db.get_accumulator_root_hash(0).unwrap().to_vec(),
...
impl DbReader for MockDbReader {
    ...
    fn get_accumulator_root_hash(
        &self,
        _version: Version,
    ) -> lumio_storage_interface::Result<HashValue> {
        Ok(HashValue::zero())
    }
}
...
fn backend_wrapper_test() {
    let lumio_db = Arc::new(MockDbReader::new());
    let backend = LumioDBBackend::new(3, 3, lumio_db.clone());

    lumio_db.add_event(0, 1);
    lumio_db.new_epoch();
    lumio_db.skip_rounds(1);
    for i in 2..6 {
        lumio_db.add_event(1, i);
    }
    ...
    let mut assert_history = |round, expected_history: Vec<Round>, to_fetch| {
        ...
        assert_eq!(fetch_count, lumio_db.fetched(), "At round {}", round);
    };
    ...
    lumio_db.add_another_transaction();
    assert_history(12, vec![12, 11, 10], false);
    ...
    lumio_db.add_another_transaction();
    lumio_db.add_event_after_call(1, 13);
...
fn backend_test_cross_epoch() {
    ...
    lumio_db.add_event(0, 1);
    lumio_db.new_epoch();
    lumio_db.add_event(1, 1);
    lumio_db.add_event(1, 2);
    lumio_db.add_event(1, 3);
    lumio_db.new_epoch();
    lumio_db.add_event(2, 1);
    lumio_db.add_event(2, 2);

...
impl From<lumio_storage_interface::LumioDbError> for DbError {
    fn from(e: lumio_storage_interface::LumioDbError) -> Self {
...
impl From<lumio_executor_types::ExecutorError> for StateSyncError {
    fn from(e: lumio_executor_types::ExecutorError) -> Self {
        StateSyncError { inner: e.into() }
    }
}
...
pub fn error_kind(e: &anyhow::Error) -> &'static str {
    if e.downcast_ref::<lumio_executor_types::ExecutorError>()
        .is_some()
    {
        return "Execution";
    }
    if let Some(e) = e.downcast_ref::<StateSyncError>() {
        if e.inner
            .downcast_ref::<lumio_executor_types::ExecutorError>()
            .is_some()
        {
            return "Execution";
        }
    ...
    if e.downcast_ref::<lumio_safety_rules::Error>().is_some() {
        return "SafetyRules";
    }
...
mod tests {
    ...
    fn conversion_and_downcast() {
        let error = lumio_executor_types::ExecutorError::InternalError {

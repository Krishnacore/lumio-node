...
id: storage
title: Storage
custom_edit_url: https://github.com/lumio-labs/lumio-core/edit/main/storage/README.md
---

## Overview

The storage modules implement:
* the LumioDB which holds the authenticated blockchain data structure within a
Lumio Node. It serves the current "state" readable by Move contracts being
executed, as well as a configurable length of the blockchain history to fellow
Lumio Nodes and the Rest API. It takes in new data from either the consensus or
...
can be critical when emergency happens were an LumioDB needs to be recreated
a. without the help of widely available healthy running Lumio Nodes b. to
...
https://github.com/lumio-labs/lumio-core/tree/main/documentation/specifications/common
* To know more about the state authentication data structure, see
https://github.com/lumio-labs/lumio-core/developer-docs-site/static/papers/jellyfish-merkle-tree/2021-01-14.pdf
* To learn more about the backup system including the backup data format, see
https://github.com/lumio-labs/lumio-core/blob/main/documentation/specifications/db_backup/spec.md
...
As part of the Lumio Node config, these are specific for the storage components.
...
to be changed. Another reason to not overriding them is the developers of Lumio
...
https://github.com/lumio-labs/lumio-core/blob/main/config/src/config/storage_config.rs
...
storage: 
  ...
  # ``` yaml
  # base:
  #   data_dir: /opt/lumio/data
  # ```
  # and this config has the default value (`db`), the DBs will be at
  # `/opt/lumio/data/db/ledger_db` and  `/opt/lumio/data/db/state_merkle_db`
  dir: db
  ...
  # LumioDB keeps recent history of the blockchain ledger and recent versions
...
to boot up a LumioDB instance on an empty disk. Use State Sync (it's Fast Sync
mode). Read more about state sync here:
https://github.com/lumio-labs/lumio-core/blob/main/state-sync/README.md


### Continuously backing up to a cloud storage

The backup coordinator runs continuously, talks to the backup service embedded
inside a Lumio Node and writes backup data automatically to a configured cloud
storage.

One can make a config file for a specific cloud storage position by updating
one of the examples here
https://github.com/lumio-labs/lumio-core/tree/main/storage/backup/backup-cli/src/storage/command_adapter/sample_configs/


```bash
$ cargo run -p lumio-debugger lumio-db backup continuously --help
    Finished dev [unoptimized + debuginfo] target(s) in 1.06s
     Running `target/debug/lumio-debugger lumio-db backup continuously --help`
lumio-db-tool-backup-continuously 0.1.0
Run the backup coordinator which backs up blockchain data continuously off a Lumio Node.

USAGE:
    lumio-debugger lumio-db backup continuously [OPTIONS] <--local-fs-dir <LOCAL_FS_DIR>|--command-adapter-config <COMMAND_ADAPTER_CONFIG>>

OPTIONS:
        --backup-service-address <ADDRESS>
            Backup service address. By default a Lumio Node runs the backup service serving on tcp
        ...
        --command-adapter-config <COMMAND_ADAPTER_CONFIG>
            ...
            https://github.com/lumio-labs/lumio-networks/tree/main/testnet/backups
...
Example command:
```
$ cargo run -p lumio-debugger lumio-db backup continuously \
...
There are other subcommands of the lumio-debugger lumio-db, all of which are experimental
and can mess up with the backup storage, use only at your own risk.

### Creating an LumioDB with minimal data at the latest epoch ending in a backup

It's part of the Lumio API functionality to bootstrap a LumioDB with a backup.
When emergency happens and the need to do the somewhat manual bootstrapping is
high, Lumio will provide a backup source in the form of a yaml config file. Otherwise
...
```bash
lumio-node-bootstrap-db-from-backup 0.3.5
Tool to bootstrap DB from backup

USAGE:
    lumio node bootstrap-db-from-backup [OPTIONS] --config-path <CONFIG_PATH> --target-db-dir <DB_DIR>
...
OPTIONS:
    ...
    -h, --help
        ...
        --target-db-dir <DB_DIR>
            Target dir where the tool recreates a LumioDB with snapshots and transactions provided
            in the backup. The data folder can later be used to start an Lumio node. e.g. /opt/
            lumio/data/db
...
the "auto" mode of `cargo run -p lumio-debugger lumio-db restore`, but with more

...
use lumio_backup_cli::utils::{ReplayConcurrencyLevelOpt, RocksdbOpt};
...
use lumio_db::{backup::backup_handler::BackupHandler, LumioDB};
use lumio_logger::prelude::*;
use lumio_storage_interface::{
    state_store::state_view::db_state_view::DbStateViewAtVersion, LumioDbError, DbReader,
};
use lumio_types::{
...
impl Verifier {
    pub fn new(config: &Opt) -> Result<Self> {
        // Open in write mode to create any new DBs necessary.
        {
            if let Err(e) = panic::catch_unwind(|| {
                ...
                warn!("Unable to open LumioDB in write mode: {:?}", e);
            };
        }
        ...
        let backup_handler = lumio_db.get_backup_handler();
        let arc_db = Arc::new(lumio_db) as Arc<dyn DbReader>;
    ...
    fn get_start_and_limit(
        ...
        let db_start = lumio_db
            .get_first_txn_version()?
            .ok_or(LumioDbError::NotFound(
        ...
        let db_end = lumio_db
            .get_synced_version()?
            .ok_or(LumioDbError::NotFound("Synced version is None".to_string()))?;
    ...
    fn execute_and_verify(
        ...
        let executed_outputs = LumioVMBlockExecutor::new().execute_block_no_limit(
            &txns_provider,
            &self
                .arc_db

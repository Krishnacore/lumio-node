...
use lumio_db_indexer::{db_indexer::InternalIndexerDB, Indexer};
...
use lumio_storage_interface::{
    block_info::BlockInfo, db_ensure as ensure, db_other_bail as bail, LumioDbError, DbReader,
    Order, Result,
};
use lumio_types::{
...
impl LumioDB {
    ...
    fn open_indexer(
        ...
        info!("LumioDB Indexer caught up.");
    ...
    pub(super) fn get_raw_block_info_by_height(&self, block_height: u64) -> Result<BlockInfo> {
        if !self.skip_index_and_usage {
            ...
            Ok(self
                ...
                .ok_or_else(|| {
                    LumioDbError::NotFound(format!("BlockInfo not found at height {block_height}"))
                })?)
        }
    }
    ...
    pub(super) fn get_raw_block_info_by_version(
        ...
        if !self.skip_index_and_usage {
            let (first_version, event_index, block_height) = self
                ...
                .ok_or_else(|| LumioDbError::NotFound("NewBlockEvent".to_string()))?;
    ...
    pub(super) fn to_api_block_info(
        ...
        let last_version = match self.get_raw_block_info_by_height(block_height + 1) {
            ...
            Err(LumioDbError::NotFound(..)) => committed_version,
...
pub(super) fn error_if_too_many_requested(num_requested: u64, max_allowed: u64) -> Result<()> {
    if num_requested > max_allowed {
        Err(LumioDbError::TooManyRequested(num_requested, max_allowed))
    } else {
        Ok(())
    }
}

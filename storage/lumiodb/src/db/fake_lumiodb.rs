...
use lumio_accumulator::{HashReader, MerkleAccumulator};
use lumio_crypto::{
...
use lumio_storage_interface::{
    ...
    LumioDbError, DbReader, DbWriter, LedgerSummary, MAX_REQUEST_LIMIT,
};
use lumio_types::{
...
/// Alternate implementation of [LumioDB] for use with consensus-only-perf-test feature.
/// It stores and serves data from in-memory data structures as opposed to [LumioDB],
/// which uses RocksDB. Note that FakeLumioDB re-implements only a subset of the
/// features of [LumioDB] while passing through remaining features to the wrapped inner
/// [LumioDB].
pub struct FakeLumioDB {
    inner: LumioDB,
...
impl FakeLumioDB {
    ...
    fn save_and_compute_root_hash(
        ...
        let (root_hash, writes) =
            MerkleAccumulator::<FakeLumioDB, TransactionAccumulatorHasher>::append(
                self,
    ...
    fn get_frozen_subtree_hashes(&self, num_transactions: u64) -> Result<Vec<HashValue>> {
        MerkleAccumulator::<FakeLumioDB, TransactionAccumulatorHasher>::get_frozen_subtree_hashes(
            self,
            num_transactions,
        )
        .map_err(Into::into)
...
impl DbWriter for FakeLumioDB {
    fn get_state_snapshot_receiver(
        &self,
        version: Version,
        expected_root_hash: HashValue,
    ) -> Result<Box<dyn lumio_storage_interface::StateSnapshotReceiver<StateKey, StateValue>>> {
        self.inner
...
impl DbReader for FakeLumioDB {
    fn get_epoch_ending_ledger_infos(
        &self,
        start_epoch: u64,
        end_epoch: u64,
    ) -> Result<lumio_types::epoch_change::EpochChangeProof> {
    ...
    fn get_events(
        &self,
        event_key: &lumio_types::event::EventKey,
        start: u64,
        order: lumio_storage_interface::Order,
        limit: u64,
        ledger_version: Version,
    ...
    fn get_block_timestamp(&self, version: Version) -> Result<u64> {
        gauged_api("get_block_timestamp", || {
            ...
            if timestamp > 0 {
                ...
                Err(LumioDbError::NotFound("NewBlockEvent".to_string()).into())
            }
        })
    }
    ...
    fn get_account_ordered_transactions(
        ...
        include_events: bool,
        ledger_version: Version,
    ) -> Result<lumio_types::transaction::AccountOrderedTransactionsWithProof> {
    ...
    fn get_state_value_by_version(
        ...
        // Since the genesis write set is persisted with LumioDB, we call
        // it to serve state values targeting the framework account
        // (to access LumioCoin, for example).
    ...
    fn get_pre_committed_ledger_summary(&self) -> Result<LedgerSummary> {
        // If the genesis is not executed yet, we need to get the executed trees from the inner LumioDB
        // This is because when we call save_transactions for the genesis block, we call [LumioDB::save_transactions]
        // where there is an expectation that the root of the SMTs are the same pointers. Here,
        // we get from the inner LumioDB which ensures that the pointers match when save_transactions is called.
    ...
    fn get_latest_version(&self) -> Result<Version> {
        gauged_api("get_latest_version", || {
            self.latest_version
                .lock()
                .ok_or_else(|| LumioDbError::NotFound("No latest version found.".to_string()))
        })
    }
...
/// requests.
impl HashReader for FakeLumioDB {
...
fn error_if_too_many_requested(num_requested: u64, max_allowed: u64) -> Result<()> {
    if num_requested > max_allowed {
        Err(LumioDbError::TooManyRequested(num_requested, max_allowed).into())
    } else {
        Ok(())
    }
}
...
mod tests {
    ...
    proptest! {
        ...
        fn test_save_transactions(input in arb_blocks_to_commit()) {
            ...
            let db = FakeLumioDB::new(LumioDB::new_for_test(&tmp_dir));

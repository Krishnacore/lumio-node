...
use crate::{
    ...
    db::{lumiodb_internal::gauged_api, LumioDB},
    ledger_db::{
...
use lumio_storage_interface::{
    chunk_to_commit::ChunkToCommit, db_ensure as ensure, LumioDbError, DbReader, DbWriter, Result,
        ...
        if old_committed_version.is_none() || version > old_committed_version.unwrap() {
            ...
            if let Some(update_sender) = &self.update_subscriber {
                update_sender
                    ...
                    .map_err(|err| {
                        LumioDbError::Other(format!("Failed to send update to subscriber: {}", err))
                    })?;
            }

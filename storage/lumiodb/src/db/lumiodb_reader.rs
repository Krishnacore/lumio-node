...
use crate::{
    ...
    db::{
        lumiodb_internal::{error_if_too_many_requested, gauged_api, get_first_seq_num_and_limit},
        LumioDB,
    },
...
use lumio_storage_interface::{
    ...
    LumioDbError, BlockHeight, DbReader, LedgerSummary, Order, Result, MAX_REQUEST_LIMIT,
};
use lumio_types::{
...
impl DbReader for LumioDB {
    ...
    /// Returns the transaction by version, delegates to `LumioDB::get_transaction_with_proof`.
    ...
    fn get_table_info(&self, handle: TableHandle) -> Result<TableInfo> {
        gauged_api("get_table_info", || {
            self.get_table_info_option(handle)?
                .ok_or_else(|| LumioDbError::NotFound(format!("TableInfo for {:?}", handle)))
        })
    }

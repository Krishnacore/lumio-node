...
use crate::{LumioDB, EventStore, LedgerPrunerManager, PrunerManager};
use lumio_config::config::LedgerPrunerConfig;
use lumio_proptest_helpers::Index;
...
fn verify_event_store_pruner(events: Vec<Vec<ContractEvent>>) {
    ...
    let event_store = &lumio_db.event_store;
    ...
    lumio_db.ledger_db.event_db().write_schemas(batch).unwrap();

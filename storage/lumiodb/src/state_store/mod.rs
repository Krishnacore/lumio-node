...
use lumio_db_indexer::db_indexer::InternalIndexerDB;
...
use lumio_jellyfish_merkle::iterator::JellyfishMerkleIterator;
use lumio_logger::info;
use lumio_metrics_core::TimerHelper;
use lumio_schemadb::batch::{NativeBatch, SchemaBatch, WriteBatch};
...
use lumio_storage_interface::{
    ...
    LumioDbError, DbReader, Result, StateSnapshotReceiver,
};
use lumio_types::{
...
impl StateStore {
    ...
    // TODO: change to async comment once it does like https://github.com/lumio-labs/lumio-core/blob/159b00f3d53e4327523052c1b99dd9889bf13b03/storage/backup/backup-cli/src/backup_types/state_snapshot/restore.rs#L147 or overlap at least two chunks.
    ...
    pub fn get_all_jmt_nodes_referenced(
        &self,
        version: Version,
    ) -> Result<Vec<lumio_jellyfish_merkle::node_type::NodeKey>> {
        lumio_jellyfish_merkle::JellyfishMerkleTree::new(self.state_merkle_db.as_ref())
            .get_all_nodes_referenced(version)
    }

    #[cfg(test)]
    pub fn get_all_jmt_nodes(&self) -> Result<Vec<lumio_jellyfish_merkle::node_type::NodeKey>> {
        let mut iter = self
            .state_db
            .state_merkle_db
            .metadata_db()
        ...
        let mut keys: Vec<lumio_jellyfish_merkle::node_type::NodeKey> =
...
mod test_only {
    ...
    use lumio_schemadb::batch::SchemaBatch;

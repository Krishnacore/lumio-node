...
use crate::LumioDB;
use anyhow::anyhow;
use lumio_config::config::{NodeConfig, StorageDirPaths};
...
/// This is a wrapper around [LumioDB] that is used to bootstrap the node for fast sync mode
pub struct FastSyncStorageWrapper {
    // Used for storing genesis data during fast sync
    temporary_db_with_genesis: Arc<LumioDB>,
    // Used for restoring fast sync snapshot and all the read/writes afterwards
    db_for_fast_sync: Arc<LumioDB>,
...
impl FastSyncStorageWrapper {
    ...
    /// Otherwise, we return LumioDB directly and the FastSyncStorageWrapper is None
    pub fn initialize_dbs(
        config: &NodeConfig,
        internal_indexer_db: Option<InternalIndexerDB>,
        update_sender: Option<Sender<(Instant, Version)>>,
    ) -> Result<Either<LumioDB, Self>> {
        let mut db_main = LumioDB::open(
    ...
    pub fn get_fast_sync_db(&self) -> Arc<LumioDB> {
        self.db_for_fast_sync.clone()
    }

    pub fn get_temporary_db_with_genesis(&self) -> Arc<LumioDB> {
    ...
    pub(crate) fn get_lumio_db_read_ref(&self) -> &LumioDB {
    ...
    pub(crate) fn get_lumio_db_write_ref(&self) -> &LumioDB {
...
impl DbWriter for FastSyncStorageWrapper {
    ...
    fn finalize_state_snapshot(
        ...
        self.get_lumio_db_write_ref().finalize_state_snapshot(
            version,
            output_with_proof,
            ledger_infos,
        )?;

...
fn open_db(dir: &lumio_temppath::TempPath) -> DB {
...
fn open_db_read_only(dir: &lumio_temppath::TempPath) -> DB {
    DB::open_cf_readonly(&rocksdb::Options::default(), dir.path(), "test", get_cfds())
        .expect("Failed to open DB.")
}

fn open_db_as_secondary(dir: &lumio_temppath::TempPath, dir_sec: &lumio_temppath::TempPath) -> DB {
...
fn collect_values<S: Schema>(db: &TestDB) -> Vec<(S::Key, S::Value)> {
    ...
    iter.seek_to_first();
    iter.collect::<Result<Vec<_>, LumioDbError>>().unwrap()
}
...
fn test_open_as_secondary() {
    let tmpdir = lumio_temppath::TempPath::new();
    let tmpdir_sec = lumio_temppath::TempPath::new();
...
fn test_checkpoint() {
    let tmpdir = lumio_temppath::TempPath::new();
    let checkpoint = lumio_temppath::TempPath::new();
    {
        let db = open_db(&tmpdir);
        db.put::<TestSchema1>(&TestField(0), &TestField(0)).unwrap();

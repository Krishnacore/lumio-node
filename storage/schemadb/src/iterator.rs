...
use crate::{
    IntoDbResult, KeyCodec, Schema, SeekKeyCodec, ValueCodec, LUMIO_SCHEMADB_ITER_BYTES,
    ...
    S: Schema,
{
    ...
    pub fn seek_to_first(&mut self) {
        let _timer = LUMIO_SCHEMADB_SEEK_LATENCY_SECONDS
    ...
    /// `seek_key`.
    pub fn seek<SK>(&mut self, seek_key: &SK) -> lumio_storage_interface::Result<()>
    where
        SK: SeekKeyCodec<S>,
    {
        let _timer =
            LUMIO_SCHEMADB_SEEK_LATENCY_SECONDS.timer_with(&[S::COLUMN_FAMILY_NAME, "seek"]);
    ...
    pub fn seek_for_prev<SK>(&mut self, seek_key: &SK) -> lumio_storage_interface::Result<()>
    where
        SK: SeekKeyCodec<S>,
    {
        let _timer = LUMIO_SCHEMADB_SEEK_LATENCY_SECONDS
    ...
    fn next_impl(&mut self) -> lumio_storage_interface::Result<Option<(S::Key, S::Value)>> {
        let _timer = LUMIO_SCHEMADB_ITER_LATENCY_SECONDS.timer_with(&[S::COLUMN_FAMILY_NAME]);
        ...
        LUMIO_SCHEMADB_ITER_BYTES.observe_with(
...
where
    S: Schema,
{
    type Item = lumio_storage_interface::Result<(S::Key, S::Value)>;

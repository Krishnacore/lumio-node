...
use lumio_db::{db::test_helper::arb_blocks_to_commit, LumioDB};
...
pub fn tmp_db_empty() -> (TempPath, Arc<LumioDB>) {
    let tmpdir = TempPath::new();
    let db = Arc::new(LumioDB::new_for_test(&tmpdir));
...
pub fn start_local_backup_service(db: Arc<LumioDB>) -> (Runtime, u16) {

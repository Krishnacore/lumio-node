...
use lumio_api::bootstrap as bootstrap_api;
use lumio_build_info::build_information;
use lumio_config::config::{merge_node_config, NodeConfig, PersistableConfig};
use lumio_framework::ReleaseBundle;
use lumio_genesis::builder::GenesisConfiguration;
use lumio_logger::{prelude::*, telemetry_log_writer::TelemetryLog, Level, LoggerFilterUpdater};
use lumio_state_sync_driver::driver_factory::StateSyncRuntimes;
use lumio_types::{
...
pub fn start_and_report_ports(
    ...
    // Setup panic handler
    lumio_crash_handler::setup_panic_handler();
    ...
    R: rand::RngCore + rand::CryptoRng,
{
    ...
    let test_dir =
        test_dir.unwrap_or_else(|| lumio_temppath::TempPath::new().as_ref().to_path_buf());
    ...
    R: rand::RngCore + rand::CryptoRng,
{
    ...
    lumio_config::config::sanitize_node_config(validators[0].config.override_config_mut())?;
...
pub fn setup_environment_and_start_node(
    ...
    // Set the chain_id in global LumioNodeIdentity
    lumio_node_identity::set_chain_id(chain_id)?;
...
fn verify_tool() {
    ...
    LumioNodeArgs::command().debug_assert()
}

...
use lumio_config::config::{NodeConfig, WaypointConfig};
use lumio_event_notifications::EventSubscriptionService;
use lumio_infallible::RwLock;
use lumio_storage_interface::{DbReader, DbReaderWriter, DbWriter};
...
fn test_create_single_node_test_config() {
    ...
    let test_dir = lumio_temppath::TempPath::new().as_ref().to_path_buf();
    fs::DirBuilder::new()
        .recursive(true)
        .create(&test_dir)

...
use lumio_config::{config::NodeConfig, utils::get_genesis_txn};
use lumio_db::{fast_sync_storage_wrapper::FastSyncStorageWrapper, LumioDB};
use lumio_db_indexer::db_indexer::InternalIndexerDB;
use lumio_executor::db_bootstrapper::maybe_bootstrap;
...
pub(crate) fn maybe_apply_genesis(
    ...
    if let Some(genesis) = get_genesis_txn(node_config) {
        let ledger_info_opt =
            maybe_bootstrap::<LumioVMBlockExecutor>(db_rw, genesis, genesis_waypoint)
...
/// In consensus-only mode, return a in-memory based [FakeLumioDB] and
...
fn create_rocksdb_checkpoint_and_change_working_dir(
    ...
    LumioDB::create_checkpoint(
        &source_dir,
        &checkpoint_dir,
        node_config.storage.rocksdb_configs.enable_storage_sharding,
    )
    .expect("LumioDB checkpoint creation failed.");

    // Create a consensus db checkpoint
    lumio_consensus::create_checkpoint(&source_dir, &checkpoint_dir)
        .expect("ConsensusDB checkpoint creation failed.");

    // Create a state sync db checkpoint
    let state_sync_db =
        lumio_state_sync_driver::metadata_storage::PersistentMetadataStorage::new(&source_dir);
    state_sync_db

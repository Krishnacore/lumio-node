...
pub fn benchmark(args: &[String]) {
    let cmd_line_parser = Command::new("benchmark")
        ...
        .arg(
            Arg::new("lumio-natives")
                .short('a')
                .long("lumio")
                .num_args(0)
                .action(Set)
                .help("whether the lumio-natives should be included."),
        )
        .arg(
    ...
    let use_lumio_natives = matches.contains_id("lumio-natives");

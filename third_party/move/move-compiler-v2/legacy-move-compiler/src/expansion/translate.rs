...
// This is a hack to recognize LUMIO StdLib, Framework, and Token libs to avoid warnings on some old errors.
// This will be removed after library attributes are cleaned up.
// (See https://github.com/aptos-labs/aptos-core/issues/9410)
fn module_is_in_lumio_libs(module_address: Option<Spanned<Address>>) -> bool {
    const LUMIO_STDLIB_NAME: &str = "lumio_std";
    static LUMIO_STDLIB_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x1").unwrap());
    const LUMIO_FRAMEWORK_NAME: &str = "lumio_framework";
    static LUMIO_FRAMEWORK_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x1").unwrap());
    const LUMIO_TOKEN_NAME: &str = "lumio_token";
    static LUMIO_TOKEN_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
        Lazy::new(|| NumericalAddress::parse_str("0x3").unwrap());
    const LUMIO_TOKEN_OBJECTS_NAME: &str = "lumio_token_objects";
    static LUMIO_TOKEN_OBJECTS_NUMERICAL_ADDRESS: Lazy<NumericalAddress> =
    ...
    match &module_address {
        Some(spanned_address) => {
            ...
            match address {
                ...
                {
                    Some(spanned_symbol) => {
                        ((&spanned_symbol.value as &str) == LUMIO_STDLIB_NAME
                            && (spanned_numerical_address.value == *LUMIO_STDLIB_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == LUMIO_FRAMEWORK_NAME
                                && (spanned_numerical_address.value
                                    == *LUMIO_FRAMEWORK_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == LUMIO_TOKEN_NAME
                                && (spanned_numerical_address.value
                                    == *LUMIO_TOKEN_NUMERICAL_ADDRESS))
                            || ((&spanned_symbol.value as &str) == LUMIO_TOKEN_OBJECTS_NAME
                                && (spanned_numerical_address.value
                                    == *LUMIO_TOKEN_OBJECTS_NUMERICAL_ADDRESS))
                    },
                    None => false,
                },
...
fn module_(
    ...
    if context.env.flags().warn_of_deprecation_use_in_lumio_libs() {
        context.in_lumio_libs = false;
    } else {
        context.in_lumio_libs = module_is_in_lumio_libs(module_address);
    }
...
fn check_for_deprecated_module_use(context: &mut Context, mident: &ModuleIdent) -> bool {
    ...
    if !warn_deprecation || context.in_deprecated_code || context.in_lumio_libs {
        return false;
    }

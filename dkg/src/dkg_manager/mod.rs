...
use lumio_crypto::Uniform;
use lumio_infallible::duration_since_epoch;
use lumio_logger::{debug, error, info, warn};
use lumio_types::{
    dkg::{
...
use lumio_validator_transaction_pool::{TxnGuard, VTxnPoolState};
use fail::fail_point;
...
pub struct DKGManager<DKG: DKGTrait> {
    ...
    agg_trx_tx: Option<lumio_channel::Sender<(), DKG::Transcript>>,

    // When we put vtxn in the pool, we also put a copy of this so later pool can notify us.
    pull_notification_tx: lumio_channel::Sender<(), Arc<ValidatorTransaction>>,
    pull_notification_rx: lumio_channel::Receiver<(), Arc<ValidatorTransaction>>,
...
impl<DKG: DKGTrait> DKGManager<DKG> {
    pub fn new(
        ...
        let (pull_notification_tx, pull_notification_rx) =
            lumio_channel::new(QueueStyle::KLAST, 1, None);
        Self {
            dealer_sk,
            my_addr,
            my_index,
    ...
    pub async fn run(
        ...
        mut dkg_start_event_rx: lumio_channel::Receiver<(), DKGStartEvent>,
        mut rpc_msg_rx: lumio_channel::Receiver<
            AccountAddress,
        ...
        let (agg_trx_tx, mut agg_trx_rx) = lumio_channel::new(QueueStyle::KLAST, 1, None);

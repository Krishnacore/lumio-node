...
describe('releaseImages', () => {
    ...
    it('gets lumio-node as the node image group', () => {
        const prefix = 'lumio-node-vX.Y.Z';
        const releaseGroup = getImageReleaseGroupByImageTagPrefix(prefix);
        expect(releaseGroup).toEqual('lumio-node');
    });
    it('determines image is a release image', () => {
        expect(isReleaseImage("nightly-banana")).toEqual(false);
        expect(isReleaseImage("lumio-node-v1.2.3")).toEqual(true);
    });
    it('asserts version match', async () => {
        ...
        expect(() => assertTagMatchesSourceVersion("lumio-node-v0.0.0")).toThrow("image tag does not match cargo version: lumio-node-v0.0.0");
    });
});

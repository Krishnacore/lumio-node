...
function usage {
  echo "Usage:"
  echo "Installs or updates necessary dev tools for lumiolabs/lumio-core."
  ...
  echo "This command must be called from the root folder of the Lumio-core project."
}
...
function install_rustup_components_and_nightly {
  ...
  rustup update
  rustup toolchain install stable # Install the latest toolchain to ensure that dependencies can always be built (even if lumio-core is behind)
...
# this is needed for hdpi crate from lumio-ledger
...
build, test and inspect Lumio Core.
...
if [ ! -f rust-toolchain.toml ]; then
  echo "Unknown location. Please run this from the lumio-core repository. Abort."
  exit 1
fi

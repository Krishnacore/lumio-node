...
use lumio_executor_types::ChunkCommitNotification;
use lumio_infallible::{Mutex, RwLock};
use lumio_mempool_notifications::MempoolNotificationListener;
use lumio_storage_interface::{LumioDbError, DbReaderWriter};
...
async fn test_save_states_invalid_chunk() {
    ...
    snapshot_receiver
        .expect_add_chunk()
        .with(always(), always())
        .returning(|_, _| Err(LumioDbError::Other("Invalid chunk!".to_string())));

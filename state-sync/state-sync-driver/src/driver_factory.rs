...
use lumio_consensus_notifications::ConsensusNotificationListener;
use lumio_data_client::client::LumioDataClient;
use lumio_data_streaming_service::streaming_client::StreamingServiceClient;
use lumio_event_notifications::{EventNotificationSender, EventSubscriptionService};
use lumio_executor_types::ChunkExecutorTrait;
...
impl DriverFactory {
    ...
    >(
        ...
        let driver_runtime = if create_runtime {
            let runtime = lumio_runtimes::spawn_named_runtime("sync-driver".into(), None);
            Some(runtime)
        } else {
            None
        };
...
impl StateSyncRuntimes {
    pub fn new(
        lumio_data_client: Runtime,
        state_sync: DriverFactory,
        storage_service: Runtime,
        streaming_service: Runtime,
    ) -> Self {
        Self {
            _lumio_data_client: lumio_data_client,
            state_sync,

...
use lumio_data_client::{
    ...
    interface::LumioDataClientInterface,
};
...
pub struct DataStreamingService<T> {
    ...
    stream_update_listener: lumio_channel::Receiver<(), StreamUpdateNotification>,
...
impl<T: LumioDataClientInterface + Send + Clone + 'static> DataStreamingService<T> {
    pub fn new(
        data_client_config: LumioDataClientConfig,
        streaming_service_config: DataStreamingServiceConfig,
        lumio_data_client: T,
        ...
        let (stream_update_notifier, stream_update_listener) =
            lumio_channel::new(QueueStyle::LIFO, STREAM_PROGRESS_UPDATE_CHANNEL_SIZE, None);
...
fn spawn_global_data_summary_refresher<T: LumioDataClientInterface + Send + Clone + 'static>(
...
fn refresh_global_data_summary<T: LumioDataClientInterface + Send + Clone + 'static>(
    lumio_data_client: T,
    cached_global_data_summary: Arc<ArcSwap<GlobalDataSummary>>,
) {
    // Fetch the global data summary and update the cache
    match fetch_global_data_summary(lumio_data_client) {
...
fn fetch_global_data_summary<T: LumioDataClientInterface + Send + Clone + 'static>(
    lumio_data_client: T,
) -> Result<GlobalDataSummary, Error> {
    // Fetch the global data summary from the data client
    let global_data_summary = lumio_data_client.get_global_data_summary();
...
fn verify_optimal_chunk_sizes(optimal_chunk_sizes: &OptimalChunkSizes) -> Result<(), Error> {
    ...
    {
        Err(Error::LumioDataClientResponseIsInvalid(format!(

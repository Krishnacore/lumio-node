...
pub enum Error {
    ...
    #[error("Error returned by the lumio data client: {0}")]
    LumioDataClientError(String),
    #[error("The response from the lumio data client is invalid! Error: {0}")]
    LumioDataClientResponseIsInvalid(String),
...
impl Error {
    ...
    pub fn get_label(&self) -> &'static str {
        match self {
            ...
            Self::LumioDataClientError(_) => "lumio_data_client_error",
            Self::LumioDataClientResponseIsInvalid(_) => "lumio_data_client_response_is_invalid",
...
impl From<lumio_data_client::error::Error> for Error {
    fn from(error: lumio_data_client::error::Error) -> Self {
        Error::LumioDataClientError(error.to_string())
    }
}

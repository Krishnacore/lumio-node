...
use lumio_config::{
    config::{LumioDataClientConfig, BaseConfig},
    network_id::PeerNetworkId,
};
use lumio_id_generator::{IdGenerator, U64IdGenerator};
use lumio_infallible::Mutex;
use lumio_logger::{info, sample, sample::SampleRate, trace, warn};
use lumio_network::{
...
/// An [`LumioDataClientInterface`] that fulfills requests from remote peers' Storage Service
/// over LumioNet.
///
/// The `LumioDataClient`:
///
/// 1. Sends requests to connected Lumio peers.
...
/// and/or threads.
#[derive(Clone, Debug)]
pub struct LumioDataClient {
    /// The base config of the node.
    base_config: Arc<BaseConfig>,
    /// The config for the LumioNet data client.
    data_client_config: Arc<LumioDataClientConfig>,
    /// The underlying LumioNet storage service client.
...
impl LumioDataClient {
        ...
        E: Into<Error>,
    {
        ...
        for peer in peers {
            ...
            let lumio_data_client = self.clone();
    ...
    async fn send_request_to_peer(
        ...
        match result {
            Ok(response) => {
                ...
                // with this RPC.
                let response_callback = LumioNetResponseCallback {
            ...
            Err(error) => {
                ...
                let client_error = match error {
                    lumio_storage_service_client::Error::RpcError(rpc_error) => match rpc_error {
                    ...
                    lumio_storage_service_client::Error::StorageServiceError(err) => {
...
#[async_trait]
impl LumioDataClientInterface for LumioDataClient {
...
/// The LumioNet-specific request context needed to update a peer's scoring.
struct LumioNetResponseCallback {
    data_client: LumioDataClient,
    id: ResponseId,
    peer: PeerNetworkId,
    request: StorageServiceRequest,
}

impl ResponseCallback for LumioNetResponseCallback {
...
impl fmt::Debug for LumioNetResponseCallback {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("LumioNetResponseCallback")
            .field("data_client", &"..")
            .field("id", &self.id)

...
/// An error returned by the Lumio Data Client for failed API calls.
...
impl From<lumio_storage_service_client::Error> for Error {
    fn from(error: lumio_storage_service_client::Error) -> Self {
        Self::UnexpectedErrorEncountered(error.to_string())
    }
}

impl From<lumio_storage_service_types::responses::Error> for Error {
    fn from(error: lumio_storage_service_types::responses::Error) -> Self {

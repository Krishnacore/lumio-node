...
use crate::{
    ...
    interface::{LumioDataClientInterface, Response},
...
use lumio_config::{
    config::{LumioDataClientConfig, LumioDataPollerConfig},
    network_id::PeerNetworkId,
};
use lumio_logger::{debug, error, info, sample, sample::SampleRate, warn};
...
pub struct DataSummaryPoller {
    data_client_config: Arc<LumioDataClientConfig>, // The configuration for the data client
    data_client: LumioDataClient,                   // The data client through which to poll peers
...
pub async fn start_poller(poller: DataSummaryPoller) {
    ...
    info!((LogSchema::new(LogEntry::DataSummaryPoller).message("Starting the Lumio data poller!")));
    loop {
...
pub(crate) fn calculate_num_peers_to_poll(
    ...
    data_poller_config: LumioDataPollerConfig,
) -> u64 {

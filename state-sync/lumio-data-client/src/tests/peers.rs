...
use lumio_config::{
    config::{LumioDataClientConfig, LumioDataMultiFetchConfig},
    network_id::{NetworkId, PeerNetworkId},
};
use lumio_storage_service_server::network::NetworkRequest;
use lumio_storage_service_types::{
    requests::DataRequest,
    responses::{CompleteDataRange, DataResponse, StorageServerSummary, StorageServiceResponse},
    StorageServiceError,
};
use lumio_types::transaction::{TransactionListWithProof, TransactionListWithProofV2};
...
async fn all_bad_peers_with_invalid_responses() {
    ...
    for peer_priority in PeerPriority::get_all_ordered_priorities() {
        ...
        let data_client_config = LumioDataClientConfig {
            data_multi_fetch_config: LumioDataMultiFetchConfig {
...
fn verify_peer_states(client: &LumioDataClient, all_peers: HashSet<PeerNetworkId>) {

...
use crate::{
    client::LumioDataClient,
    error::Result,
    global_summary::GlobalDataSummary,
    interface::{LumioDataClientInterface, Response, SubscriptionRequestMetadata},
    poller::DataSummaryPoller,
    priority::PeerPriority,
};
use lumio_channels::{lumio_channel, message_queues::QueueStyle};
use lumio_config::{
    config::{LumioDataClientConfig, BaseConfig, RoleType},
...
use lumio_storage_service_server::network::{NetworkRequest, ResponseSender};
...
pub struct MockNetwork {
    ...
    peer_mgr_reqs_rxs:
        HashMap<NetworkId, lumio_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>>, // The peer manager request receivers
}

impl MockNetwork {
    pub fn new(
        base_config: Option<BaseConfig>,
        data_client_config: Option<LumioDataClientConfig>,
        networks: Option<Vec<NetworkId>>,
    ) -> (Self, MockTimeService, LumioDataClient, DataSummaryPoller) {
        ...
        for network in &networks {
            ...
            let queue_cfg = lumio_channel::Config::new(10).queue_style(QueueStyle::FIFO);
        ...
        let (client, poller) = LumioDataClient::new(
            data_client_config,
            base_config.clone(),
            mock_time.clone(),
...
pub fn create_mock_data_client() -> Arc<dyn LumioDataClientInterface + Send + Sync> {
    Arc::new(MockLumioDataClient::new())
}

// This automatically creates a MockLumioDataClient
mock! {
    pub LumioDataClient {}
...
impl DbReader for MockDatabaseReader {
    fn get_block_timestamp(&self, version: Version) -> lumio_storage_interface::Result<u64> {
        Ok(version * 100_000)
    }
}

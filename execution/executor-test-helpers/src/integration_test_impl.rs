...
use lumio_config::config::DEFAULT_MAX_NUM_NODES_PER_LRU_CACHE_SHARD;
...
pub fn test_execution_with_storage_impl() -> Arc<LumioDB> {
...
pub fn test_execution_with_storage_impl_inner(
    ...
    let (lumio_db, db, executor, waypoint) =
...
pub fn create_db_and_executor<P: AsRef<std::path::Path>>(
    ...
    Arc<LumioDB>,
    DbReaderWriter,
    BlockExecutor<LumioVMBlockExecutor>,
    Waypoint,
) {
    let (db, dbrw) = force_sharding
        .then(|| {
            DbReaderWriter::wrap(LumioDB::new_for_test_with_sharding(
                &path,
                DEFAULT_MAX_NUM_NODES_PER_LRU_CACHE_SHARD,
            ))
        })
        .unwrap_or_else(|| DbReaderWriter::wrap(LumioDB::new_for_test(&path)));
    let waypoint = bootstrap_genesis::<LumioVMBlockExecutor>(&dbrw, genesis).unwrap();

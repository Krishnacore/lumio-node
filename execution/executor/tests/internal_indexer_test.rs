...
use lumio_db_indexer::db_indexer::DBIndexer;
...
use lumio_types::{
    ...
    account_config::lumio_test_root_address,
...
#[cfg(test)]
pub fn create_test_db() -> (Arc<LumioDB>, LocalAccount) {
    // create test db
    let path = lumio_temppath::TempPath::new();
    let (genesis, validators) = lumio_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis));
    let core_resources_account: LocalAccount = LocalAccount::new(
        lumio_test_root_address(),
        AccountKey::from_private_key(lumio_vm_genesis::GENESIS_KEYPAIR.0.clone()),
        0,
    );
    let (lumio_db, _db, executor, _waypoint) =
    ...
    let signer = lumio_types::validator_signer::ValidatorSigner::new(
    ...
    let reconfig1 = core_resources_account.sign_with_transaction_builder(
        txn_factory.payload(lumio_stdlib::lumio_governance_force_end_epoch_test_only()),
    );
    ...
    (lumio_db, core_resources_account)
}
...
fn test_db_indexer_data() {
    ...
    // create test db
    let (lumio_db, core_account) = create_test_db();
    let total_version = lumio_db.expect_synced_version();
    assert_eq!(total_version, 11);
    let temp_path = TempPath::new();
    let mut node_config = lumio_config::config::NodeConfig::default();
    ...
    let db_indexer = DBIndexer::new(internal_indexer_db.clone(), lumio_db.clone());
    ...
    let expected_code = vec![
        ...
        ident_str!("lumio_coin"),
        ident_str!("lumio_hash"),
    ...
    let expected_resources = vec![
        ...
        (false, "0x1::stake::LumioCoinCapabilities"),
        ...
        (false, "0x1::lumio_governance::VotingRecords"),
        (false, "0x1::state_storage::StateStorageUsage"),
        (false, "0x1::lumio_governance::VotingRecordsV2"),
        ...
        (false, "0x1::lumio_governance::GovernanceConfig"),
        (false, "0x1::lumio_governance::GovernanceEvents"),
        ...
        (false, "0x1::transaction_fee::LumioCoinMintCapability"),
        (false, "0x1::transaction_fee::LumioFABurnCapabilities"),
        (false, "0x1::jwk_consensus_config::JWKConsensusConfig"),
        (false, "0x1::lumio_governance::ApprovedExecutionHashes"),
        (false, "0x1::lumio_governance::GovernanceResponsbility"),
        ...
        (false, "0x1::coin::CoinInfo<0x1::lumio_coin::LumioCoin>"),
        (
            false,

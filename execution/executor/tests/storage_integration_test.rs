...
use lumio_crypto::{hash::CryptoHash, PrivateKey};
...
use lumio_storage_interface::state_store::state_view::db_state_view::DbStateViewAtVersion;
use lumio_types::{
    account_config::{lumio_test_root_address, AccountResource, CORE_CODE_ADDRESS},
    block_metadata::BlockMetadata,
    on_chain_config::{LumioVersion, OnChainConfig, ValidatorSet},
...
fn test_genesis() {
    ...
    let (lumio_framework_account_resource, state_proof) = db
        .reader
    ...
    state_proof
        .verify(
            ...
            lumio_framework_account_resource.as_ref(),
        )
        .unwrap();
}
...
fn test_reconfiguration() {
    ...
    let txn1 = get_test_signed_transaction(
        ...
        Some(lumio_stdlib::lumio_coin_mint(validator_account, 1_000_000)),
    );
    ...
    // txn3 = set the lumio version for next epoch
    let txn3 = get_test_signed_transaction(
        lumio_test_root_address(),
        /* sequence_number = */ 1,
        genesis_key.clone(),
        genesis_key.public_key(),
        Some(lumio_stdlib::version_set_for_next_epoch(42)),
    );
    ...
    let txn4 = get_test_signed_transaction(
        ...
        genesis_key.clone(),
        genesis_key.public_key(),
        Some(lumio_stdlib::lumio_governance_force_end_epoch_test_only()),
    );
    ...
    assert_eq!(
        LumioVersion::fetch_config(&db_state_view).unwrap().major,
        42
    );
}

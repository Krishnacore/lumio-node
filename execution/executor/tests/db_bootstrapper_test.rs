...
use lumio_crypto::{ed25519::Ed25519PrivateKey, HashValue, PrivateKey, Uniform};
use lumio_db::LumioDB;
use lumio_executor::{
    block_executor::BlockExecutor,
    db_bootstrapper::{generate_waypoint, maybe_bootstrap},
};
use lumio_executor_test_helpers::{
...
use lumio_types::{
    ...
    account_config::{
        lumio_test_root_address, new_block_event_key, primary_lum_store, FungibleStoreResource,
...
#[test]
fn test_empty_db() {
    let genesis = lumio_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis.0));
    let tmp_dir = TempPath::new();
    let db_rw = DbReaderWriter::new(LumioDB::new_for_test(&tmp_dir));
    ...
    // Bootstrap empty DB.
    let waypoint =
        generate_waypoint::<LumioVMBlockExecutor>(&db_rw, &genesis_txn).expect("Should not fail.");
    maybe_bootstrap::<LumioVMBlockExecutor>(&db_rw, &genesis_txn, waypoint).unwrap();
    ...
    assert!(
        maybe_bootstrap::<LumioVMBlockExecutor>(&db_rw, &genesis_txn, waypoint)
            .unwrap()
            .is_none()
    );
}
...
fn execute_and_commit(txns: Vec<Transaction>, db: &DbReaderWriter, signer: &ValidatorSigner) {
    ...
    let executor = BlockExecutor::<LumioVMBlockExecutor>::new(db.clone());
    let output = executor
        .execute_block(
...
fn get_lumio_coin_mint_transaction(
    lumio_root_key: &Ed25519PrivateKey,
    lumio_root_seq_num: u64,
    account: &AccountAddress,
    amount: u64,
) -> Transaction {
    get_test_signed_transaction(
        lumio_test_root_address(),
        /* sequence_number = */ lumio_root_seq_num,
        lumio_root_key.clone(),
        lumio_root_key.public_key(),
        Some(lumio_stdlib::lumio_coin_mint(*account, amount)),
    )
}
...
fn get_account_transaction(
    ...
    get_test_signed_transaction(
        ...
        lumio_root_key.clone(),
        lumio_root_key.public_key(),
        Some(lumio_stdlib::lumio_account_create_account(*account)),
    )
}

fn get_lumio_coin_transfer_transaction(
    ...
    get_test_signed_transaction(
        ...
        Some(lumio_stdlib::lumio_coin_transfer(recipient, amount)),
    )
}
...
fn test_new_genesis() {
    let genesis = lumio_vm_genesis::test_genesis_change_set_and_validators(Some(1));
    let genesis_key = &lumio_vm_genesis::GENESIS_KEYPAIR.0;
    let genesis_txn = Transaction::GenesisTransaction(WriteSetPayload::Direct(genesis.0));
    // Create bootstrapped DB.
    let tmp_dir = TempPath::new();
    let db = DbReaderWriter::new(LumioDB::new_for_test(&tmp_dir));
    let waypoint = bootstrap_genesis::<LumioVMBlockExecutor>(&db, &genesis_txn).unwrap();
    ...
    let txn3 = get_lumio_coin_mint_transaction(genesis_key, 2, &account1, 200_000_000);
    let txn4 = get_lumio_coin_mint_transaction(genesis_key, 3, &account2, 200_000_000);
        ...
        WriteSetMut::new(vec![
            ...
            (
                StateKey::resource_group(
                    &primary_lum_store(account1),
                    &ObjectGroupResource::struct_tag(),
                ),
                WriteOp::legacy_modification(
                    lumio_transaction_simulation::FungibleStore::new(
                        account1,
                        AccountAddress::TEN,
                        100_000_000,
                        false,
    ...
    let waypoint = generate_waypoint::<LumioVMBlockExecutor>(&db, &genesis_txn).unwrap();
    assert!(
        maybe_bootstrap::<LumioVMBlockExecutor>(&db, &genesis_txn, waypoint)
            .unwrap()
            .is_some()
    );
    ...
    // Transfer some money.
    let txn = get_lumio_coin_transfer_transaction(account1, 0, &account1_key, account2, 50_000_000);

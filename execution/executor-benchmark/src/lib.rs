...
use lumio_config::config::{NodeConfig, PrunerConfig, NO_OP_STORAGE_PRUNER_CONFIG};
use lumio_db::LumioDB;
use lumio_executor::block_executor::BlockExecutor;
use lumio_jellyfish_merkle::metrics::{
    LUMIO_JELLYFISH_INTERNAL_ENCODED_BYTES, LUMIO_JELLYFISH_LEAF_ENCODED_BYTES,
};
...
use lumio_types::on_chain_config::{FeatureFlag, Features};
use lumio_vm::{lumio_vm::LumioVMBlockExecutor, LumioVM, VMBlockExecutor};
...
fn create_checkpoint(
    ...
    LumioDB::create_checkpoint(source_dir, checkpoint_dir, enable_storage_sharding)
...
enum InitializedBenchmarkWorkload {
    TransactionMix {
        transaction_generators: Vec<Box<dyn lumio_transaction_generator_lib::TransactionGenerator>>,
    ...
    V: VMBlockExecutor + 'static,
{
    ...
    let (mut config, genesis_key) =
        lumio_genesis::test_utils::test_config_with_custom_features(init_features);
    ...
    if let BenchmarkWorkload::TransactionMix(mix) = &workload {
        for (transaction_type, _) in mix {
            if matches!(transaction_type, CoinTransfer { non_conflicting, .. } if *non_conflicting)
            {
                ...
                // `lumio_executor_benchmark::transaction_generator::TransactionGenerator` needs to hold
    ...
    let initialized_workload = match workload {
        BenchmarkWorkload::TransactionMix(transaction_mix) => {
            ...
            let (transaction_generator_creator, phase) = init_workload::<LumioVMBlockExecutor>(
                transaction_mix,
                root_account.clone(),
    ...
    assert_eq!(0, lumio_logger::ERROR_LOG_COUNT.get());
    ...
    V: VMBlockExecutor + 'static,
{
    ...
    println!(
        ...
        LUMIO_JELLYFISH_INTERNAL_ENCODED_BYTES.get()
    );
    println!(
        "Total written leaf nodes value size: {} bytes",
        LUMIO_JELLYFISH_LEAF_ENCODED_BYTES.get()
    );
}
...
pub fn run_single_with_default_params(
    ...
    LumioVM::set_num_shards_once(1);
    LumioVM::set_concurrency_level_once(concurrency_level);
    LumioVM::set_processed_transactions_detailed_counters();
    ...
    features.enable(FeatureFlag::NEW_ACCOUNTS_DEFAULT_TO_FA_LUM_STORE);
    features.enable(FeatureFlag::OPERATIONS_DEFAULT_TO_FA_LUM_STORE);
    ...
    create_db_with_accounts::<LumioVMBlockExecutor>(
    ...
    run_benchmark::<LumioVMBlockExecutor>(
...
mod tests {
    use crate::{
        ...
        native::{
            lumio_vm_uncoordinated::LumioVMParallelUncoordinatedBlockExecutor,
    ...
    use lumio_config::config::NO_OP_STORAGE_PRUNER_CONFIG;
    use lumio_crypto::HashValue;
    use lumio_executor::block_executor::BlockExecutor;
    use lumio_executor_types::BlockExecutorTrait;
    use lumio_sdk::{transaction_builder::lumio_stdlib, types::LocalAccount};
    use lumio_temppath::TempPath;
    use lumio_transaction_generator_lib::WorkflowProgress;
    use lumio_transaction_workloads_lib::args::TransactionTypeArg;
    use lumio_types::{
        access_path::Path,
    ...
    fn test_compare_vm_and_vm_uncoordinated() {
        test_compare_prod_and_another_all_types::<LumioVMParallelUncoordinatedBlockExecutor>(true);
    }
    ...
    fn test_compare_prod_and_another_all_types<E: VMBlockExecutor>(values_match: bool) {
        ...
        non_fa_features.disable(FeatureFlag::NEW_ACCOUNTS_DEFAULT_TO_FA_LUM_STORE);
        non_fa_features.disable(FeatureFlag::OPERATIONS_DEFAULT_TO_FA_LUM_STORE);
        ...
        test_compare_prod_and_another::<E>(values_match, non_fa_features.clone(), |address| {
            lumio_stdlib::lumio_account_transfer(address, 1000)
        });

        test_compare_prod_and_another::<E>(
            values_match,
            non_fa_features,
            lumio_stdlib::lumio_account_create_account,
        );

        let mut fa_features = default_benchmark_features();
        fa_features.enable(FeatureFlag::NEW_ACCOUNTS_DEFAULT_TO_FA_LUM_STORE);
        fa_features.enable(FeatureFlag::OPERATIONS_DEFAULT_TO_FA_LUM_STORE);
        ...
        test_compare_prod_and_another::<E>(values_match, fa_features.clone(), |address| {
            lumio_stdlib::lumio_account_fungible_transfer_only(address, 1000)
        });
    ...
    fn test_compare_prod_and_another<E: VMBlockExecutor>(
        ...
        let (mut config, genesis_key) =
            lumio_genesis::test_utils::test_config_with_custom_features(features);
        ...
        let (txn, vm_result) = {
            ...
            let vm_executor = BlockExecutor::<LumioVMBlockExecutor>::new(vm_db.clone());
        ...
        E: VMBlockExecutor + 'static,
    {
        ...
        crate::db_generator::create_db_with_accounts::<LumioVMBlockExecutor>(
            100, /* num_accounts */
    ...
    fn test_benchmark_default() {
        test_generic_benchmark::<LumioVMBlockExecutor>(None, true);
    }

    #[test]
    fn test_publish_transaction() {
        LumioVM::set_num_shards_once(1);
        LumioVM::set_concurrency_level_once(4);
        LumioVM::set_processed_transactions_detailed_counters();
        test_generic_benchmark::<LumioVMBlockExecutor>(

...
impl NativeTransaction {
    pub fn parse(txn: &SignatureVerifiedTransaction) -> Self {
        match &txn.expect_valid() {
            lumio_types::transaction::Transaction::UserTransaction(user_txn) => {
                match user_txn.payload() {
                    lumio_types::transaction::TransactionPayload::EntryFunction(f) => {
                        match (
                            *f.module().address(),
                            f.module().name().as_str(),
                            f.function().as_str(),
                        ) {
                            (AccountAddress::ONE, "lumio_account", "fungible_transfer_only") => {
                                Self::FaTransfer {
                            ...
                            (AccountAddress::ONE, "lumio_account", "transfer") => Self::Transfer {
                                sender: user_txn.sender(),
                                sequence_number: user_txn.sequence_number(),
                                recipient: bcs::from_bytes(&f.args()[0]).unwrap(),
                                amount: bcs::from_bytes(&f.args()[1]).unwrap(),
                            ...
                            (AccountAddress::ONE, "lumio_account", "create_account") => {
                                Self::Transfer {
                                    sender: user_txn.sender(),
                                    sequence_number: user_txn.sequence_number(),
                                    recipient: bcs::from_bytes(&f.args()[0]).unwrap(),
                            ...
                            (AccountAddress::ONE, "lumio_account", "batch_transfer") => {
                                Self::BatchTransfer {
                                    sender: user_txn.sender(),
                                    sequence_number: user_txn.sequence_number(),
            ...
            lumio_types::transaction::Transaction::BlockEpilogue(_) => Self::BlockEpilogue,
            _ => unimplemented!(),
        }
    }
}

...
pub use crate::client_builder::{LumioBaseUrl, ClientBuilder};
use crate::{lumio::LumioVersion, error::RestError};
use anyhow::{anyhow, Result};
pub use lumio_api_types::{
...
use lumio_api_types::{
    ...
    LumioError, LumioErrorCode, BcsBlock, Block, GasEstimation, HexEncodedBytes, IndexResponse,
...
use lumio_logger::{debug, info, sample, sample::SampleRate};
use lumio_types::{
...
const X_LUMIO_SDK_HEADER_VALUE: &str = concat!("lumio-rust-sdk/", env!("CARGO_PKG_VERSION"));

type LumioResult<T> = Result<T, RestError>;
...
impl Client {
    pub fn builder(lumio_base_url: LumioBaseUrl) -> ClientBuilder {
        ClientBuilder::new(lumio_base_url)
    }

    pub fn new(base_url: Url) -> Self {
        Self::builder(LumioBaseUrl::Custom(base_url)).build()
    }
    ...
    pub fn version_path_base(mut self, version_path_base: String) -> LumioResult<Self> {
    ...
    pub fn build_path(&self, path: &str) -> LumioResult<Url> {
    ...
    pub async fn get_lumio_version(&self) -> LumioResult<Response<LumioVersion>> {
        self.get_resource::<LumioVersion>(CORE_CODE_ADDRESS, "0x1::version::Version")
            .await
    }
    ...
    pub async fn lookup_address(
        ...
        must_exist: bool,
    ) -> LumioResult<Response<AccountAddress>> {
        ...
        {
            Ok(inner) => Ok(inner),
            Err(RestError::Api(LumioErrorResponse {
                error:
                    LumioError {
                        error_code: LumioErrorCode::TableItemNotFound,
                        ..
                    },
                ..
            })) => {
    ...
    /// * A coin type (e.g. "0x1::lumio_coin::LumioCoin")
    /// * A fungible asset metadata address (e.g. "0xa")
    /// For more details, see: https://lumio.dev/en/build/apis/fullnode-rest-api-reference#tag/accounts/GET/accounts/{address}/balance/{asset_type}
    pub async fn get_account_balance(
        &self,
        address: AccountAddress,
        asset_type: &str,
    ) -> LumioResult<Response<u64>> {
        let url = self.build_path(&format!(
            "accounts/{}/balance/{}",
            address.to_hex(),
            asset_type
    ...
    pub async fn view_lum_account_balance_at_version(
        &self,
        address: AccountAddress,
        version: u64,
    ) -> LumioResult<Response<u64>> {
        self.view_account_balance_bcs_impl(address, "0x1::lumio_coin::LumioCoin", Some(version))
            .await
    }

    pub async fn view_lum_account_balance(
        &self,
        address: AccountAddress,
    ) -> LumioResult<Response<u64>> {
        self.view_account_balance_bcs_impl(address, "0x1::lumio_coin::LumioCoin", None)
            .await
    }

    pub async fn get_index(&self) -> LumioResult<Response<IndexResponse>> {
        self.get(self.build_path("")?).await
    }

    pub async fn get_index_bcs(&self) -> LumioResult<Response<IndexResponseBcs>> {
        let url = self.build_path("")?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }

    // TODO: Remove this, just use `get_index`: https://github.com/lumio-labs/lumio-core/issues/5597.
    pub async fn get_ledger_information(&self) -> LumioResult<Response<State>> {
    ...
    pub async fn view(
        ...
        request: &ViewRequest,
        version: Option<u64>,
    ) -> LumioResult<Response<Vec<serde_json::Value>>> {
    ...
    ) -> LumioResult<Response<Vec<UserTransaction>>> {
    ...
    pub async fn simulate_bcs(
        &self,
        txn: &SignedTransaction,
    ) -> LumioResult<Response<TransactionOnChainData>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions/simulate")?;
    ...
    pub async fn submit(
        &self,
        txn: &SignedTransaction,
    ) -> LumioResult<Response<PendingTransaction>> {
        let txn_payload = bcs::to_bytes(txn)?;
    ...
    pub async fn submit_bcs(&self, txn: &SignedTransaction) -> LumioResult<Response<()>> {
        let txn_payload = bcs::to_bytes(txn)?;
        let url = self.build_path("transactions")?;
    ...
    ) -> LumioResult<Response<TransactionsBatchSubmissionResult>> {
    ...
    pub async fn submit_and_wait(
        &self,
        txn: &SignedTransaction,
    ) -> LumioResult<Response<Transaction>> {
    ...
    where
        F: Fn(HashValue) -> Fut,
        Fut: Future<Output = LumioResult<WaitForTransactionResult<T>>>,
    {
        ...
        loop {
            ...
            match fetch(hash).await {
                ...
                Ok(WaitForTransactionResult::NotFound(error)) => {
                    if let RestError::Api(lumio_error_response) = error {
                        if let Some(state) = lumio_error_response.state {
                            if expiration_timestamp_secs <= state.timestamp_usecs / 1_000_000 {
                                if reached_mempool {
            ...
            if let Some(max_server_lag_wait_duration) = max_server_lag_wait {
                if lumio_infallible::duration_since_epoch().as_secs()
                    ...
                    debug!(
                        ...
                        if let Some(timestamp_usecs) = chain_timestamp_usecs {
                            format!(
                                ...
                                lumio_infallible::duration_since_epoch()
    ...
    pub async fn get_transactions(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> LumioResult<Response<Vec<Transaction>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transactions_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> LumioResult<Response<Vec<TransactionOnChainData>>> {
        let url = self.build_path("transactions")?;
    ...
    pub async fn get_transaction_by_hash_bcs(
        &self,
        hash: HashValue,
    ) -> LumioResult<Response<TransactionData>> {
    ...
    pub async fn get_account_resources(
        &self,
        address: AccountAddress,
    ) -> LumioResult<Response<Vec<Resource>>> {
    ...
    pub async fn get_account_resources_bcs(
        &self,
        address: AccountAddress,
    ) -> LumioResult<Response<BTreeMap<StructTag, Vec<u8>>>> {
    ...
    pub async fn get_account_resource(
        ...
        address: AccountAddress,
        resource_type: &str,
    ) -> LumioResult<Response<Option<Resource>>> {
        let url = self.build_path(&format!(
            "accounts/{}/resource/{}",
            address.to_hex(),
            resource_type
    ...
    pub async fn get_account_modules(
        &self,
        address: AccountAddress,
    ) -> LumioResult<Response<Vec<MoveModuleBytecode>>> {
        self.paginate_with_cursor(
    ...
    pub async fn get_account_modules_bcs(
        &self,
        address: AccountAddress,
    ) -> LumioResult<Response<BTreeMap<MoveModuleId, Vec<u8>>>> {
        self.paginate_with_cursor_bcs(
            &format!("accounts/{}/modules", address.to_hex()),
            MODULES_PER_CALL_PAGINATION,
            None,
    ...
    pub async fn get_account_module(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> LumioResult<Response<MoveModuleBytecode>> {
        let url = self.build_path(&format!(
            "accounts/{}/module/{}",
            address.to_hex(),
            module_name
    ...
    pub async fn get_account_module_bcs(
        ...
        address: AccountAddress,
        module_name: &str,
    ) -> LumioResult<Response<bytes::Bytes>> {
        let url = self.build_path(&format!(
            "accounts/{}/module/{}",
            address.to_hex(),
            module_name
    ...
    pub async fn get_account_events(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> LumioResult<Response<Vec<VersionedEvent>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
    ...
    pub async fn get_account_sequence_number(
        ...
        match res {
            ...
            Err(error) => match error {
                RestError::Api(error) => {
                    if matches!(error.error.error_code, LumioErrorCode::AccountNotFound) {
    ...
    pub async fn get_account_events_bcs(
        ...
        start: Option<u64>,
        limit: Option<u16>,
    ) -> LumioResult<Response<Vec<EventWithVersion>>> {
        let url = self.build_path(&format!(
            "accounts/{}/events/{}/{}",
            address.to_hex_literal(),
            struct_tag,
    ...
    pub async fn get_account(&self, address: AccountAddress) -> LumioResult<Response<Account>> {
    ...
    pub async fn get_account_bcs(
        &self,
        address: AccountAddress,
    ) -> LumioResult<Response<AccountResource>> {
        let url = self.build_path(&format!("accounts/{}", address.to_hex()))?;
        let response = self.get_bcs(url).await?;
        Ok(response.and_then(|inner| bcs::from_bytes(&inner))?)
    }
    ...
    pub async fn get_account_transaction_summaries(
        ...
        limit: Option<u16>,
    ) -> LumioResult<Response<Vec<IndexedTransactionSummary>>> {
        let url = self.build_path(&format!(
    ...
    pub async fn estimate_gas_price(&self) -> LumioResult<Response<GasEstimation>> {
        let url = self.build_path("estimate_gas_price")?;
        let response = self.inner.get(url).send().await?;
        self.json(response).await
    }

    pub async fn set_failpoint(&self, name: String, actions: String) -> LumioResult<String> {
    ...
    ) -> LumioResult<(reqwest::Response, State)> {
        if !response.status().is_success() {
            Err(parse_error(response).await)
        } else {
    ...
    pub async fn health_check(&self, seconds: u64) -> LumioResult<()> {
    ...
    async fn get<T: DeserializeOwned>(&self, url: Url) -> LumioResult<Response<T>> {
        self.json(self.inner.get(url).send().await?).await
    }

    async fn get_bcs(&self, url: Url) -> LumioResult<Response<bytes::Bytes>> {
        let response = self.inner.get(url).header(ACCEPT, BCS).send().await?;
        self.check_and_parse_bcs_response(response).await
    }

    pub async fn make_prover_request(&self, req: ProverRequest) -> LumioResult<ZeroKnowledgeSig> {
    ...
    pub async fn make_pepper_request(&self, req: PepperRequest) -> LumioResult<Pepper> {
    ...
    where
        F: Fn() -> Fut,
        RetryFun: Fn(StatusCode, Option<LumioError>) -> bool,
        Fut: Future<Output = LumioResult<T>>,
    {
    ...
    pub async fn paginate_with_cursor_bcs<T: for<'a> Deserialize<'a> + Ord>(
        ...
        limit_per_request: u64,
        ledger_version: Option<u64>,
    ) -> LumioResult<Response<BTreeMap<T, Vec<u8>>>> {
...
pub fn retriable_with_404(status_code: StatusCode, lumio_error: Option<LumioError>) -> bool {
    retriable(status_code, lumio_error) | matches!(status_code, StatusCode::NOT_FOUND)
}

pub fn retriable(status_code: StatusCode, _lumio_error: Option<LumioError>) -> bool {
    matches!(
        status_code,
...
fn parse_state(response: &reqwest::Response) -> LumioResult<State> {

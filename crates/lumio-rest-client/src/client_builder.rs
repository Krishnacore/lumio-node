...
use crate::{
    get_version_path_with_base, Client, DEFAULT_VERSION_PATH_BASE, X_LUMIO_SDK_HEADER_VALUE,
};
use anyhow::Result;
...
impl LumioBaseUrl {
    pub fn to_url(&self) -> Url {
        match self {
            LumioBaseUrl::Mainnet => Url::from_str("https://api.mainnet.lumiolabs.com").unwrap(),
            LumioBaseUrl::Devnet => Url::from_str("https://api.devnet.lumiolabs.com").unwrap(),
            LumioBaseUrl::Testnet => Url::from_str("https://api.testnet.lumiolabs.com").unwrap(),
            LumioBaseUrl::Custom(url) => url.to_owned(),
        }
    }
}
...
impl ClientBuilder {
    pub fn new(lumio_base_url: LumioBaseUrl) -> Self {
        let mut headers = HeaderMap::new();
        headers.insert(
            X_LUMIO_CLIENT,
            HeaderValue::from_static(X_LUMIO_SDK_HEADER_VALUE),
        );

        let mut client_builder = Self {
            reqwest_builder: ReqwestClient::builder(),
            base_url: lumio_base_url.to_url(),

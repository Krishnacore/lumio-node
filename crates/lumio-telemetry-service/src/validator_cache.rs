...
use lumio_config::config::{Peer, PeerRole, PeerSet};
use lumio_infallible::RwLock;
use lumio_rest_client::Response;
use lumio_types::{
...
impl PeerSetCacheUpdater {
    ...
    async fn update_for_chain(
        ...
        let client = lumio_rest_client::Client::new(Url::parse(url).map_err(|e| {
...
mod tests {
    ...
    use lumio_infallible::RwLock;
    use lumio_rest_client::lumio_api_types::*;
    use lumio_types::{
    ...
    async fn test_validator_cache_updater_with_invalid_address() {
        ...
        let mock = server.mock(|when, then| {
            ...
            then.status(200)
                ...
                .header(X_LUMIO_CHAIN_ID, "25")
                .header(X_LUMIO_EPOCH, "10")
                .header(X_LUMIO_LEDGER_VERSION, "10")
                .header(X_LUMIO_LEDGER_OLDEST_VERSION, "2")
                .header(X_LUMIO_BLOCK_HEIGHT, "25")
                .header(X_LUMIO_OLDEST_BLOCK_HEIGHT, "10")
                .header(X_LUMIO_LEDGER_TIMESTAMP, "10");
        });
    ...
    async fn test_validator_cache_updater_with_valid_address() {
        ...
        let validator_info = ValidatorInfo::new(
            ...
            ValidatorConfig::new(
                ...
                bcs::to_bytes(&vec![NetworkAddress::from_str("/dns/fullnode0.testnet.lumiolabs.com/tcp/6182/noise-ik/0xea19ab47ed9191865f15d85d751ed0663205c0b2f0f465714b1947c023715973/handshake/0").unwrap()]).unwrap(),
                2,
            ),
        );

...
#[clap(name = "Lumio Telemetry Service", author, version)]
pub struct LumioTelemetryServiceArgs {
    #[clap(short = 'f', long, value_parser)]
    config_path: PathBuf,
}

impl LumioTelemetryServiceArgs {
    pub async fn run(self) {
        ...
        let validators = Arc::new(lumio_infallible::RwLock::new(HashMap::new()));
        let validator_fullnodes = Arc::new(lumio_infallible::RwLock::new(HashMap::new()));
        let peer_locations = Arc::new(lumio_infallible::RwLock::new(HashMap::new()));
...
fn verify_tool() {
    ...
    LumioTelemetryServiceArgs::command().debug_assert()
}

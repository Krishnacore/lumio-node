...
The Lumio Faucet is a service that runs alongside a test network and mints coins for users to test and develop on Lumio.
...
In all cases, if a directory holds a crate, the name of that crate is `lumio-faucet-<directory>`. For example the name of the crate in `metrics-server/` is `lumio-faucet-metrics-server`.
...
```
cargo run -p lumio-faucet-service -- run-simple --key <private_key> --node-url <api_url> --chain-id TESTING
```

Another example, running alongside a localnet (without `--use-faucet`):
```
cargo run -p lumio -- node run-local-testnet --force-restart --assume-yes
cargo run -p lumio-faucet-service -- run-simple --key ~/.lumio/testnet/mint.key --node-url http://127.0.0.1:8080 --chain-id TESTING
```
...
```
cargo run -p lumio-faucet-service -- run -c <path_to_config_file>
```
...
```
cd lumio-move/move-examples/scripts/minter
lumio move compile
```

If you have issues with this, try deleting `~/.move`, updating your Lumio CLI, and changing the LumioFramework version.
...
```
cd crates/lumio-faucet/configs
ls . | xargs -I@ cargo run -p lumio-faucet-service -- validate-config -c @
```

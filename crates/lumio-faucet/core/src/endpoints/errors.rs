...
pub struct LumioTapError {
    /// A message describing the error
    pub message: String,
    /// A code describing the error for programmatic use.
    pub error_code: LumioTapErrorCode,
...
impl LumioTapError {
    pub fn new(message: String, error_code: LumioTapErrorCode) -> Self {
        if *USE_HELPFUL_ERRORS.get().unwrap_or(&true) {
            ...
            Self {
                ...
                error_code: LumioTapErrorCode::YeahNahYeahYeahYeahNahYeahNah,
                rejection_reasons: vec![],
                txn_hashes: vec![],
            }
        }
...
impl std::fmt::Display for LumioTapError {
...
impl From<LumioTapError> for LumioTapErrorResponse {
    fn from(error: LumioTapError) -> Self {
...
pub enum LumioTapErrorCode {
    ...
    /// Failed when making requests to the Lumio API.
    LumioApiError = 50,
...
impl LumioTapErrorCode {
    pub fn status(&self) -> StatusCode {
        match self {
            LumioTapErrorCode::InvalidRequest
            | LumioTapErrorCode::AccountDoesNotExist
            | LumioTapErrorCode::EndpointNotEnabled => StatusCode::BAD_REQUEST,
            LumioTapErrorCode::Rejected
            | LumioTapErrorCode::SourceIpMissing
            | LumioTapErrorCode::TransactionFailed
            | LumioTapErrorCode::AuthTokenInvalid => StatusCode::FORBIDDEN,
            LumioTapErrorCode::LumioApiError
            | LumioTapErrorCode::TransactionTimedOut
            | LumioTapErrorCode::SerializationError
            | LumioTapErrorCode::BypasserError
            | LumioTapErrorCode::CheckerError
            | LumioTapErrorCode::StorageError
            | LumioTapErrorCode::InternalError => StatusCode::INTERNAL_SERVER_ERROR,
            LumioTapErrorCode::ServerOverloaded | LumioTapErrorCode::FunderAccountProblem => {
                StatusCode::SERVICE_UNAVAILABLE
            },
            LumioTapErrorCode::YeahNahYeahYeahYeahNahYeahNah => StatusCode::IM_A_TEAPOT,
            // We shouldn't get here, this code is only used in error_converter.rs.
            LumioTapErrorCode::WebFrameworkError => StatusCode::INTERNAL_SERVER_ERROR,
        }
    }
}

...
use super::{errors::LumioTapErrorResponse, ApiTags, LumioTapError, LumioTapErrorCode};
...
impl CaptchaApi {
    ...
    async fn request_captcha(&self) -> Result<Response<Binary<Vec<u8>>>, LumioTapErrorResponse> {
        if !self.enabled {
            return Err(LumioTapError::new(
                "The CaptchaChecker is not enabled".to_string(),
                LumioTapErrorCode::EndpointNotEnabled,
            )
            .into());
        }
        ...
        let (key, image) = match captcha_manager.create_challenge() {
            ...
            Err(e) => {
                return Err(
                    LumioTapError::new_with_error_code(e, LumioTapErrorCode::CheckerError).into(),
                );
            },
        };

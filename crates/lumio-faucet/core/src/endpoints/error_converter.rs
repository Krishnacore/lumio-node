...
use super::errors::LumioTapErrorResponse;
...
/// be a JSON representation of LumioTapError. For our own errors, this is exactly
...
pub async fn convert_error(error: poem::Error) -> impl poem::IntoResponse {
    ...
    if is_framework_error {
        // Build an LumioTapErrorResponse and then reset its status code
...
fn build_error_response(error_string: String) -> Response {
    LumioTapErrorResponse::from(LumioTapError::new_with_error_code(
        error_string,
        LumioTapErrorCode::WebFrameworkError,
    ))
    .into_response()
}

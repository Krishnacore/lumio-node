...
impl FirebaseJwtVerifier {
    ...
    pub async fn validate_jwt(&self, headers: Arc<HeaderMap>) -> Result<String, LumioTapError> {
        ...
        let token_data = match verify.await {
            ...
            None => {
                return Err(LumioTapError::new(
                    ...
                    LumioTapErrorCode::AuthTokenInvalid,
                ));
            },
        };
...
pub fn jwt_sub(headers: Arc<HeaderMap>) -> Result<String, LumioTapError> {
    let is_jwt = headers
        .get(X_IS_JWT_HEADER)

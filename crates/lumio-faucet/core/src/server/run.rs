...
use lumio_config::keys::ConfigKey;
use lumio_faucet_metrics_server::{run_metrics_server, MetricsServerConfig};
use lumio_logger::info;
use lumio_sdk::{
    crypto::ed25519::Ed25519PrivateKey,
    types::{account_config::lumio_test_root_address, chain_id::ChainId},
};
use clap::Parser;
    ...
    pub fn build_for_cli(
        ...
        Self {
            ...
            funder_config: FunderConfig::MintFunder(MintFunderConfig {
                ...
                mint_account_address: Some(lumio_test_root_address()),
                do_not_delegate,
            }),
            handler_config: HandlerConfig {
...
mod test {
    ...
    use crate::{
        endpoints::{
            LumioTapError, LumioTapErrorCode, FundRequest, FundResponse, RejectionReasonCode,
        },
    ...
    async fn start_server(config_content: &'static str) -> Result<(u16, JoinHandle<Result<()>>)> {
        ...
        let port = lumio_config::utils::get_available_port();
    ...
    async fn test_checkers() -> Result<()> {
        ...
        let response = reqwest::Client::new()
            ...
            .send()
            .await?;
        let lumio_error = LumioTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as LumioError");
        assert!(!lumio_error.rejection_reasons.is_empty());
        ...
        let lumio_error = LumioTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as LumioError");
        let rejection_reason_codes: HashSet<RejectionReasonCode> = lumio_error
            .rejection_reasons
            .into_iter()
            .map(|r| r.get_code())
            .collect();
    ...
    async fn test_redis_ratelimiter() -> Result<()> {
        ...
        let lumio_node_api_client = lumio_sdk::rest_client::Client::new(
            reqwest::Url::from_str("http://127.0.0.1:8080").unwrap(),
        );
        lumio_node_api_client
            .get_index_bcs()
            .await
    ...
    async fn test_transfer_health() -> Result<()> {
        ...
        let serialized_keys = lumio_sdk::bcs::to_bytes(&private_key)?;
        ...
        unwrap_reqwest_result(
            reqwest::Client::new()
                .post("https://faucet.devnet.lumiolabs.com/fund")
                .body(
                    FundRequest {
        ...
        let lumio_error = LumioTapError::parse_from_json_string(&response.text().await?)
            .expect("Failed to read response as LumioError");
        assert_eq!(
            lumio_error.error_code,
            LumioTapErrorCode::FunderAccountProblem
        );

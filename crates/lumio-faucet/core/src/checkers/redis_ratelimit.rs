...
impl RatelimitKeyProvider {
    ...
    /// JWT's `sub` field).
    pub async fn ratelimit_key_value(&self, data: &CheckerData) -> Result<String, LumioTapError> {
        match self {
...
/// worry about it as a followup: https://github.com/lumio-labs/lumio-tap/issues/15.
...
impl RedisRatelimitChecker {
    ...
    pub async fn get_redis_connection(&self) -> Result<Connection, LumioTapError> {
        self.db_pool.get().await.map_err(|e| {
            LumioTapError::new_with_error_code(
...
impl CheckerTrait for RedisRatelimitChecker {
    async fn check(
        ...
        data: CheckerData,
        dry_run: bool,
    ) -> Result<Vec<RejectionReason>, LumioTapError> {
        let mut conn = self
            .get_redis_connection()
            .await
            .map_err(|e| LumioTapError::new_with_error_code(e, LumioTapErrorCode::StorageError))?;
    ...
    async fn complete(&self, data: CompleteData) -> Result<(), LumioTapError> {

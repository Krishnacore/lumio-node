...
use crate::{
    endpoints::{LumioTapError, LumioTapErrorCode},
...
const DEFAULT_KEY_FILE_PATH: &str = "/opt/lumio/etc/mint.key";
...
pub struct ApiConnectionConfig {
    ...
    /// To keep Testnet simple, we used one private key for lumio root account
...
impl ApiConnectionConfig {
    ...
    pub fn get_key(&self) -> Result<Ed25519PrivateKey> {
        ...
        let result = lumio_sdk::bcs::from_bytes(&key_bytes); //.with_context(|| "bad bcs");
        if let Ok(x) = result {
            return Ok(x);
        }
...
pub async fn update_sequence_numbers(
    ...
    amount: u64,
    wait_for_outstanding_txns_secs: u64,
) -> Result<(u64, Option<u64>), LumioTapError> {
...
pub async fn submit_transaction(
    ...
    let (result, event_on_success) = if wait_for_transactions {
        ...
        // anyhow error. https://github.com/lumio-labs/lumio-tap/issues/60.
        (
            client
                ...
                .map(|_| ())
                .map_err(|e| {
                    LumioTapError::new_with_error_code(e, LumioTapErrorCode::TransactionFailed)
                }),
            "transaction_success",
        )
    } else {
...
pub struct GasUnitPriceManager {
    api_client: lumio_sdk::rest_client::Client,
...
impl GasUnitPriceManager {
    pub fn new(node_url: Url, cache_ttl: Duration) -> Self {
        Self {
            api_client: lumio_sdk::rest_client::Client::new(node_url),

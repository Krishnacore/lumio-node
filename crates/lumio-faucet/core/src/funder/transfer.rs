...
use crate::{
    endpoints::{LumioTapError, LumioTapErrorCode, RejectionReason, RejectionReasonCode},
...
impl TransferFunder {
    ...
    ) -> Result<SignedTransaction, LumioTapError> {
    ...
    async fn is_healthy_as_result(&self) -> Result<(), LumioTapError> {
    ...
    async fn fund(
        ...
        let transactions = if check_only {
            ...
            let txn = self
                .execute_transaction(
                    &client,
                    lumio_stdlib::lumio_account_transfer(receiver_address, amount),
                    &receiver_address,
                )
                .await?;
            info!(

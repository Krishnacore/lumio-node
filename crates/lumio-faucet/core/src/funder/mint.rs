...
static MINTER_SCRIPT: &[u8] = include_bytes!(
    "../../../../../lumio-move/move-examples/scripts/minter/build/Minter/bytecode_scripts/main.mv"
);
...
impl MintFunder {
    ...
    async fn get_gas_unit_price(&self) -> Result<u64, LumioTapError> {
        match self.txn_config.gas_unit_price_override {
            ...
            None => self
                ...
                .await
                .map_err(|e| {
                    LumioTapError::new_with_error_code(e, LumioTapErrorCode::LumioApiError)
                }),
        }
    }

    async fn get_transaction_factory(&self) -> Result<TransactionFactory, LumioTapError> {
        Ok(self
            .transaction_factory
            .clone()
    ...
    pub async fn use_delegated_account(&mut self) -> Result<()> {
        ...
        // Delegate minting to the account
        {
            ...
            client
                .submit_and_wait(&faucet_account.sign_with_transaction_builder(
                    transaction_factory.payload(lumio_stdlib::lumio_coin_delegate_mint_capability(
        ...
        client
            .submit_and_wait(&delegated_account.sign_with_transaction_builder(
                transaction_factory.payload(lumio_stdlib::lumio_coin_claim_mint_capability()),
            ))
            .await
    ...
    pub fn get_api_client(&self) -> Client {
        let mut builder = Client::builder(LumioBaseUrl::Custom(self.node_url.clone()));
    ...
    ) -> Result<Vec<SignedTransaction>, LumioTapError> {

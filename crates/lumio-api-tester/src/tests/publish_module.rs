...
use lumio_api_types::{HexEncodedBytes, U64};
use lumio_cached_packages::lumio_stdlib::EntryFunctionCall;
use lumio_framework::{BuildOptions, BuiltPackage};
use lumio_logger::error;
use lumio_rest_client::Client;
use lumio_sdk::{bcs, types::LocalAccount};
use lumio_types::{
...
async fn build_module(address: AccountAddress) -> Result<BuiltPackage, TestFailure> {
    // get file to compile
    let move_dir = PathBuf::from("./lumio-move/move-examples/hello_blockchain");
...
async fn publish_module(
    ...
    // create payload
    let payload: lumio_types::transaction::TransactionPayload =

...
use lumio_sdk::{coin_client::CoinClient, types::LocalAccount};
...
async fn check_account_balance(
    ...
    // actual
    let actual = match client.view_lum_account_balance(address).await {
...
async fn check_account_balance_at_version(
    ...
    let actual = match client
        .view_lum_account_balance_at_version(address, transaction_version - 1)
        .await
    {
        Ok(response) => response.into_inner(),
        Err(e) => {

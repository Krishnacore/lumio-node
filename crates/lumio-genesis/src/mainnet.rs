...
use lumio_vm_genesis::{AccountBalance, EmployeePool, ValidatorWithCommissionRate};
...
impl MainnetGenesisInfo {
    ...
    fn generate_genesis_txn(&self) -> Transaction {
        lumio_vm_genesis::encode_lumio_mainnet_genesis_transaction(
            &self.accounts,
            &self.employee_vesting_accounts,
            &self.validators,
            &self.framework,
            self.chain_id,
            &lumio_vm_genesis::GenesisConfiguration {
    ...
    pub fn generate_waypoint(&mut self) -> anyhow::Result<Waypoint> {
        ...
        let db_rw = DbReaderWriter::new(lumiodb);
        lumio_executor::db_bootstrapper::generate_waypoint::<LumioVMBlockExecutor>(&db_rw, genesis)
    }
}

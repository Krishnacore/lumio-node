...
use crate::{
    ...
    test_utils::{random_subset, KeyPair, TestLumioCrypto},
...
fn random_message_for_signing(rng: &mut OsRng) -> TestLumioCrypto {
    TestLumioCrypto(rng.sample_iter(&Alphanumeric).take(256).collect::<String>())
}

/// Returns several 256-character unique strings that can be aggregate-signed by the BLS API.
fn random_messages_for_signing(rng: &mut OsRng, n: usize) -> Vec<TestLumioCrypto> {
    (0..n)
        .map(|_| random_message_for_signing(rng))
        .collect::<Vec<TestLumioCrypto>>()
}
...
fn bls12381_aggsig_should_verify() {
    ...
    let msgs_refs = messages.iter().collect::<Vec<&TestLumioCrypto>>();
    ...
    let msgs_wrong_refs = messages_wrong.iter().collect::<Vec<&TestLumioCrypto>>();
...
/// lumio-move/framework/move-stdlib/sources/signer.move
...
fn bls12381_sample_signature_verifies() {
    ...
    assert!(sig.verify_arbitrary_msg(b"Hello Lumio!", &pk).is_ok());
}
...
fn bls12381_sample_doc_test_for_normal_sigs() {
    ...
    println!("// signature on TestLumioCrypto(\"test\".to_owned())");
    println!(
...
fn bls12381_sample_aggregate_pk_and_aggsig() {
    ...
    println!(
        "// The signed messages are \"Hello, Lumio <i>!\", where <i> \\in {{1, ..., {}}}",
        num
    );

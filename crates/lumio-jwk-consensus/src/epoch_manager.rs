...
use lumio_config::config::SafetyRulesConfig;
...
use lumio_reliable_broadcast::ReliableBroadcast;
use lumio_safety_rules::{safety_rules_manager::storage, PersistentSafetyStorage};
use lumio_types::{
...
pub struct EpochManager<P: OnChainConfigProvider> {
    ...
    jwk_updated_event_txs: Option<lumio_channel::Sender<(), ObservedJWKsUpdated>>,
    jwk_rpc_msg_tx:
        Option<lumio_channel::Sender<AccountAddress, (AccountAddress, IncomingRpcRequest)>>,
    jwk_manager_close_tx: Option<oneshot::Sender<oneshot::Sender<()>>>,

    // network utils
    self_sender: lumio_channels::Sender<Event<JWKConsensusMsg>>,
    ...
    async fn start_new_epoch(&mut self, payload: OnChainConfigPayload<P>) -> Result<()> {
        ...
        if jwk_manager_should_run && my_index.is_some() {
            ...
            let rb = ReliableBroadcast::new(
                ...
                lumio_time_service::TimeService::real(),
            ...
            let (jwk_event_tx, jwk_event_rx) = lumio_channel::new(QueueStyle::KLAST, 1, None);
            self.jwk_updated_event_txs = Some(jwk_event_tx);
            let (jwk_rpc_msg_tx, jwk_rpc_msg_rx) = lumio_channel::new(QueueStyle::FIFO, 100, None);

...
use lumio_types::{LumioCoinType, CoinType};
...
impl TransactionProcessor for CoinTransactionProcessor {
    ...
    async fn process_transactions(
        ...
        // get lumio_coin info for supply tracking
        // TODO: This only needs to be fetched once. Need to persist somehow
        let maybe_lumio_coin_info = &CoinInfoQuery::get_by_coin_type(
            LumioCoinType::type_tag().to_canonical_string(),
            &mut conn,
        )
        .unwrap();
        ...
        for txn in &transactions {
            let (
                ...
                current_coin_balances,
                mut coin_supply,
            ) = CoinActivity::from_transaction(txn, maybe_lumio_coin_info);

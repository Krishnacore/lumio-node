...
use lumio_types::{chain_id::ChainId, PeerId};
use once_cell::sync::OnceCell;
use std::sync::Arc;

/// The global [LumioNodeIdentity]
static LUMIO_NODE_IDENTITY: OnceCell<Arc<LumioNodeIdentity>> = OnceCell::new();

/// Structure that holds information related to a node's identity
pub struct LumioNodeIdentity {
...
/// Initializes the [LumioNodeIdentity] using the provided [PeerId] and
/// sets it globally exactly once.
pub fn init(peer_id: Option<PeerId>) -> Result<()> {
    let identity = LumioNodeIdentity {
    ...
    LUMIO_NODE_IDENTITY
        ...
        .map_err(|_| format_err!("LUMIO_NODE_IDENTITY was already set"))
}

/// Sets the [ChainId] in the global [LumioNodeIdentity], returning an error
/// if [init] was not called already.
pub fn set_chain_id(chain_id: ChainId) -> Result<()> {
    match LUMIO_NODE_IDENTITY.get() {
        Some(identity) => identity
            .chain_id
            .set(chain_id)
            .map_err(|_| format_err!("chain_id was already set.")),
        None => Err(format_err!("LUMIO_NODE_IDENTITY has not been set yet")),
    }
}

/// Returns the [PeerId] from the global `LUMIO_NODE_IDENTITY`
...
/// Returns the [PeerId] as [str] from the global `LUMIO_NODE_IDENTITY`
...
/// Returns the [ChainId] from the global `LUMIO_NODE_IDENTITY`

...
[Rosetta](https://en.wikipedia.org/wiki/Rosetta_(software)) works as a sidecar to an Lumio fullnode.  Rosetta then proxies the Rosetta standard
API calls to underlying Lumio REST API calls and builds the appropriate data.  


## Running Rosetta

The `lumio-rosetta` binary can run in three modes:
1. `online` -> This runs a local fullnode and blocks the Lumio REST API from outside access, using it only as a local proxy for Rosetta APIs.
...
### Balances
* Only the native `LUM` is supported.
...
All transactions are parsed from the events provided by the LumioFramework.  There are a few exceptions to this that use the transaction payload, but only for errors.
...
* Transfers support only LUM at this moment.
...
file to run the automated checks.  Additionally, the [lumio.ros](./lumio.ros)
file uses the Rosetta CLI DSL to describe the possible operations that
can be run.

Additionally, we have our `lumio-rosetta-cli` crate for local testing.

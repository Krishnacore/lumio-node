...
/// Identifier for a "block".  On Lumio, we use a transaction model, so the index
...
pub struct NetworkIdentifier {
    /// Blockchain name, should always be `lumio` and be hardcoded
...
impl From<lumio_crypto::HashValue> for TransactionIdentifier {
    fn from(hash: lumio_crypto::HashValue) -> Self {
        TransactionIdentifier {

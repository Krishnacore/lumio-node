...
use lumio_crypto::{ed25519::Ed25519PublicKey, ValidCryptoMaterialStringExt};
use lumio_logger::warn;
use lumio_rest_client::lumio_api_types::{ResourceGroup, TransactionOnChainData, U64};
use lumio_types::{
    access_path::Path,
...
pub struct Block {
    ...
    pub timestamp: u64,
    /// Transactions associated with the version.  In lumio there should only be one transaction
    pub transactions: Vec<Transaction>,
}

/// A combination of a transaction and the block associated.  In Lumio, this is just the same
...
pub struct CurrencyMetadata {
    /// Move coin type e.g. 0x1::lumio_coin::LumioCoin
    #[serde(skip_serializing_if = "Option::is_none")]
...
///
/// This is known as a write set change within Lumio
...
/// Cryptographic signature type used for signing transactions.  Lumio only uses
...
impl Transaction {
    pub async fn from_transaction(
        ...
        use lumio_types::transaction::Transaction::*;
    ...
    if let TransactionPayload::EntryFunction(inner) = payload {
        match (
            *inner.module().address(),
            ...
            | (AccountAddress::ONE, LUMIO_ACCOUNT_MODULE, TRANSFER_COINS_FUNCTION) => {
            ...
            (AccountAddress::ONE, LUMIO_ACCOUNT_MODULE, TRANSFER_FUNCTION) => {
            ...
            | (AccountAddress::ONE, LUMIO_ACCOUNT_MODULE, TRANSFER_FUNGIBLE_ASSETS_FUNCTION) => {
    ...
    /*    if let Some(owner_address) = server_context.pool_address_to_owner.get(&pool_address) {
            if let Ok(stakepool) = bcs::from_bytes::<StakePool>(data) {
                ...
                let add_stake_events = filter_events(
                    ...
                    |event_key, event| {
                        if let Ok(event) = bcs::from_bytes::<lumio_types::stake_pool::AddStakeEvent>(
                            event.event_data(),
                        ) {
                            Some(event)
                        } else {
                ...
                let set_operator_events = filter_events(
                    ...
                    |event_key, event| {
                        if let Ok(event) = bcs::from_bytes::<lumio_types::stake_pool::SetOperatorEvent>(
                            event.event_data(),
                        ) {
                            Some(event)
                        } else {
...
impl InternalOperation {
    ...
    pub fn payload(
        &self,
    ) -> ApiResult<(lumio_types::transaction::TransactionPayload, AccountAddress)> {
        Ok(match self {
            InternalOperation::CreateAccount(create_account) => (
                lumio_stdlib::lumio_account_create_account(create_account.new_account),
            ...
            InternalOperation::Transfer(transfer) => {
                ...
                // We special case LUM, because we don't want the behavior to change
                if currency == &native_coin() {
                    return Ok((
                        lumio_stdlib::lumio_account_transfer(transfer.receiver, transfer.amount.0),
                        transfer.sender,
                    ));
                }
                ...
                if let Some(ref metadata) = currency.metadata {
                    match (&metadata.move_type, &metadata.fa_address) {
                        ...
                        (Some(coin_type), Some(_)) | (Some(coin_type), None) => {
                            ...
                            (
                                lumio_stdlib::lumio_account_transfer_coins(
                                    coin_type_tag,
                                    transfer.receiver,
                                    transfer.amount.0,
                                ),
            ...
            InternalOperation::SetVoter(set_voter) => {
                ...
                (
                    lumio_stdlib::staking_contract_update_voter(
            ...
            InternalOperation::InitializeStakePool(init_stake_pool) => (
                lumio_stdlib::staking_contract_create_staking_contract(
            ...
            InternalOperation::ResetLockup(reset_lockup) => (
                lumio_stdlib::staking_contract_reset_lockup(reset_lockup.operator),
                reset_lockup.owner,
            ),
            InternalOperation::UnlockStake(unlock_stake) => (
                lumio_stdlib::staking_contract_unlock_stake(
            ...
            InternalOperation::UpdateCommission(update_commision) => (
                lumio_stdlib::staking_contract_update_commision(
            ...
            InternalOperation::DistributeStakingRewards(distribute_staking_rewards) => (
                lumio_stdlib::staking_contract_distribute(
            ...
            InternalOperation::AddDelegatedStake(add_delegated_stake) => (
                lumio_stdlib::delegation_pool_add_stake(
            ...
            InternalOperation::UnlockDelegatedStake(unlock_delegated_stake) => (
                lumio_stdlib::delegation_pool_unlock(
            ...
            InternalOperation::WithdrawUndelegated(withdraw_undelegated) => (
                lumio_stdlib::delegation_pool_withdraw(

...
use lumio_rest_client::lumio_api_types::{ResourceGroup, TransactionOnChainData};
use lumio_types::{
    account_config::{
...
const LUM_ADDRESS: AccountAddress = AccountAddress::TEN;
...
fn test_transaction(
    ...
    TransactionOnChainData {
        version,
        transaction: lumio_types::transaction::Transaction::UserTransaction(
            lumio_types::transaction::SignedTransaction::new(
...
async fn test_fa_mint() {
    ...
    let (mint_changes, mint_events) = mint_fa_output(sender, LUM_ADDRESS, store_address, 0, amount);

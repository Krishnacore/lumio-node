...
use lumio_config::config::{ApiConfig, DEFAULT_MAX_PAGE_SIZE};
use lumio_logger::prelude::*;
use lumio_node::LumioNodeArgs;
use lumio_rosetta::{bootstrap, common::native_coin, types::Currency};
use lumio_sdk::move_types::language_storage::StructTag;
...
async fn main() {
    ...
    match args {
        CommandArgs::OnlineRemote(_) => {
            println!("lumio-rosetta: Starting Rosetta in Online remote (no local full node) mode")
        },
        CommandArgs::Online(_) => {
            println!("lumio-rosetta: Starting Rosetta in Online (with local full node) mode")
        },
        CommandArgs::Offline(_) => println!("lumio-rosetta: Starting Rosetta in Offline mode"),
    }
    ...
    }) = args
    {
            ...
            match client.get_index_bcs().await {
                ...
                Err(err) => {
                    sample!(
                        ...
                        println!(
                            "lumio-rosetta: Full node REST API isn't responding yet.  You should check the node logs.  It's been waiting {} seconds.  Error: {:?}",
        ...
        println!("lumio-rosetta: Local full node started successfully");
        Some(runtime)
    } else {
    ...
    println!("lumio-rosetta: Starting rosetta");
    ...
    .expect("lumio-rosetta: Should bootstrap rosetta server");

    println!("lumio-rosetta: Rosetta started");
...
trait ServerArgs {
    ...
    fn rest_client(&self) -> Option<lumio_rest_client::Client>;
...
/// Lumio Rosetta API Server
///
/// Provides an implementation of [Rosetta](https://www.rosetta-api.org/docs/Reference.html) on Lumio.
#[derive(Debug, Parser)]
#[clap(name = "lumio-rosetta", author, version, propagate_version = true)]
...
pub struct OfflineArgs {
    ...
    /// A file of currencies to support other than LUM
    ///
...
pub struct OnlineRemoteArgs {
    ...
    /// URL for the Lumio REST API. e.g. https://fullnode.devnet.lumiolabs.com
...
impl ServerArgs for OnlineRemoteArgs {
    ...
    fn rest_client(&self) -> Option<lumio_rest_client::Client> {
        Some(lumio_rest_client::Client::new(self.rest_api_url.clone()))
    }

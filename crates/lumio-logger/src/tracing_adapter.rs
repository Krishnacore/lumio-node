...
/// A layer that translates tracing events into lumio-logger events.
pub struct TracingToLumioDataLayer;
...
impl<S> Layer<S> for TracingToLumioDataLayer
where
    S: tracing::Subscriber + for<'span> LookupSpan<'span>,
{
    ...
    fn on_event(&self, event: &Event, ctx: Context<S>) {
        let metadata = match translate_metadata(event.metadata()) {
            ...
            None => {
                dl::warn!(
                    "[tracing-to-lumio-logger] failed to translate event due to unknown level {:?}",

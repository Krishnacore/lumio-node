...
impl LogEntry {
    fn new(event: &Event, thread_name: Option<&str>, enable_backtrace: bool) -> Self {
        ...
        #[cfg(node_identity)]
        {
            peer_id = lumio_node_identity::peer_id_as_str();
            chain_id = lumio_node_identity::chain_id().map(|chain_id| chain_id.id());
        }
...
/// A builder for a `LumioData`, configures what, where, and how to write logs.
pub struct LumioDataBuilder {
    channel_size: usize,
...
impl LumioDataBuilder {
    ...
    fn build_logger(&mut self) -> Arc<LumioData> {
    ...
    pub fn build(&mut self) -> Arc<LumioData> {
...
impl LumioData {
    pub fn builder() -> LumioDataBuilder {
        LumioDataBuilder::new()
    }

    #[allow(clippy::new_ret_no_self)]
    pub fn new() -> LumioDataBuilder {
        Self::builder()
    }

    pub fn init_for_testing() {
        // Create the Lumio Data Builder
...
/// or to a `LumioData` for external logging.
struct LoggerService {
    receiver: sync::mpsc::Receiver<LoggerServiceEvent>,
    printer: Option<Box<dyn Writer>>,
    facade: Arc<LumioData>,
...
/// Example:
/// 2020-03-07 05:03:03 INFO [thread_name] common/lumio-logger/src/lib.rs:261 Hello { "world": true }
...
pub struct LoggerFilterUpdater {
    logger: Arc<LumioData>,
    logger_builder: LumioDataBuilder,
}

impl LoggerFilterUpdater {
    pub fn new(logger: Arc<LumioData>, logger_builder: LumioDataBuilder) -> Self {
        Self {
            logger,
            logger_builder,
        }
...
#[cfg(test)]
mod tests {
    use super::{text_format, LumioData, LogEntry};
    use crate::{
        lumio_logger::{json_format, TruncatedLogString, RUST_LOG_TELEMETRY},
        debug, error, info,
        logger::Logger,
        telemetry_log_writer::TelemetryLog,
        trace, warn, LumioDataBuilder, Event, Key, KeyValue, Level, LoggerFilterUpdater, Metadata,
    ...
    fn basic() {
        ...
        let expected = format!("{{\"level\":\"INFO\",\"source\":{{\"package\":\"lumio_logger\",\"file\":\"crates/lumio-logger/src/lumio_logger.rs:{line_num}\"}},\"thread_name\":\"{thread_name}\",\"hostname\":\"test-host\",\"timestamp\":\"2022-07-24T23:42:29.540278Z\",\"message\":\"This is a log\",\"data\":{{\"bar\":\"foo_bar\",\"category\":\"name\",\"display\":\"12345\",\"foo\":5,\"test\":true}}}}");
    ...
    fn new_async_logger() -> (LumioDataBuilder, Arc<LumioData>) {
        let mut logger_builder = LumioDataBuilder::new();
    ...
    fn new_text_logger() -> (
        ...
        let logger = LumioDataBuilder::new()
            .level(Level::Debug)

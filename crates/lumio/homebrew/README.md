...
The [Lumio command line interface (CLI)](https://lumio.dev/tools/lumio-cli/install-cli/) may be installed
via [Homebrew](https://brew.sh/) for simplicity. This is an in-depth overview of Homebrew and the Lumio formula. In this
guide, we go over each section of the Homebrew formula and steps to implement changes in the future.

## Quick guide

- [Formula in Homebrew GitHub](https://github.com/Homebrew/homebrew-core/blob/master/Formula/lumio.rb)
- [Lumio 1.0.3 New Formula PR for GitHub](https://github.com/Homebrew/homebrew-core/pull/119832)
- [Lumio Formula Fix PR to use build_cli_release.sh](https://github.com/Homebrew/homebrew-core/pull/120051)
...
# lumio 1.0.5
# Lumio Labs <opensource@lumiolabs.com>
# Command Line Interface (CLI) for developing and interacting with the Lumio blockchain
# ...
```

## Change guide

Note: This guide is for developers who are trying to update the Lumio homebrew formula.

You can get the latest formula here: https://github.com/Homebrew/homebrew-core/blob/master/Formula/a/lumio.rb

Copy the `lumio.rb` file to your `homebrew` `formula` directory. For example, on macOS with an M1, this will likely be:
...
After you've copied `lumio.rb` to your local `homebrew` `formula` directory, you can modify it and use the commands
below for testing.
...
brew audit --new-formula lumio      # For new formula
brew audit lumio --strict --online
brew install lumio
brew test lumio

# For debugging issues during the installation process you can do
brew install lumio --interactive    # Interactive, gives you access to the shell
brew install lumio -d               # Debug mode

# Livecheck
brew livecheck --debug lumio
```
...
1. Commit your changes to `lumio-core` in `crates/lumio/homebrew`.
...
class Lumio < Formula
  ...
  homepage "https://lumiolabs.com/"
  url "https://github.com/lumio-labs/lumio-core/archive/refs/tags/lumio-cli-v1.0.3.tar.gz"
...
software by checking upstream. The strategy used below checks for all `lumio-cli-v<SEMVER>` tags for `lumio-core`. The
...
...
  ...
  # with matching regex of href="<URL>/tag/lumio-cli-v<SEMVER>". This
  # is done to automatically check for new release versions of the CLI.
  livecheck do
    url :stable
    regex(/^lumio-cli[._-]v?(\d+(?:\.\d+)+)$/i)
  end
...
```
...
```ruby
  # Installs listed homebrew dependencies before Lumio installation
  # Dependencies needed: https://lumio.dev/cli-tools/build-lumio-cli
  # See scripts/dev_setup.sh in lumio-core for more info
  ...
  # root Cargo.toml file of lumio-core (currently it is pegged to Rust 
...
The current test generates a new key via the Lumio CLI and ensures the shell output matches the filename(s) for that
key.

```ruby
  ...
  test do
    assert_match(/output.pub/i, shell_output("#{bin}/lumio key generate --output-file output"))
  end
  ...
```

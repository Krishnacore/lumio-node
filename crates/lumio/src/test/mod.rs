...
use lumio_config::config::Peer;
use lumio_crypto::{
    bls12381,
    ed25519::{Ed25519PrivateKey, Ed25519PublicKey},
    x25519, PrivateKey,
};
use lumio_framework::chunked_publish::CHUNK_SIZE_IN_BYTES;
use lumio_genesis::config::HostAndPort;
use lumio_keygen::KeyGen;
use lumio_logger::warn;
use lumio_rest_client::{
    lumio_api_types::{MoveStructTag, MoveType},
    Transaction,
};
use lumio_sdk::move_types::{account_address::AccountAddress, language_storage::ModuleId};
use lumio_temppath::TempPath;
use lumio_types::on_chain_config::ValidatorSet;
...
impl CliTestFramework {
            ...
            entry_function_args: EntryFunctionArguments {
                ...
                type_arg_vec: TypeArgVec {
                    type_args: vec![MoveType::Struct(MoveStructTag::new(
                        ...
                        ident_str!("lumio_coin").into(),
                        ident_str!("LumioCoin").into(),
                        vec![],
                    ))],
                },
                json_file: None,
    ...
    async fn last_n_transactions_details(&self, count: u16) -> String {
        let result = lumio_rest_client::Client::new(self.endpoint.clone())
    ...
    pub fn add_move_files(&self) {
        ...
        let hello_blockchain_test_contents = include_str!("../../../../lumio-move/move-examples/hello_blockchain/sources/hello_blockchain_test.move");
    ...
    /// Runs the given script contents using the local lumio_framework directory.
    pub async fn run_script(
        ...
        self.run_script_with_framework_package(index, script_contents, FrameworkPackageArgs {
            framework_git_rev: None,
            framework_local_dir: Some(Self::lumio_framework_dir()),
            skip_fetch_latest_git_deps: false,
        })
        .await
    }
    ...
    /// Runs the given script contents using the lumio_framework from lumio-core git repository.
    ...
    pub fn lumio_framework_dir() -> PathBuf {

...
use lumio_vm_genesis::{default_gas_schedule, AccountBalance, EmployeePool};
...
/// Tool for setting up an Lumio chain Genesis transaction
///
...
pub fn fetch_mainnet_genesis_info(git_options: GitOptions) -> CliTypedResult<MainnetGenesisInfo> {
    ...
    // Check that the user has a reasonable amount of LUM, since below the minimum gas amount is
    // not useful 1 LUM minimally

...
use crate::{
    common::{
        types::{
            ...
            ProfileSummary, LUMIO_FOLDER_GIT_IGNORE, CONFIG_FOLDER, GIT_IGNORE,
        },
...
use lumio_cli_common::generate_cli_completions;
...
/// Tool for interacting with configuration of the Lumio CLI tool
///
...
impl CliCommand<()> for GenerateShellCompletions {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        generate_cli_completions::<Tool>("lumio", self.shell, self.output_file.as_path())
...
pub struct SetGlobalConfig {
    ...
    ///
    /// `Workspace` will put the `.lumio/` folder in the current directory, where
    /// `Global` will put the `.lumio/` folder in your home directory
    #[clap(long)]
...
pub enum ConfigType {
    /// Per system user configuration put in `<HOME>/.lumio`
    Global,
    /// Per directory configuration put in `<CURRENT_DIR>/.lumio`
    Workspace,
}

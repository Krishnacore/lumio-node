...
use anyhow::Context;
use lumio_types::vm::module_metadata::prelude::*;
...
/// 1. Download the package with `lumio move download --account 0x42 --package PackName --bytecode`
/// 2. Disassemble the package bytecode with `lumio move disassemble --package-path PackName/bytecode_modules`
#[derive(Debug, Parser)]
...
/// 1. Download the package with `lumio move download --account 0x42 --package PackName --bytecode`
/// 2. Decompile the package bytecode with `lumio decompile --package-path PackName/bytecode_modules`
#[derive(Debug, Parser)]
...
struct BytecodeMetadata {
    lumio_metadata: Option<RuntimeModuleMetadataV1>,
...
impl BytecodeCommand {
    ...
    fn print_metadata(&self, bytecode_path: &Path) -> Result<String, CliError> {
        ...
        let metadata = if self.is_script {
            ...
            BytecodeMetadata {
                lumio_metadata: get_metadata_from_compiled_code(&script),
            ...
            BytecodeMetadata {
                lumio_metadata: get_metadata_from_compiled_code(&module),

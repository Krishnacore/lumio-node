...
use lumio_gas_schedule::{MiscGasParameters, NativeGasParameters};
use lumio_move_debugger::lumio_debugger::LumioDebugger;
use lumio_rest_client::{
    lumio_api_types::{EntryFunctionId, HexEncodedBytes, IdentifierWrapper, MoveModuleId},
    error::RestError,
    LumioBaseUrl, Client,
};
use lumio_types::{
    ...
    on_chain_config::lumio_test_feature_flags_genesis,
    transaction::{
        ReplayProtector, Transaction, TransactionArgument, TransactionPayload, TransactionStatus,
    },
};
use lumio_vm::data_cache::AsMoveResolver;
...
const HELLO_BLOCKCHAIN_EXAMPLE: &str = include_str!(
    "../../../../lumio-move/move-examples/hello_blockchain/sources/hello_blockchain.move"
);
...
pub struct FrameworkPackageArgs {
    /// Git revision or branch for the Lumio framework
    ///
    ...
    /// Local framework directory for the Lumio framework
    ///
...
impl FrameworkPackageArgs {
    pub fn init_move_dir(
        ...
        if let Some(ref path) = self.framework_local_dir {
            dependencies.insert(LUMIO_FRAMEWORK.to_string(), Dependency {
            ...
            dependencies.insert(LUMIO_FRAMEWORK.to_string(), Dependency {
                local: None,
                git: Some(LUMIO_GIT_PATH.to_string()),
...
/// This can then be run with `lumio move run-script`
#[derive(Parser)]
...
pub struct TestPackage {
    ...
    /// Collect coverage information for later use with the various `lumio move coverage` subcommands
    #[clap(long = "coverage")]
...
impl CliCommand<&'static str> for TestPackage {
    ...
    async fn execute(self) -> CliTypedResult<&'static str> {
        ...
        let result = move_cli::base::test::run_move_unit_tests(
            ...
            lumio_debug_natives::lumio_debug_natives(
                NativeGasParameters::zeros(),
                MiscGasParameters::zeros(),
            ),
            lumio_test_feature_flags_genesis(),
            None,
            None,
        ...
        if self.compute_coverage {
            ...
            println!("Please use `lumio move coverage -h` for more detailed source or bytecode test coverage of this package");
        }
...
/// Publishes the modules in a Move package to the Lumio blockchain
#[derive(Parser)]
...
fn create_package_publication_data(
    ...
    let payload = match publish_type {
        PublishType::AccountDeploy => {
            lumio_cached_packages::lumio_stdlib::code_publish_package_txn(
                metadata_serialized.clone(),
                compiled_units.clone(),
            )
        },
        PublishType::ObjectDeploy => {
            lumio_cached_packages::lumio_stdlib::object_code_deployment_publish(
                metadata_serialized.clone(),
                compiled_units.clone(),
            )
        },
        PublishType::ObjectUpgrade => {
            lumio_cached_packages::lumio_stdlib::object_code_deployment_upgrade(
                metadata_serialized.clone(),
                compiled_units.clone(),
...
/// Publishes the modules in a Move package to the Lumio blockchain, under an object (legacy version of `deploy-object`)
#[derive(Parser)]
...
/// Publishes the modules in a Move package to the Lumio blockchain, under an object.
#[derive(Parser)]
...
async fn submit_chunked_publish_transactions(
    ...
    if !is_staging_area_empty(txn_options, large_packages_module_address).await? {
        ...
        \nUse the `lumio move clear-staging-area` command to clean up the `StagingArea` resource under the account.",
    ...
    for (idx, payload) in payloads.into_iter().enumerate() {
        ...
        match result {
            ...
            Err(e) => {
                ...
                \nTo avoid this, use the `lumio move clear-staging-area` command to clean up the `StagingArea` resource under your account before retrying.".bold());
                return Err(e);
            },
        }
    }
...
async fn is_staging_area_empty(
    ...
    match staging_area_response {
        ...
        Err(RestError::Api(lumio_error_response))
            if lumio_error_response.error.error_code == LumioErrorCode::ResourceNotFound =>
        {
...
/// Publishes the modules in a Move package to the Lumio blockchain under a resource account
#[derive(Parser)]
...
impl CliCommand<TransactionSummary> for CreateResourceAccountAndPublishPackage {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        .map(|p| p.account)
        {
            ...
            return Err(CliError::CommandArgumentError(
                "Please provide an account using --profile or run lumio init".to_string(),
            ));
        };
        ...
        let payload = lumio_cached_packages::lumio_stdlib::resource_account_create_resource_account_and_publish_package(
            seed,
...
impl ReplayNetworkSelection {
    pub fn to_base_url(&self) -> CliTypedResult<LumioBaseUrl> {
        match self {
            ReplayNetworkSelection::Mainnet => Ok(LumioBaseUrl::Mainnet),
            ReplayNetworkSelection::Testnet => Ok(LumioBaseUrl::Testnet),
            ReplayNetworkSelection::Devnet => Ok(LumioBaseUrl::Devnet),
            ReplayNetworkSelection::RestEndpoint(url) => {
                Ok(LumioBaseUrl::Custom(Url::parse(url).map_err(|e| {
...
impl CliCommand<TransactionSummary> for Replay {
    ...
    async fn execute(self) -> CliTypedResult<TransactionSummary> {
        ...
        let debugger = LumioDebugger::rest_client(client)?;
...
/// Duplicated from lumio_types, as we also need to load_account_arg from the CLI.
#[derive(Debug, Clone)]
pub struct MemberId {

...
/// 1 LUM (might not actually get that much, depending on the faucet)
const NUM_DEFAULT_OCTAS: u64 = 100000000;

/// Tool to initialize current directory for the lumio tool
///
/// Configuration will be pushed into .lumio/config.yaml
#[derive(Debug, Parser)]
pub struct InitTool {
    ...
    ///
    /// Make sure that you have your Ledger device connected and unlocked, with the Lumio app installed and opened.
...
impl CliCommand<()> for InitTool {
    ...
    async fn execute(self) -> CliTypedResult<()> {
        ...
        let mut profile_config = if let Some(profile_config) = config.remove_profile(profile_name) {
            prompt_yes_with_override(&format!("Lumio already initialized for profile {}, do you want to overwrite the existing config?", profile_name), self.prompt_options)?;
            profile_config
        } else {
        ...
        match network {
            ...
            Network::Testnet => {
                profile_config.rest_url =
                    Some("https://fullnode.testnet.lumiolabs.com".to_string());
            ...
            Network::Devnet => {
                ...
                profile_config.faucet_url = Some("https://faucet.devnet.lumiolabs.com".to_string());
            },
            Network::Local => {
            ...
            self.hardware_wallet_options.extract_derivation_path()?
        {
            ...
            let account_map = lumio_ledger::fetch_batch_accounts(Some(0..5))?;
            eprintln!(
            ...
            let path = lumio_ledger::DERIVATION_PATH.replace("{index}", input_index);

            // Validate the path
            if !lumio_ledger::validate_derivation_path(&path) {
        ...
        let public_key = if self.is_hardware_wallet() {
            let pub_key = match lumio_ledger::get_public_key(
                derivation_path
                    .ok_or_else(|| {
        ...
        let client = lumio_rest_client::Client::new(rest_url);
        ...
        let funded = matches!(client
            .get_account_balance(address, "0x1::LumioCoin::LumioCoin")
        ...
        eprintln!(
            "\n---\nLumio CLI is now set up for account {} as profile {}!\n---\n",
        ...
        if !funded {
            match network {
                ...
                Network::Testnet => {
                    ...
                    eprintln!("The account has not been funded on chain yet. To fund the account and get LUM on testnet you must visit {}", mint_site_url);

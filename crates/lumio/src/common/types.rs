...
use lumio_framework::chunked_publish::{
    default_large_packages_module_address, CHUNK_SIZE_IN_BYTES,
};
use lumio_global_constants::adjust_gas_headroom;
use lumio_keygen::KeyGen;
use lumio_logger::Level;
use lumio_move_debugger::lumio_debugger::LumioDebugger;
use lumio_rest_client::{
    lumio_api_types::{EntryFunctionId, HashValue, MoveType, ViewRequest},
    error::RestError,
    LumioBaseUrl, Client, Transaction,
};
use lumio_sdk::{
    transaction_builder::TransactionFactory,
    types::{HardwareWalletAccount, HardwareWalletType, LocalAccount, TransactionSigner},
};
use lumio_transaction_simulation::SimulationStateStore;
use lumio_transaction_simulation_session::Session;
use lumio_types::{
...
pub const USER_AGENT: &str = concat!("lumio-cli/", env!("CARGO_PKG_VERSION"));
...
pub const LUMIO_FOLDER_GIT_IGNORE: &str = indoc! {"
    *
    testnet/
    config.yaml
"};
...
const X_LUMIO_CLIENT_VALUE: &str = concat!("lumio-cli/", env!("CARGO_PKG_VERSION"));
...
pub enum CliError {
    ...
    #[error("Unable to find config {0}, have you run `lumio init`?")]
...
impl From<lumio_config::config::Error> for CliError {
    fn from(e: lumio_config::config::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}

impl From<lumio_github_client::Error> for CliError {
    fn from(e: lumio_github_client::Error) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
impl From<lumio_crypto::CryptoMaterialError> for CliError {
    fn from(e: lumio_crypto::CryptoMaterialError) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
impl From<lumio_ledger::LumioLedgerError> for CliError {
    fn from(e: lumio_ledger::LumioLedgerError) -> Self {
        CliError::UnexpectedError(e.to_string())
    }
}
...
/// Config saved to `.lumio/config.yaml`
...
pub const CONFIG_FOLDER: &str = ".lumio";

/// An individual profile
#[derive(Debug, Default, Serialize, Deserialize)]
pub struct ProfileConfig {
    /// Name of network being used, if setup from lumio init
    #[serde(skip_serializing_if = "Option::is_none")]
    ...
    /// URL for the Lumio rest endpoint
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl CliConfig {
    ...
    pub fn config_exists(mode: ConfigSearchMode) -> bool {
        if let Ok(folder) = Self::lumio_folder(mode) {
    ...
    pub fn load(mode: ConfigSearchMode) -> CliTypedResult<Self> {
        let folder = Self::lumio_folder(mode)?;
    ...
    /// Saves the config to ./.lumio/config.yaml
    pub fn save(&self) -> CliTypedResult<()> {
        let lumio_folder = Self::lumio_folder(ConfigSearchMode::CurrentDir)?;

        // Create if it doesn't exist
        let no_dir = !lumio_folder.exists();
        create_dir_if_not_exist(lumio_folder.as_path())?;

        // If the `.lumio/` doesn't exist, we'll add a .gitignore in it to ignore the config file
        // so people don't save their credentials...
        if no_dir {
            write_to_user_only_file(
                lumio_folder.join(GIT_IGNORE).as_path(),
                GIT_IGNORE,
                LUMIO_FOLDER_GIT_IGNORE.as_bytes(),
            )?;
        }

        // Save over previous config file
        let config_file = lumio_folder.join(CONFIG_FILE);
        ...
        let legacy_config_file = lumio_folder.join(LEGACY_CONFIG_FILE);
    ...
    /// Finds the current directory's .lumio folder
    fn lumio_folder(mode: ConfigSearchMode) -> CliTypedResult<PathBuf> {
...
impl RestOptions {
    ...
    pub fn url(&self, profile: &ProfileOptions) -> CliTypedResult<reqwest::Url> {
        ...
        .map(|p| p.rest_url)
        {
            ...
            Err(CliError::CommandArgumentError("No rest url given.  Please add --url or add a rest_url to the .lumio/config.yaml for the current profile".to_string()))
        }
    }

    pub fn client(&self, profile: &ProfileOptions) -> CliTypedResult<Client> {
        let mut client = Client::builder(LumioBaseUrl::Custom(self.url(profile)?))
            .timeout(Duration::from_secs(self.connection_timeout_secs))
            .header(lumio_api_types::X_LUMIO_CLIENT, X_LUMIO_CLIENT_VALUE)?;
...
pub fn load_account_arg(str: &str) -> Result<AccountAddress, CliError> {
            ...
            .map(|p| p.private_key)
    {
        ...
        Err(CliError::CommandArgumentError(
            "'--account' or '--profile' after using lumio init must be provided".to_string(),
        ))
    }
}
...
pub struct FaucetOptions {
    /// URL for the faucet endpoint e.g. `https://faucet.devnet.lumiolabs.com`
    #[clap(long)]
...
impl TransactionOptions {
        ...
        ) -> CliTypedResult<(VMStatus, VMOutput)>,
    {
        ...
        let debugger = LumioDebugger::rest_client(client).unwrap();

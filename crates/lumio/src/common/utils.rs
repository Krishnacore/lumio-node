...
use lumio_logger::{debug, Level};
use lumio_rest_client::{lumio_api_types::HashValue, Account, Client, FaucetClient, State};
use lumio_telemetry::service::telemetry_is_disabled;
use lumio_types::{
...
async fn send_telemetry_event(command: &str, latency: Duration, error: Option<&str>) {
    ...
    // Send the event
    lumio_telemetry::cli_metrics::send_cli_telemetry_event(
        build_information,
        command.into(),
        latency,
        error.is_none(),
...
pub fn start_logger(level: Level) {
    let mut logger = lumio_logger::Logger::new();
...
pub fn explorer_account_link(hash: AccountAddress, network: Option<Network>) -> String {
    ...
    if let Some(network) = network {
        format!(
            "https://explorer.lumiolabs.com/account/{}?network={}",
            hash, network
        )
    } else {
        format!("https://explorer.lumiolabs.com/account/{}", hash)
    }
}
...
pub fn explorer_transaction_link(
    ...
    if let Some(network) = network {
        format!(
            "https://explorer.lumiolabs.com/txn/{}?network={}",
            hash.to_hex_literal(),
            network
        )
    } else {
        format!(
            "https://explorer.lumiolabs.com/txn/{}",
            hash.to_hex_literal()
        )
    }
}
...
///
/// [Read about AIP-80](https://github.com/lumio-foundation/AIPs/blob/main/aips/aip-80.md)

...
use lumio_types::{account_address::AccountAddress, LumioCoinType, CoinType};
...
pub struct Balance {
    ...
    /// Coin type to lookup.  Defaults to 0x1::lumio_coin::LumioCoin
    #[clap(long)]
    ...
    async fn execute(self) -> CliTypedResult<Vec<AccountBalance>> {
        ...
        .map(|p| p.account)
        {
            ...
            return Err(CliError::CommandArgumentError(
                "Please provide an account using --account or run lumio init".to_string(),
            ));
        };
...
impl Balance {
    async fn coin_balance(self, account: AccountAddress) -> CliTypedResult<Vec<AccountBalance>> {
        let coin_type = if let Some(coin) = self.coin_type {
            ...
            // If nothing is given, use the default LUM

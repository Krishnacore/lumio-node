...
use lumio_rest_client::{
    lumio_api_types::{HashValue, WriteResource, WriteSetChange},
    Transaction,
};
...
// TODO: Add ability to transfer non-LUM coins
// TODO: Add ability to not create account by default
/// Transfer LUM between accounts
///
#[derive(Debug, Parser)]
pub struct TransferCoins {
    /// Address of account to send LUM to
    #[clap(long, value_parser = crate::common::types::load_account_arg)]
    pub(crate) account: AccountAddress,

    /// Amount of Octas (10^-8 LUM) to transfer
    #[clap(long)]
...
impl CliCommand<TransferSummary> for TransferCoins {
    ...
    async fn execute(self) -> CliTypedResult<TransferSummary> {
        self.txn_options
            .submit_transaction(lumio_stdlib::lumio_account_transfer(
                self.account,
                self.amount,
            ))
            .await
...
const SUPPORTED_COINS: [&str; 1] = ["0x1::coin::CoinStore<0x1::lumio_coin::LumioCoin>"];

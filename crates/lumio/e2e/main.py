...
Example (testing locally built CLI binary):
  python3 main.py --base-network devnet --test-cli-path ~/lumio-core/target/release/lumio

This means, run the CLI test suite using a CLI built from mainnet_0431e2251d0b42920d89a52c63439f7b9eda6ac3 against a localnet built from the testnet branch of lumio-core.
...
from cases.init import test_lumio_header_included, test_init, test_metrics_accessible
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="What branch the Lumio CLI used for the localnet should be built from",
    )
    parser.add_argument(
    ...
    test_cli_args.add_argument(
        "--test-cli-path",
        help="Path to CLI binary we want to test, e.g. /home/dport/lumio-core/target/release/lumio",
    )
    parser.add_argument(
        "--working-directory",
        default="/tmp/lumio-cli-tests",
...
async def run_tests(run_helper):
    ...
    test_lumio_header_included(run_helper)

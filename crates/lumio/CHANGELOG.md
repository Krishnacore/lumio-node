...
All notable changes to the Lumio CLI will be captured in this file. This project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html) and the format set out by [Keep a Changelog](https://keepachangelog.com/en/1.0.0/).
...
- Add flag `--file-path <FILE_PATH>...` to `lumio move fmt`, which allows to specify individual files to format.
- Fixed bug where `lumio move fmt` would format current directory if invalid `--package-path` was provided.

## [7.3.0]

- Update boogie from 3.2.4 to 3.5.1.
- Change behavior of `lumio init` to first look and see if the account has an LUM balance rather than checking if the account exists

## [7.2.0]

- Add ability to retrieve fungible asset balances
- Add `lumio key extract-public-key` which generates a public key or a proof of possession for the given key.

## [7.1.0]
- Add CLI outputs and on-disk storage to be stored in AIP-80 format.  Will allow for legacy formats to be taken in as well

## [7.0.0]
- Compiler v1 is now deprecated. It is now removed from the Lumio CLI.
- Added a new option `lumio move compile --fail-on-warning` which fails the compilation if any warnings are found.
...
- The new subcommand `lumio update move-mutation-test` will install/update the external binary `move-mutation-test`, which performs mutation testing on a Move project to find blind spots in Move unit tests.
...
## [6.1.1]
- Added a new feature to `lumio workspace run`: The workspace server now listens for a "stop" command from
stdin, which triggers a graceful shutdown when received.

## [6.1.0]
- Remove FFI support from Lumio CLI.
- Various compiler bug fixes.
...
## [6.0.2] - 2025/01/24
- Fix `lumio workspace run` so it does not panic when writing to closed stdout/stderr, allowing it to finish its graceful shutdown sequence when used as a child process.
...
- Add flag `--benchmark` to `lumio move prove`, which allows to benchmark verification times of individual functions in a package.
- Add flag `--only <name>` to `lumio move prove`, which allows to scope verification to a function.
- Fix `lumio init` to show the explorer link for accounts when account is already created on chain instead of prompting to fund the account.
- Set Compiler v2 as the default compiler and Move 2 as the default language version.
- Add new `--move-1` flag to use Compiler v1 and Move 1.
...
## [5.0.0] - 2024/12/11
- [**Breaking Change**] `lumio init` and `lumio account fund-with-faucet` no longer work directly with testnet, you must now use the minting page at the [Lumio dev docs](https://lumio.dev/network/faucet).
## [4.7.0] - 2024/12/10
- [`Fix`] CLI config should not always require a private key field to be present.

## [4.6.0] - 2024/11/29
- Add `--node-api-key` flag to `lumio move replay` to allow for querying the fullnode with an API key.
...
- Add option `--print-metadata-only` to `lumio move decompile` and `lumio move disassemble` to print out the metadata attached to the bytecode.
- Add `--existing-hasura-url` flag to localnet to tell it to use an existing Hasura instance instead of run Hasura itself. See https://github.com/lumio-labs/lumio-core/pull/15313.
...
## [4.4.0] - 2024/11/06
- Fix typos in `lumio move compile` help text.
- Update the default version of `movefmt` to be installed from 1.0.5 to 1.0.6
- Add `--host-postgres-host` flag: https://github.com/lumio-labs/lumio-core/pull/15216.
...
## [4.2.4] - 2024/10/21
- Releasing Move 2.1, which adds compound assignments (`x += 1`) and loop labels to the language. See [Move 2 Release Notes](https://lumio.dev/en/build/smart-contracts/book/move-2).
- multiple bug fixes in the Move 2 compilation chain.
- `lumio move fmt` formats move files inside the `tests` and `examples` directory of a package.
- Added `lumio update prover-dependencies`, which installs the dependency of Move prover, boogie, z3 and cvc5.
...
- Set compiler V2 as default when using `lumio move prove`.
...
- Adds `lumio move lint` to produce lint warnings for the current package. Only a few lint rules are implemented for now,
  but more are coming.
- Adds `lumio move fmt`, which runs the Move formatter, `movefmt`, on the current package. Also adds
  `lumio update movefmt`. This installs / updates the `movefmt` binary.
...
## [3.5.0] - 2024/07/06
- Add balance command to easily get account balances for LUM currently
...
- [Early beta release of the Move compiler v2](https://lumio.dev/move/compiler_v2/) is now accessible through the CLI. We now allow specifying the Move compiler version and the Move language version via the CLI.
...
## [3.0.1] - 2024/03/05
- Fix bug in `lumio update revela` if default install directory doesn't exist.

## [3.0.0] - 2024/03/05
- **Breaking Change**: `lumio update` is now `lumio update lumio`.
- Added `lumio update revela`. This installs / updates the `revela` binary, which is needed for the new `lumio move decompile` subcommand.
- Extended `lumio move download` with an option `--bytecode` to also download the bytecode of a module
- Integrated the Revela decompiler which is now available via `lumio move decompile`
- Extended `lumio move disassemble` and the new `lumio move decompile` to also work on entire packages instead of only single files
...
- Added `for` loop.
  ...
  - Documentation: https://lumio.dev/move/book/loops
...
## [2.3.1] - 2023/11/07
### Updated
- Updated processor code from https://github.com/lumio-labs/lumio-indexer-processors for the localnet to 2d5cb211a89a8705674e9e1e741c841dd899c558.
...
- Updated processor code from https://github.com/lumio-labs/lumio-indexer-processors for the localnet to bcba94c26c8a6372056d2b69ce411c5719f98965.
...
## [2.2.2] - 2023/10/16
### Updated
- Updated processor code from https://github.com/lumio-labs/lumio-indexer-processors for the localnet to d6f55d4baba32960ea7be60878552e73ffbe8b7e.

## [2.2.1] - 2023/10/13
### Fixed
- Fixed postgres data persistence between restarts when using `lumio node run-local-testnet --with-indexer-api`.

## [2.2.0] - 2023/10/11
### Added
- Added `--with-indexer-api` to `lumio node run-local-testnet`. With this flag you can run a full processor + indexer API stack as part of your localnet. You must have Docker installed to use this feature. For more information, see https://lumio.dev/nodes/local-testnet/local-testnet-index.
### Updated
- Updated CLI source compilation to use rust toolchain version 1.72.1 (from 1.71.1).

## [2.1.1] - 2023/09/27
### Added
- Added an option `--print-metadata` to the command `lumio move download` to print out the metadata of the package to be downloaded.
  - Example: `lumio move download  --account 0x1 --package LumioFramework --url https://mainnet.lumiolabs.com/v1 --print-metadata`
### Updated
- The `--with-faucet` flag has been removed from `lumio node run-local-testnet`, we now run a faucet by default. To disable the faucet use the `--no-faucet` flag.
- **Breaking change**: When using `lumio node run-local-testnet` we now expose a transaction stream. Learn more about the transaction stream service here: https://lumio.dev/indexer/txn-stream/. Opt out of this with `--no-txn-stream`. This is marked as a breaking change since the CLI now uses a port (50051 by default) that it didn't used to. If you need this port, you can tell the CLI to use a different port with `--txn-stream-port`.
...
- Added basic ledger support for CLI
  - Example: `lumio init --ledger` to create a new profile from ledger. After this, you can use it the same way as other profiles.
  - Note: `Ledger Nano s Plus` or `Ledger Nano X` is highly recommended.

## [2.0.3] - 2023/08/04
### Fixed
- Fixed the following input arguments issue when running `lumio move view`
...
- init command from config subcommand is removed. Please use init from the root command.
  - Example: `lumio config init` -> `lumio init`
### Fixed
- Panic issue when running `lumio move test` is fixed - GitHub issue #8516
...
- Added custom header *x-lumio-client* for analytic purpose
...
## [1.0.13] - 2023/04/27
### Fixed
* Previously `--skip-fetch-latest-git-deps` would not actually do anything when used with `lumio move test`. This has been fixed.
* Fixed the issue of the hello_blockchain example where feature enable was missing

## [1.0.12] - 2023/04/25
### Added
* Support for creating and interacting with multisig accounts v2. More details can be found at [AIP 12](https://github.com/lumio-foundation/AIPs/blob/main/aips/aip-12.md).
* Added `disassemble` option to the CLI - This can be invoked using `lumio move disassemble` to disassemble the bytecode and save it to a file
* Fixed handling of `vector<string>` as an entry function argument in `lumio move run`

## [1.0.11] - 2023/04/14
### Fixed
* Fixed creating a new test account with `lumio init` would fail if the account didn't already exist
...
* Update `lumio init` to fix an incorrect account address issue, when trying to init with a rotated private key. Right now it does an actual account lookup instead of deriving from public key
...
## [1.0.9] - 2023/03/29
### Added
* `lumio move show abi` allows for viewing the ABI of a compiled move package
...
## [1.0.8] - 2023/03/16
### Added
* Added an `lumio account derive-resource-account-address` command to add the ability to derive an address easily
* Added the ability for different input resource account seeds, to allow matching directly with onchain code
* Added beta support for coverage via `lumio move coverage` and `lumio move test --coverage`
...
* `vector<string>` inputs to now work for both `lumio move view` and `lumio move run`
...
## [1.0.7]
* For logs earlier than 1.0.7, please check out the [releases on GitHub](https://github.com/lumio-labs/lumio-core/releases?q="Lumio+CLI+Release")

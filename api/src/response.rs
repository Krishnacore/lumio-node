...
//! The Lumio API response / error handling philosophy.
//!
...
// TODO: https://github.com/lumio-labs/lumio-core/issues/2279

use super::{accept_type::AcceptType, bcs_payload::Bcs};
use lumio_api_types::{Address, LumioError, LumioErrorCode, HashValue, LedgerInfo};
use move_core_types::{
...
pub enum LumioResponseContent<T: ToJSON + Send + Sync> {
...
/// an error response to manipulate the LumioError inside it.
pub trait LumioErrorResponse {
    fn inner_mut(&mut self) -> &mut LumioError;
}
...
/// implementor also implements LumioErrorResponse, which saves functions from
...
macro_rules! generate_error_traits {
    ($($trait_name:ident),*) => {
        paste::paste! {
        $(
        pub trait [<$trait_name Error>]: LumioErrorResponse {
            // With ledger info and an error code
            #[allow(unused)]
            fn [<$trait_name:snake _with_code>]<Err: std::fmt::Display>(
                err: Err,
                error_code: lumio_api_types::LumioErrorCode,
                ledger_info: &lumio_api_types::LedgerInfo,
            ...
            fn [<$trait_name:snake _with_optional_vm_status_and_ledger_info>]<Err: std::fmt::Display>(
                err: Err,
                error_code: lumio_api_types::LumioErrorCode,
                vm_status: Option<lumio_types::vm_status::StatusCode>,
                ledger_info: Option<&lumio_api_types::LedgerInfo>
            ) -> Self where Self: Sized;
            ...
            fn [<$trait_name:snake _with_vm_status>]<Err: std::fmt::Display>(
                err: Err,
                error_code: lumio_api_types::LumioErrorCode,
                vm_status: lumio_types::vm_status::StatusCode,
                ledger_info: &lumio_api_types::LedgerInfo
            ) -> Self where Self: Sized;

            #[allow(unused)]
            fn [<$trait_name:snake _from_lumio_error>](
                lumio_error: lumio_api_types::LumioError,
                ledger_info: &lumio_api_types::LedgerInfo
            ) -> Self where Self: Sized;
        }
        )*
...
macro_rules! generate_error_response {
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        pub enum $enum_name {
            $(
            #[oai(status = $status)]
            $name(poem_openapi::payload::Json<Box<lumio_api_types::LumioError>>,
                // We use just regular u64 here instead of U64 since all header
                // values are implicitly strings anyway.
                /// Chain ID of the current chain
                #[oai(header = "X-Lumio-Chain-Id")] Option<u8>,
                /// Current ledger version of the chain
                #[oai(header = "X-Lumio-Ledger-Version")] Option<u64>,
                /// Oldest non-pruned ledger version of the chain
                #[oai(header = "X-Lumio-Ledger-Oldest-Version")] Option<u64>,
                /// Current timestamp of the chain
                #[oai(header = "X-Lumio-Ledger-TimestampUsec")] Option<u64>,
                /// Current epoch of the chain
                #[oai(header = "X-Lumio-Epoch")] Option<u64>,
                /// Current block height of the chain
                #[oai(header = "X-Lumio-Block-Height")] Option<u64>,
                /// Oldest non-pruned block height of the chain
                #[oai(header = "X-Lumio-Oldest-Block-Height")] Option<u64>,
                /// The cost of the call in terms of gas
                #[oai(header = "X-Lumio-Gas-Used")] Option<u64>,
            ),
            )*
        }
        ...
        impl $crate::response::[<$name Error>] for $enum_name {
            fn [<$name:snake _with_code>]<Err: std::fmt::Display>(
                ...
                let error = lumio_api_types::LumioError::new_with_error_code(err, error_code);
            ...
            fn [<$name:snake _with_optional_vm_status_and_ledger_info>]<Err: std::fmt::Display>(
                ...
                let error = if let Some(vm_status) = vm_status {
                    lumio_api_types::LumioError::new_with_vm_status(err, error_code, vm_status)
                } else {
                    lumio_api_types::LumioError::new_with_error_code(err, error_code)
                };
                let payload = poem_openapi::payload::Json(Box::new(error));
                Self::from($enum_name::$name(
                    payload,
            ...
            fn [<$name:snake _with_vm_status>]<Err: std::fmt::Display>(
                ...
                let error = lumio_api_types::LumioError::new_with_vm_status(err, error_code, vm_status);
                let payload = poem_openapi::payload::Json(Box::new(error));
                Self::from($enum_name::$name(
                    payload,
                    Some(ledger_info.chain_id),
            ...
            fn [<$name:snake _from_lumio_error>](
                lumio_error: lumio_api_types::LumioError,
                ledger_info: &lumio_api_types::LedgerInfo
            ) -> Self where Self: Sized {
                let payload = poem_openapi::payload::Json(Box::new(lumio_error));
                Self::from($enum_name::$name(
                    payload,
                    Some(ledger_info.chain_id),
                    Some(ledger_info.ledger_version.into()),
        ...
        // Generate a function that helps get the LumioError within.
        impl $crate::response::LumioErrorResponse for $enum_name {
            fn inner_mut(&mut self) -> &mut lumio_api_types::LumioError {
                match self {
                    $(
    ...
    ($enum_name:ident, $(($status:literal, $name:ident)),*) => {
        ...
        pub enum $enum_name<T: poem_openapi::types::ToJSON + Send + Sync> {
            ...
            $name(
                // We use just regular u64 here instead of U64 since all header
                // values are implicitly strings anyway.
                $crate::response::LumioResponseContent<T>,
                /// Chain ID of the current chain
                #[oai(header = "X-Lumio-Chain-Id")] u8,
                /// Current ledger version of the chain
                #[oai(header = "X-Lumio-Ledger-Version")] u64,
                /// Oldest non-pruned ledger version of the chain
                #[oai(header = "X-Lumio-Ledger-Oldest-Version")] u64,
                /// Current timestamp of the chain
                #[oai(header = "X-Lumio-Ledger-TimestampUsec")] u64,
                /// Current epoch of the chain
                #[oai(header = "X-Lumio-Epoch")] u64,
                /// Current block height of the chain
                #[oai(header = "X-Lumio-Block-Height")] u64,
                /// Oldest non-pruned block height of the chain
                #[oai(header = "X-Lumio-Oldest-Block-Height")] u64,
                /// The cost of the call in terms of gas
                #[oai(header = "X-Lumio-Gas-Used")] Option<u64>,
                ...
                #[oai(header = "X-Lumio-Cursor")] Option<String>,
            ),
            )*
        }
        ...
        // Generate a From impl that builds a response from LumioResponseContent.
        ...
        impl <T: poem_openapi::types::ToJSON + Send + Sync> From<($crate::response::LumioResponseContent<T>, &lumio_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (
                    $crate::response::LumioResponseContent<T>,
                    &lumio_api_types::LedgerInfo,
                    [<$enum_name Status>]
                ),
            ) -> Self {
                match status {
        ...
        impl<T: poem_openapi::types::ToJSON + Send + Sync> From<(poem_openapi::payload::Json<T>, &lumio_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (poem_openapi::payload::Json<T>, &lumio_api_types::LedgerInfo, [<$enum_name Status>]),
            ) -> Self {
                let content = $crate::response::LumioResponseContent::Json(value);
                Self::from((content, ledger_info, status))
            }
        }

        // Generate a From impl that builds a response from a Bcs<Vec<u8>> and friends.
        impl<T: poem_openapi::types::ToJSON + Send + Sync> From<($crate::bcs_payload::Bcs, &lumio_api_types::LedgerInfo, [<$enum_name Status>])>
            for $enum_name<T>
        {
            fn from(
                (value, ledger_info, status): (
                ...
                let content = $crate::response::LumioResponseContent::Bcs(value);
                Self::from((content, ledger_info, status))
            }
        }
        ...
        impl<T: poem_openapi::types::ToJSON + Send + Sync + serde::Serialize> $enum_name<T> {
                        ...
                        $crate::bcs_payload::Bcs(
                            bcs::to_bytes(&value)
                                .map_err(|e| E::internal_with_code(
                                    e,
                                    lumio_api_types::LumioErrorCode::InternalError,
                                    ledger_info
                                ))?
                        ),
                        ledger_info,
           ...
           pub fn try_from_json<E: $crate::response::InternalError>(
            ...
            pub fn with_cursor(mut self, new_cursor: Option<lumio_types::state_store::state_key::StateKey>) -> Self {
                match self {
                    $(
                    [<$enum_name>]::$name(_, _, _, _, _, _, _, _, _, ref mut cursor) => {
                        *cursor = new_cursor.map(|c| lumio_api_types::StateKeyWrapper::from(c).to_string());
                    }
                    )*
                }
                self
...
pub fn version_not_found<E: NotFoundError>(ledger_version: u64, ledger_info: &LedgerInfo) -> E {
    build_not_found(
        ...
        LumioErrorCode::VersionNotFound,
        ledger_info,
    )
}
...
pub fn transaction_not_found_by_version<E: NotFoundError>(
    ...
    build_not_found(
        "Transaction",
        format!("Ledger version({})", ledger_version),
        LumioErrorCode::TransactionNotFound,
        ledger_info,
    )
}
...
pub fn account_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        LumioErrorCode::AccountNotFound,
        ledger_info,
    )
}
...
pub fn resource_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        LumioErrorCode::ResourceNotFound,
        ledger_info,
    )
}
...
pub fn struct_field_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        LumioErrorCode::StructFieldNotFound,
        ledger_info,
    )
}
...
pub fn table_item_not_found<E: NotFoundError>(
    ...
    build_not_found(
        ...
        LumioErrorCode::TableItemNotFound,
        ledger_info,
    )
}

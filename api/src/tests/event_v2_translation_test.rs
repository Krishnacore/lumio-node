...
use lumio_crypto::{ed25519::Ed25519PrivateKey, SigningKey, ValidCryptoMaterial};
use lumio_sdk::types::LocalAccount;
use lumio_types::account_config::RotationProofChallenge;
...
static NEW_ACCOUNTS_DEFAULT_TO_FA_LUM_STORE: u64 = 64;
static OPERATIONS_DEFAULT_TO_FA_LUM_STORE: u64 = 65;
...
async fn test_event_v2_translation_coin_deposit_event() {
    ...
    context
        .api_execute_lumio_account_transfer(account2, account1.address(), 101)
        .await;
    ...
    context
        .api_execute_lumio_account_transfer(account2, account1.address(), 102)
        .await;
    ...
    let resp = context
        .gen_events_by_handle(
            &account1.address(),
            "0x1::coin::CoinStore%3C0x1::lumio_coin::LumioCoin%3E",
            "deposit_events",
        )
        .await;
    assert!(is_expected_event(resp.as_array().unwrap().last().unwrap()));
...
async fn test_event_v2_translation_coin_withdraw_event() {
    ...
    context.disable_feature(MODULE_EVENT_MIGRATION).await;
    context
        .disable_feature(NEW_ACCOUNTS_DEFAULT_TO_FA_LUM_STORE)
        .await;
    context
        .disable_feature(OPERATIONS_DEFAULT_TO_FA_LUM_STORE)
        .await;
    context
...
async fn test_event_v2_translation_account_coin_register_event() {
    ...
    let is_expected_event = |e: &Value| {
        matches_event_details(
            ...
            == format!("0x{}", hex::encode("LumioCoin".to_string().as_bytes()))
    };

    // Transfer coins from account2 to account1, emitting V2 events as the feature is enabled
    context
        .api_execute_lumio_account_transfer(account1, account2.address(), 102)
        .await;
    context.wait_for_internal_indexer_caught_up().await;

...
use lumio_crypto::{hash::CryptoHash, signing_message};
...
impl TransactionsApi {
    ...
    ///   1. Hash message bytes: "RawTransaction" bytes + BCS bytes of [Transaction](https://lumio-labs.github.io/lumio-core/lumio_types/transaction/enum.Transaction.html).
    ...
    async fn simulate_transaction(
        ...
        api_spawn_blocking(move || {
            ...
            let estimated_max_gas_amount = if estimate_max_gas_amount.0.unwrap_or_default() {
                ...
                let output = LumioVM::execute_view_function(
                    &state_view,
    ...
    async fn encode_submission(
        ...
        data.0
            ...
            .map_err(|err| {
                BasicError::bad_request_with_code_no_info(err, LumioErrorCode::InvalidInput)
            })?;
...
impl TransactionsApi {
    ...
    async fn create_internal(&self, txn: SignedTransaction) -> Result<(), LumioError> {
        ...
        match mempool_status.code {
            ...
            MempoolStatusCode::InvalidSeqNumber => Err(LumioError::new_with_error_code(
                mempool_status.message,
                LumioErrorCode::SequenceNumberTooOld,
            )),
            MempoolStatusCode::InvalidUpdate => Err(LumioError::new_with_error_code(
                mempool_status.message,
                LumioErrorCode::InvalidTransactionUpdate,
            )),
            MempoolStatusCode::UnknownStatus => Err(LumioError::new_with_error_code(
                format!("Transaction was rejected with status {}", mempool_status,),
                LumioErrorCode::InternalError,
            )),
            MempoolStatusCode::RejectedByFilter => Err(LumioError::new_with_error_code(
                mempool_status.message,
                LumioErrorCode::RejectedByFilter,
            )),
        }
    }
    ...
    async fn create(
        ...
        match self.create_internal(txn.clone()).await {
            ...
            Err(error) => match error.error_code {
                LumioErrorCode::InternalError => Err(
                    SubmitTransactionError::internal_from_lumio_error(error, ledger_info),
                ),
                LumioErrorCode::VmError
                | LumioErrorCode::SequenceNumberTooOld
                | LumioErrorCode::InvalidTransactionUpdate => Err(
                    SubmitTransactionError::bad_request_from_lumio_error(error, ledger_info),
                ),
                LumioErrorCode::MempoolIsFull => Err(
                    SubmitTransactionError::insufficient_storage_from_lumio_error(
                        error,
                        ledger_info,
                    ),
                ),
                _ => Err(SubmitTransactionError::internal_from_lumio_error(
                    error,
                    ledger_info,
                )),
            },
    ...
    // TODO: This function leverages a lot of types from lumio_types, use the
    ...
    pub fn simulate(
        ...
        let (vm_status, output) =
            LumioSimulationVM::create_vm_and_simulate_signed_transaction(&txn, &state_view);
        ...
        let txn = lumio_types::transaction::Transaction::UserTransaction(txn);
        let zero_hash = lumio_crypto::HashValue::zero();
        let info = lumio_types::transaction::TransactionInfo::new(
            txn.hash(),
            zero_hash,
            zero_hash,
            None,
    ...
    pub fn get_signing_message(
        ...
        if accept_type == &AcceptType::Bcs {
            return Err(BasicError::bad_request_with_code_no_info(
                ...
                LumioErrorCode::BcsNotSupported,
            ));
        }
        ...
        let raw_txn: RawTransaction = state_view
            ...
            .map_err(|err| {
                BasicError::bad_request_with_code(err, LumioErrorCode::InvalidInput, &ledger_info)
            })?;

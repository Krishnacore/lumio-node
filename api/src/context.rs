...
use lumio_api_types::{
    transaction::ReplayProtector, LumioErrorCode, AsConverter, BcsBlock, GasEstimation, LedgerInfo,
    ResourceGroup, TransactionOnChainData, TransactionSummary,
};
use lumio_config::config::{GasEstimationConfig, NodeConfig, RoleType};
use lumio_crypto::HashValue;
use lumio_gas_schedule::{LumioGasParameters, FromOnChainGasSchedule};
use lumio_logger::{error, info, Schema};
use lumio_mempool::{MempoolClientRequest, MempoolClientSender, SubmissionStatus};
use lumio_storage_interface::{
    ...
    LumioDbError, DbReader, Order, MAX_REQUEST_LIMIT,
};
use lumio_types::{
...
impl Context {
    ...
    pub fn latest_state_view_poem<E: InternalError>(
        ...
        self.db
            ...
            .map_err(|e| E::internal_with_code(e, LumioErrorCode::InternalError, ledger_info))
    }
    ...
    pub fn state_view<E: StdApiError>(
        ...
        let state_view = self
            ...
            .map_err(|err| {
                E::internal_with_code(err, LumioErrorCode::InternalError, &latest_ledger_info)
            })?;
    ...
    pub fn get_oldest_version_and_block_height<E: ServiceUnavailableError>(
        ...
        self.db
            ...
            .map_err(|e| E::service_unavailable_with_code_no_info(e, LumioErrorCode::InternalError))
    }
    ...
    pub fn get_latest_storage_ledger_info<E: ServiceUnavailableError>(
        ...
        let ledger_info = self
            ...
            .map_err(|e| {
                E::service_unavailable_with_code_no_info(e, LumioErrorCode::InternalError)
            })?;
        ...
        if let Some(indexer_reader) = self.indexer_reader.as_ref() {
            if indexer_reader.is_internal_indexer_enabled() {
                if let Some(mut latest_version) = indexer_reader
                    ...
                    .map_err(|err| {
                        E::service_unavailable_with_code_no_info(err, LumioErrorCode::InternalError)
                    })?
                {
    ...
    pub fn get_block_timestamp<E: InternalError>(
        ...
        self.db
            ...
            .map_err(|err| E::internal_with_code(err, LumioErrorCode::InternalError, ledger_info))
    }
    ...
    pub fn render_transactions_sequential<E: InternalError>(
        ...
        mut timestamp: u64,
    ) -> Result<Vec<lumio_api_types::Transaction>, E> {
        if data.is_empty() {
            return Ok(vec![]);
        }
        ...
        let converter = state_view.as_converter(self.db.clone(), self.indexer_reader.clone());
        let txns: Vec<lumio_api_types::Transaction> = data
            .into_iter()
            .map(|t| {
            ...
            .map_err(|err| {
                E::internal_with_code(err, LumioErrorCode::InternalError, ledger_info)
            })?;
    ...
    ) -> Result<Vec<lumio_api_types::TransactionSummary>, E> {
        if data.is_empty() {
            return Ok(vec![]);
        }

        let txn_summaries: Vec<lumio_api_types::TransactionSummary> = data
            .into_iter()
            .map(|t| {
                Ok(TransactionSummary {
                    ...
                    replay_protector: match t.replay_protector() {
                        lumio_types::transaction::ReplayProtector::Nonce(nonce) => {
                            ReplayProtector::Nonce(nonce.into())
                        },
                        lumio_types::transaction::ReplayProtector::SequenceNumber(seq_num) => {
    ...
    pub fn get_account_ordered_transactions<E: NotFoundError + InternalError>(
        ...
        let txns_res = if !db_sharding_enabled(&self.node_config) {
            ...
            self.indexer_reader
                ...
                .map_err(|e| LumioDbError::Other(e.to_string()))
        };
        let txns = txns_res
    ...
    pub fn get_gas_schedule<E: InternalError>(
        &self,
        ledger_info: &LedgerInfo,
    ) -> Result<(u64, LumioGasParameters), E> {
        ...
        // Otherwise refresh the cache
        {
            ...
            let state_view = self
                ...
                .map_err(|e| {
                    E::internal_with_code(e, LumioErrorCode::InternalError, ledger_info)
                })?;
            ...
            let gas_schedule_params = {
                let may_be_params =
                    GasScheduleV2::fetch_config(&state_view).and_then(|gas_schedule| {
                        ...
                        LumioGasParameters::from_on_chain_gas_schedule(
                            &gas_schedule,
                            feature_version,
                        )
                        .ok()
                ...
                match may_be_params {
                    ...
                    None => GasSchedule::fetch_config(&state_view)
                        .and_then(|gas_schedule| {
                            let gas_schedule = gas_schedule.into_btree_map();
                            LumioGasParameters::from_on_chain_gas_schedule(&gas_schedule, 0).ok()
                        })
                        .ok_or_else(|| {
                            E::internal_with_code(
...
pub struct GasScheduleCache {
    ...
    gas_schedule_params: Option<LumioGasParameters>,
}
    ...
    E: InternalError + Send + 'static,
{
    tokio::task::spawn_blocking(func)
        .await
        .map_err(|err| E::internal_with_code_no_info(err, LumioErrorCode::InternalError))?
}

...
use lumio_api::{attach_poem_to_runtime, BasicError, Context};
use lumio_api_types::{
    mime_types, HexEncodedBytes, TransactionOnChainData, X_LUMIO_CHAIN_ID,
    X_LUMIO_LEDGER_TIMESTAMP, X_LUMIO_LEDGER_VERSION,
};
...
use lumio_crypto::{ed25519::Ed25519PrivateKey, hash::HashValue, SigningKey};
use lumio_db::LumioDB;
use lumio_executor::{block_executor::BlockExecutor, db_bootstrapper};
use lumio_executor_types::BlockExecutorTrait;
use lumio_framework::{BuildOptions, BuiltPackage};
use lumio_indexer_grpc_table_info::internal_indexer_db_service::MockInternalIndexerDBService;
...
use lumio_vm_validator::vm_validator::PooledVMValidator;
use bytes::Bytes;
...
pub fn new_test_context_inner(
    ...
    lumio_vm_logging::disable_speculative_logging();
    let tmp_dir = TempPath::new();
    tmp_dir.create_as_dir().unwrap();

    let mut rng = ::rand::rngs::StdRng::from_seed([0u8; 32]);
    let builder = lumio_genesis::builder::Builder::new(
        tmp_dir.path(),
        lumio_cached_packages::head_release_bundle().clone(),
    )
    .unwrap()
    ...
    let (db, db_rw) = if use_db_with_indexer {
        let mut lumio_db = LumioDB::new_for_test_with_indexer(
            &tmp_dir,
        ...
        {
            lumio_db.add_version_update_subscriber(sender).unwrap();
        }
        DbReaderWriter::wrap(lumio_db)
    } else {
        let mut lumio_db = LumioDB::open(
    ...
    let ret = db_bootstrapper::maybe_bootstrap::<LumioVMBlockExecutor>(
        &db_rw,
        &genesis,
        genesis_waypoint,
    )
    ...
    TestContext::new(
        ...
        root_key,
        validator_owner,
        Box::new(BlockExecutor::<LumioVMBlockExecutor>::new(db_rw)),
        mempool,
        db,
        test_name,
        api_specific_config,
...
impl TestContext {
    ...
    pub async fn root_account(&self) -> LocalAccount {
        ...
        // transactions before.
        let root_sequence_number = self.get_sequence_number(lumio_test_root_address()).await;
        LocalAccount::new(
    ...
    pub async fn enable_feature(&mut self, feature: u64) {
        ...
        //     let lumio_framework = lumio_framework::lumio_governance::get_signer_testnet_only(root, @0x1);
        //     std::features::change_feature_flags_for_next_epoch(&lumio_framework, vector[feature], vector[]);
        //     lumio_framework::lumio_governance::reconfigure(&lumio_framework);
        //     std::features::on_new_epoch(&lumio_framework);
        //   }
        // }
    ...
    pub async fn disable_feature(&mut self, feature: u64) {
        ...
        //   fun main(root: &signer, feature: u64) {
        //     let lumio_framework = lumio_framework::lumio_governance::get_signer_testnet_only(root, @0x1);
        //     std::features::change_feature_flags_for_next_epoch(&lumio_framework, vector[], vector[feature]);
        //     lumio_framework::lumio_governance::reconfigure(&lumio_framework);
        //     std::features::on_new_epoch(&lumio_framework);
        //   }
        // }
    ...
    pub async fn api_create_account(&mut self) -> LocalAccount {
        ...
        let account = self.gen_account();
        self.api_execute_lumio_account_transfer(root, account.address(), TRANSFER_AMOUNT)
            .await;
        account
    }

    pub async fn api_execute_lumio_account_transfer(
        &mut self,
    ...
    pub fn get_latest_ledger_info(&self) -> lumio_api_types::LedgerInfo {
        self.context.get_latest_ledger_info::<BasicError>().unwrap()
    }

    pub fn get_latest_storage_ledger_info(&self) -> lumio_api_types::LedgerInfo {
        self.context
    ...
    fn build_package_with_options(
        ...
        lumio_stdlib::code_publish_package_txn(bcs::to_bytes(&metadata).unwrap(), code)
    }
    ...
    pub async fn get_lum_balance(&self, account: AccountAddress) -> u64 {
        let coin_balance_option = self
            .try_api_get_account_resource(
                ...
                "0x1",
                "coin",
                "CoinStore<0x1::lumio_coin::LumioCoin>",
            )
            .await;
    ...
    // https://github.com/lumio-labs/lumio-core/issues/2966
    ...
    pub async fn execute(&self, req: warp::test::RequestBuilder) -> Value {
        ...
        if self.expect_status_code < 300 {
            ...
            assert_eq!(headers[X_LUMIO_CHAIN_ID], "4");
            assert_eq!(
                headers[X_LUMIO_LEDGER_VERSION],
                ledger_info.version().to_string()
            );
            assert_eq!(
                headers[X_LUMIO_LEDGER_TIMESTAMP],

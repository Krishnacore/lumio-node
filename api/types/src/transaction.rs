...
use crate::{
    Address, LumioError, EntryFunctionId, EventGuid, HashValue, HexEncodedBytes,
    ...
    pub fn from_transaction_onchain_data(
        ...
        if txn.version > latest_ledger_version {
            match txn.transaction {
                lumio_types::transaction::Transaction::UserTransaction(txn) => {
...
pub struct TransactionOnChainData {
    ...
    pub transaction: lumio_types::transaction::Transaction,
    /// Information about the transaction
    pub info: lumio_types::transaction::TransactionInfo,
    /// Events emitted by the transaction
    pub events: Vec<ContractEvent>,
    /// The accumulator root hash at this version
    pub accumulator_root_hash: lumio_crypto::HashValue,
    /// Final state of resources changed by the transaction
    pub changes: lumio_types::write_set::WriteSet,
}

impl From<(TransactionWithProof, lumio_crypto::HashValue)> for TransactionOnChainData {
    fn from((txn, accumulator_root_hash): (TransactionWithProof, lumio_crypto::HashValue)) -> Self {
        Self {
            version: txn.version,
...
impl
    From<(
        u64,
        lumio_types::transaction::Transaction,
        lumio_types::transaction::TransactionInfo,
        Vec<ContractEvent>,
        lumio_crypto::HashValue,
        lumio_types::write_set::WriteSet,
    )> for TransactionOnChainData
{
    fn from(
...
/// Enum of the different types of transactions in Lumio
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Union)]
...
impl UserTransactionRequest {
    pub fn replay_protector(&self) -> lumio_types::transaction::ReplayProtector {
        if let Some(nonce) = self.replay_protection_nonce {
            lumio_types::transaction::ReplayProtector::Nonce(nonce.0)
        } else {
            lumio_types::transaction::ReplayProtector::SequenceNumber(self.sequence_number.0)
        }
    }
}
...
pub struct BlockMetadataTransaction {
    ...
    /// If some, it means the internal txn type is `lumio_types::transaction::Transaction::BlockMetadataExt`.
    /// Otherwise, it is `lumio_types::transaction::Transaction::BlockMetadata`.
    ///
    /// NOTE: we could have introduced a new LUM txn type to represent the corresponding internal type,
...
impl
    From<(
        lumio_types::validator_txn::ValidatorTransaction,
        TransactionInfo,
        Vec<Event>,
        u64,
    )> for ValidatorTransaction
{
    fn from(
        ...
        match txn {
            lumio_types::validator_txn::ValidatorTransaction::DKGResult(dkg_transcript) => {
            ...
            lumio_types::validator_txn::ValidatorTransaction::ObservedJWKUpdate(
...
/// A more API-friendly representation of the on-chain `lumio_types::jwks::QuorumCertifiedUpdate`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
...
/// A more API-friendly representation of the on-chain `lumio_types::aggregate_signature::AggregateSignature`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
...
/// A more API-friendly representation of the on-chain `lumio_types::jwks::ProviderJWKs`.
#[derive(Clone, Debug, PartialEq, Eq, Serialize, Deserialize, Object)]
...
impl TryFrom<&MultiEd25519Signature> for TransactionAuthenticator {
    ...
    fn try_from(value: &MultiEd25519Signature) -> Result<Self, Self::Error> {
        ...
        Ok(TransactionAuthenticator::multi_ed25519(
            ...
            lumio_crypto::multi_ed25519::MultiEd25519Signature::new_with_signatures_and_bitmap(
                ed25519_signatures,

...
use lumio_crypto::{x25519::PrivateKey, Uniform};
use lumio_event_notifications::{ReconfigNotification, ReconfigNotificationListener};
use lumio_infallible::{Mutex, MutexGuard, RwLock};
...
pub struct NodeNetworkInterface {
    ...
    pub(crate) network_reqs_rx: lumio_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    /// Peer notification sender for sending outgoing messages to other peers
    pub(crate) network_notifs_tx: lumio_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>,
}
...
fn setup_node_network_interface(
    ...
    let (connection_reqs_tx, _) = lumio_channel::new(QueueStyle::FIFO, MAX_QUEUE_SIZE, None);

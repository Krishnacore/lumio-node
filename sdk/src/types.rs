...
use lumio_crypto::{ed25519::Ed25519Signature, secp256r1_ecdsa, HashValue, PrivateKey, SigningKey};
use lumio_ledger::LumioLedgerError;
use lumio_rest_client::{lumio_api_types::MoveStructTag, Client, PepperRequest, ProverRequest};
pub use lumio_types::*;
use lumio_types::{
    event::EventKey,
...
/// LocalAccount represents an account on the Lumio blockchain. Internally it
...
pub fn get_paired_fa_primary_store_address(
    ...
    bytes.push(0xFC);
    AccountAddress::from_bytes(lumio_crypto::hash::HashValue::sha3_256_of(&bytes).to_vec()).unwrap()
}
...
impl LocalAccount {
    ...
    /// does not actually create an account on the Lumio blockchain, just a
    ...
    /// create an account on the Lumio blockchain, it just generates a new
    /// account locally.
...
impl HardwareWalletAccount {
    ...
    /// This requires the Ledger device to be connected, unlocked and the Lumio app to be opened
    pub fn from_ledger(
        derivation_path: String,
        sequence_number: u64,
    ) -> Result<Self, LumioLedgerError> {
        let public_key = lumio_ledger::get_public_key(&derivation_path, false)?;
    ...
    pub fn sign_arbitrary_message(
        &self,
        message: &[u8],
    ) -> Result<Ed25519Signature, LumioLedgerError> {
        lumio_ledger::sign_message(&self.derivation_path, message)
    }
}
...
mod tests {
    ...
    use lumio_rest_client::{LumioBaseUrl, FaucetClient};
    use reqwest::Url;

    #[test]
    fn test_recover_account_from_derive_path() {
        // Same constants in test cases of TypeScript
        // https://github.com/lumio-labs/lumio-core/blob/main/ecosystem/typescript/sdk/src/lumio_account.test.ts
    ...
    async fn test_derive_keyless_account() {
        let lumio_rest_client = Client::builder(LumioBaseUrl::Devnet).build();
        // This JWT is taken from https://github.com/lumio-labs/lumio-ts-sdk/blob/f644e61beb70e69dfd489e75287c67b527385135/tests/e2e/api/keyless.test.ts#L11
        ...
        let mut account = derive_keyless_account(&lumio_rest_client, jwt, ephemeral_key_pair, None)
            .await
            .unwrap();
        ...
        if balance < 10000000 {
            ...
            let faucet_client = FaucetClient::new_from_rest_client(
                Url::from_str("https://faucet.devnet.lumiolabs.com").unwrap(),
                lumio_rest_client.clone(),
            );
            faucet_client
        ...
        let coin_client = CoinClient::new(&lumio_rest_client);

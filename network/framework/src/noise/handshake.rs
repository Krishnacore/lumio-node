...
use lumio_crypto::{noise, x25519};
use lumio_infallible::{duration_since_epoch, RwLock};
use lumio_logger::{error, trace};
use lumio_short_hex_str::{AsShortHexStr, ShortHexStr};
...
pub enum HandshakeAuthMode {
    ...
    ///
    /// For example, in the Lumio validator network, validator peers will only
                ...
                match trusted_peer {
                    ...
                    None => {
                        ...
                        let derived_remote_peer_id =
                            lumio_types::account_address::from_identity_public_key(
                                remote_public_key,
                            );
...
mod test {
    ...
    use lumio_config::config::{Peer, PeerRole, RoleType};
    use lumio_crypto::{

...
use lumio_id_generator::{IdGenerator, U32IdGenerator};
use lumio_logger::prelude::*;
// Re-exposed for lumio-network-checker
pub use lumio_netcore::transport::tcp::{resolve_and_connect, TCPBufferCfg, TcpSocket};
use lumio_netcore::transport::{proxy_protocol, tcp, ConnectionOrigin, Transport};
use lumio_short_hex_str::AsShortHexStr;
use lumio_time_service::{timeout, TimeService, TimeServiceTrait};
use lumio_types::{
    chain_id::ChainId,
...
/// tcp::Transport with Lumio-specific configuration applied.
pub const LUMIO_TCP_TRANSPORT: tcp::TcpTransport = tcp::TcpTransport {
    // Use default options.
    ttl: None,
    // Use TCP_NODELAY for Lumio tcp connections.
    nodelay: Some(true),
...
async fn upgrade_inbound<T: TSocket>(
    ...
    // try to negotiate common lumionet version and supported application protocols
...
// ideas in this PR thread: https://github.com/lumio-labs/lumio-core/pull/3478#issuecomment-617385633
pub struct LumioNetTransport<TTransport> {
...
impl<TTransport> LumioNetTransport<TTransport>
where
    ...
    TTransport::Listener: Send + 'static,
{
    ...
    fn parse_dial_addr(
        ...
        // parse out the lumionet protocols (noise ik and handshake)
...
// If using `LumioNetTransport` as a `Transport` trait, then all upgrade futures
...
impl<TTransport: Transport> Transport for LumioNetTransport<TTransport>
where

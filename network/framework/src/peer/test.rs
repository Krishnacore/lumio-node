...
fn build_test_peer(
    ...
    PeerHandle,
    MemorySocket,
    lumio_channels::Receiver<TransportNotification<MemorySocket>>,
) {
    ...
    let (connection_notifs_tx, connection_notifs_rx) = lumio_channels::new_test(1);
    let (peer_reqs_tx, peer_reqs_rx) =
        lumio_channel::new(QueueStyle::FIFO, NETWORK_CHANNEL_SIZE, None);
...
async fn assert_disconnected_event(
    ...
    connection_notifs_rx: &mut lumio_channels::Receiver<TransportNotification<MemorySocket>>,
) {
...
#[derive(Clone)]
struct PeerHandle(lumio_channel::Sender<ProtocolId, PeerRequest>);
...
fn test_upstream_handlers() -> (
    ...
    let (sender, receiver) = lumio_channel::new(QueueStyle::FIFO, 100, None);

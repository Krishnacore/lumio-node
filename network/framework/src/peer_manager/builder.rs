...
use crate::{
    ...
    transport::{self, LumioNetTransport, Connection, LUMIO_TCP_TRANSPORT},
    ProtocolId,
};
use lumio_channels::{self, lumio_channel, message_queues::QueueStyle};
use lumio_config::{config::HANDSHAKE_VERSION, network_id::NetworkContext};
use lumio_crypto::x25519;
use lumio_logger::prelude::*;
#[cfg(any(test, feature = "testing", feature = "fuzzing"))]
use lumio_netcore::transport::memory::MemoryTransport;
use lumio_netcore::transport::{
    tcp::{TCPBufferCfg, TcpSocket, TcpTransport},
    Transport,
};
use lumio_time_service::TimeService;
use lumio_types::{chain_id::ChainId, network_address::NetworkAddress, PeerId};
...
struct PeerManagerContext {
    ...
    pm_reqs_tx: lumio_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
    pm_reqs_rx: lumio_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    connection_reqs_tx: lumio_channel::Sender<PeerId, ConnectionRequest>,
    connection_reqs_rx: lumio_channel::Receiver<PeerId, ConnectionRequest>,

    peers_and_metadata: Arc<PeersAndMetadata>,
    upstream_handlers:
        HashMap<ProtocolId, lumio_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>>,
...
impl PeerManagerContext {
    ...
    fn new(
        ...
        upstream_handlers: HashMap<
            ProtocolId,
            lumio_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>,
        >,
        connection_event_handlers: Vec<conn_notifs_channel::Sender>,
    ...
    fn add_upstream_handler(
        ...
        channel: lumio_channel::Sender<(PeerId, ProtocolId), ReceivedMessage>,
    ) -> &mut Self {
...
type MemoryPeerManager =
    PeerManager<LumioNetTransport<MemoryTransport>, NoiseStream<lumio_memsocket::MemorySocket>>;
type TcpPeerManager = PeerManager<LumioNetTransport<TcpTransport>, NoiseStream<TcpSocket>>;
...
impl PeerManagerBuilder {
    ...
    pub fn create(
        ...
        let (pm_reqs_tx, pm_reqs_rx) = lumio_channel::new(
            QueueStyle::FIFO,
            channel_size,
        ...
        let (connection_reqs_tx, connection_reqs_rx) =
            lumio_channel::new(QueueStyle::FIFO, channel_size, None);
    ...
    pub fn connection_reqs_tx(&self) -> lumio_channel::Sender<PeerId, ConnectionRequest> {
    ...
    pub fn build(&mut self, executor: &Handle) -> &mut Self {
        use lumio_types::network_address::Protocol::*;
        ...
        let mut lumio_tcp_transport = LUMIO_TCP_TRANSPORT.clone();
        let tcp_cfg = self.get_tcp_buffers_cfg();
        lumio_tcp_transport.set_tcp_buffers(&tcp_cfg);
    ...
    ) -> lumio_channel::Receiver<(PeerId, ProtocolId), ReceivedMessage> {
        // Register the direct send and rpc protocols
        self.transport_context()
            .add_protocols(&config.direct_send_protocols_and_preferences);
        self.transport_context()

...
use lumio_config::config::PeerSet;
#[cfg(test)]
use lumio_logger::spawn_named;
...
mod tests {
    ...
    use lumio_channels::Receiver;
    use lumio_config::{
        config::{Peer, PeerRole},
        network_id::NetworkContext,
    };
    use lumio_event_notifications::DbBackedOnChainConfig;
    use lumio_network::connectivity_manager::{ConnectivityRequest, DiscoverySource};
    ...
    fn create_listener(path: Arc<TempPath>) -> Receiver<ConnectivityRequest> {
        ...
        let (conn_mgr_reqs_tx, conn_mgr_reqs_rx) = lumio_channels::new(
            1,
            &lumio_network::counters::PENDING_CONNECTIVITY_MANAGER_REQUESTS,
        );

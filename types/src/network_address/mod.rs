...
/// ## Overview
///
/// Lumio `NetworkAddress` is a compact, efficient, self-describing and
...
/// 4. Perform a LumioNet version negotiation handshake (version 1).
///
...
/// ## Transport
///
/// In addition, `NetworkAddress` is integrated with the LumioNet concept of a
...
///
/// use lumio_types::network_address::NetworkAddress;
/// use bcs;
...
impl NetworkAddress {
    ...
    ///
    /// ```rust
    /// use lumio_crypto::{traits::ValidCryptoMaterialStringExt, x25519};
    /// use lumio_types::network_address::NetworkAddress;
    /// use std::str::FromStr;
    ///
    ...
    /// Check that a `NetworkAddress` looks like a typical LumioNet address with
    /// associated protocols.
    ///
    /// "typical" LumioNet addresses begin with a transport protocol:
    ///
    ...
    /// assert!(addr.is_lumionet_addr());
    /// ```
    pub fn is_lumionet_addr(&self) -> bool {
        parse_lumionet_protos(self.as_slice()).is_some()
    }
...
#[cfg(any(test, feature = "fuzzing"))]
pub fn arb_lumionet_addr() -> impl Strategy<Value = NetworkAddress> {
    ...
    let arb_lumionet_protos = any::<(x25519::PublicKey, u8)>()
        .prop_map(|(pubkey, hs)| vec![Protocol::NoiseIK(pubkey), Protocol::Handshake(hs)]);

    (arb_transport_protos, arb_lumionet_protos).prop_map(
        |(mut transport_protos, mut lumionet_protos)| {
            transport_protos.append(&mut lumionet_protos);
...
/// parse canonical lumionet protocols
///
/// See: [`NetworkAddress::is_lumionet_addr`]
fn parse_lumionet_protos(protos: &[Protocol]) -> Option<&[Protocol]> {
...
mod test {
    ...
    proptest! {
        #[test]
        fn test_network_address_canonical_serialization(addr in arb_lumionet_addr()) {
        ...
        #[test]
        fn test_network_address_display_roundtrip(addr in arb_lumionet_addr()) {
        ...
        #[test]
        fn test_is_lumionet_addr(addr in arb_lumionet_addr()) {
            assert!(addr.is_lumionet_addr(), "addr.is_lumionet_addr() = false; addr: '{}'", addr);
        }
    }
}

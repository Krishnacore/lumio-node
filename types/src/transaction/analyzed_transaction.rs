...
pub fn lumio_coin_info_location() -> StorageLocation {
    StorageLocation::Specific(
        StateKey::resource_typed::<CoinInfoResource<LumioCoinType>>(
         ...
         -> (Vec<StorageLocation>, Vec<StorageLocation>) {
            match (
                *func.module().address(),
                ...
                (AccountAddress::ONE, "lumio_account", "transfer") => {
                    let receiver_address = bcs::from_bytes(&func.args()[0]).unwrap();

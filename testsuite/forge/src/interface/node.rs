...
use anyhow::anyhow;
use lumio_config::{config::NodeConfig, network_id::NetworkId};
use lumio_inspection_service::inspection_client::InspectionClient;
use lumio_rest_client::{LumioBaseUrl, Client as RestClient};
...
pub trait NodeExt: Node {
    ...
    fn rest_client_with_timeout(&self, timeout: Duration) -> RestClient {
        RestClient::builder(LumioBaseUrl::Custom(self.rest_api_endpoint()))
            .timeout(timeout)
            .build()
    }
    ...
    async fn get_connected_peers(
        ...
        self.get_metric_with_fields_i64("lumio_connections", map)
            .await
    }

...
use crate::{
    ...
    AdminContext, AdminTest, LumioContext, LumioTest, CoreContext, Factory, NetworkContext,
...
#[derive(Debug, Parser)]
#[clap(about = "Forged in Fire", styles = lumio_cli_common::lumio_cli_style())]
pub struct Options {
...
impl Test for AnyTestRef<'_> {
    ...
    fn should_fail(&self) -> ShouldFail {
        match self {
            AnyTestRef::Lumio(t) => t.should_fail(),
...
impl<'cfg, F: Factory> Forge<'cfg, F> {
    ...
    pub fn run(&self) -> Result<TestSummary> {
        ...
        if test_count > 0 {
            ...
            // Run LumioTests
            for test in self.filter_tests(&self.tests.lumio_tests) {
                let mut lumio_ctx = LumioContext::new(
                    CoreContext::from_rng(&mut rng),
                    swarm.chain_info().into_lumio_public_info(),
                    &mut report,
                );
                let result = process_test_result(runtime.block_on(test.run(&mut lumio_ctx)));

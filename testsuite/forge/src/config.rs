...
pub struct ForgeConfig {
    ...
    pub lumio_tests: Vec<Box<dyn LumioTest>>,
...
impl ForgeConfig {
    ...
    pub fn add_lumio_test<T: LumioTest + 'static>(mut self, lumio_test: T) -> Self {
        self.lumio_tests.push(Box::new(lumio_test));
        self
    }
    ...
    pub fn with_lumio_tests(mut self, lumio_tests: Vec<Box<dyn LumioTest>>) -> Self {
        self.lumio_tests = lumio_tests;
        self
    }
    ...
    pub fn build_node_helm_config_fn(&self, retain_debug_logs: bool) -> Option<NodeConfigFn> {
        ...
        // Override specific helm values. See reference: terraform/helm/lumio-node/values.yaml
        Some(Arc::new(move |helm_values: &mut serde_yaml::Value| {
            ...
            if retain_debug_logs {
                helm_values["validator"]["podAnnotations"]["lumio.dev/min-log-level-to-retain"] =
                    serde_yaml::Value::String("debug".to_owned());
                helm_values["fullnode"]["podAnnotations"]["lumio.dev/min-log-level-to-retain"] =
                    serde_yaml::Value::String("debug".to_owned());
    ...
    pub fn number_of_tests(&self) -> usize {
        self.admin_tests.len() + self.network_tests.len() + self.lumio_tests.len()
    }
    ...
    pub fn all_tests(&self) -> Vec<Box<AnyTestRef<'_>>> {
        self.admin_tests
            ...
            .chain(
                self.lumio_tests
                    .iter()
                    .map(|t| Box::new(AnyTestRef::Lumio(t.as_ref()))),
            )
            .collect()
    }
}

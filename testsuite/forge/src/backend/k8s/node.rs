...
use lumio_db::common::{LEDGER_DB_NAME, STATE_MERKLE_DB_NAME};
use lumio_logger::info;
use lumio_rest_client::Client as RestClient;
use lumio_sdk::types::PeerId;
use lumio_state_sync_driver::metadata_storage::STATE_SYNC_DB_NAME;
...
const LUMIO_DATA_DIR: &str = "/opt/lumio/data";
...
impl Node for K8sNode {
    ...
    async fn clear_storage(&self) -> Result<()> {
        ...
        let ledger_db_path = format!("{}/db/{}", LUMIO_DATA_DIR, LEDGER_DB_NAME);
        let state_db_path = format!("{}/db/{}", LUMIO_DATA_DIR, STATE_MERKLE_DB_NAME);
        let state_sync_db_path = format!("{}/db/{}", LUMIO_DATA_DIR, STATE_SYNC_DB_NAME);

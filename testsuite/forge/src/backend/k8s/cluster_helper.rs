...
use crate::{
    ...
    NodeConfigFn, ReadWrite, Result, LUMIO_NODE_HELM_RELEASE_NAME, DEFAULT_ROOT_KEY,
...
async fn wait_node_haproxy(
    ...
    lumio_retrier::retry_async(k8s_wait_nodes_strategy(), || {
        ...
        Box::pin(async move {
            for i in 0..num_haproxy {
                let haproxy_deployment_name =
                    format!("{}-{}-haproxy", LUMIO_NODE_HELM_RELEASE_NAME, i);
...
/// Deletes a collection of resources in k8s as part of lumio-node
...
pub(crate) async fn delete_k8s_resources(client: K8sClient, kube_namespace: &str) -> Result<()> {
    ...
    let lumio_node_helm_selector = "app.kubernetes.io/part-of=lumio-node";
    let testnet_addons_helm_selector = "app.kubernetes.io/part-of=testnet-addons";
    let genesis_helm_selector = "app.kubernetes.io/part-of=lumio-genesis";
    ...
    // cross this with all the compute resources created by lumio-node helm chart
...
pub async fn uninstall_testnet_resources(kube_namespace: String) -> Result<()> {
    ...
    info!(
        "lumio-node resources for Forge removed in namespace: {}",
        kube_namespace
    );
...
fn get_node_default_helm_path() -> String {
    match ForgeRunnerMode::try_from_env().unwrap_or(ForgeRunnerMode::K8s) {
        ForgeRunnerMode::Local => {
            "testsuite/forge/src/backend/k8s/helm-values/lumio-node-default-values.yaml"
        },
        ForgeRunnerMode::K8s => "/lumio/terraform/lumio-node-default-values.yaml",
    }
    .to_string()
}
...
/// Installs a testnet in a k8s namespace by first running genesis, and the installing the lumio-nodes via helm
...
pub async fn install_testnet_resources(
    ...
    let mut lumio_node_helm_values =
        get_default_helm_release_values_from_cluster(LUMIO_NODE_HELM_RELEASE_NAME)
    ...
    let lumio_node_helm_values_override = construct_node_helm_values_from_input(
        node_helm_config_fn,
    ...
    info!(
        "lumio_node_helm_values: {}",
        serde_yaml::to_string(&lumio_node_helm_values).unwrap()
    );
    info!(
        "lumio_node_helm_values_override: {}",
        serde_yaml::to_string(&lumio_node_helm_values_override).unwrap()
    );

    merge_yaml(&mut lumio_node_helm_values, lumio_node_helm_values_override);
    merge_yaml(&mut genesis_helm_values, genesis_helm_values_override);

    info!(
        "lumio_node_helm_values after override: {}",
        serde_yaml::to_string(&lumio_node_helm_values).unwrap(),
    );

    // disable uploading genesis to blob storage since indexer requires it in the cluster
    if enable_indexer {
        lumio_node_helm_values["genesis_blob_upload_url"] = "".into();
    }
    ...
    let config: serde_json::Value = serde_json::from_value(serde_json::json!({
        ...
        "testnet-values": lumio_node_helm_values,

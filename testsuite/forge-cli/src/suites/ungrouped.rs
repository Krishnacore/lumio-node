...
use lumio_config::config::{ConsensusConfig, MempoolConfig, NodeConfig};
use lumio_forge::{
    ...
    AdminContext, AdminTest, LumioContext, LumioTest, EmitJobMode, EmitJobRequest, ForgeConfig,
...
pub fn run_forever() -> ForgeConfig {
    ForgeConfig::default()
        ...
        .with_genesis_module_bundle(lumio_cached_packages::head_release_bundle().clone())
        .add_lumio_test(RunForever)
}

pub fn local_test_suite() -> ForgeConfig {
    ForgeConfig::default()
        .add_lumio_test(FundAccount)
        .add_lumio_test(TransferCoins)
        .add_admin_test(GetMetadata)
...
fn background_emit_request(high_gas_price: bool) -> EmitJobRequest {
    ...
    if high_gas_price {
        result = result.gas_price(5 * lumio_global_constants::GAS_UNIT_PRICE);
    }
    result
}
...
pub(crate) fn single_cluster_test(
    ...
    ForgeConfig::default()
        ...
        .with_emit_job(
            EmitJobRequest::default()
                .mode(EmitJobMode::ConstTps { tps: 10 })
                .gas_price(5 * lumio_global_constants::GAS_UNIT_PRICE),
        )
...
pub fn optimize_state_sync_for_throughput(node_config: &mut NodeConfig, max_chunk_size: u64) {
    ...
    let data_client_config = &mut node_config.state_sync.lumio_data_client;
...
impl LumioTest for RunForever {
    async fn run<'t>(&self, _ctx: &mut LumioContext<'t>) -> Result<()> {
...
impl AdminTest for GetMetadata {
    fn run(&self, ctx: &mut AdminContext<'_>) -> Result<()> {
        ...
        runtime.block_on(client.get_lumio_version()).unwrap();
...
pub async fn check_account_balance(
    ...
    let balance = client
        .view_lum_account_balance(account_address)
        .await?
        .into_inner();
...
#[async_trait::async_trait]
impl LumioTest for FundAccount {
    async fn run<'t>(&self, ctx: &mut LumioContext<'t>) -> Result<()> {
...
#[async_trait::async_trait]
impl LumioTest for TransferCoins {
    async fn run<'t>(&self, ctx: &mut LumioContext<'t>) -> Result<()> {
        let client = ctx.client();
        ...
        let transfer_txn = payer.sign_with_transaction_builder(
            ctx.lumio_transaction_factory()
                .payload(lumio_stdlib::lumio_coin_transfer(payee.address(), 10)),
        );

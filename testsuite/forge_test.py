    ...
    def testFindRecentImage(self) -> None:
        shell = SpyShell(
            [
                ...
                FakeCommand(
                    "aws ecr describe-images --repository-name lumio/validator-testing --image-ids imageTag=potato",
                    RunResult(1, b""),
                ),
                FakeCommand("git rev-parse HEAD~1", RunResult(0, b"lychee\n")),
                FakeCommand(
                    "aws ecr describe-images --repository-name lumio/validator-testing --image-ids imageTag=lychee",
                    RunResult(0, b""),
                ),
            ]
        )
    ...
    def testFindRecentFailpointsImage(self) -> None:
        shell = SpyShell(
            [
                ...
                FakeCommand(
                    "aws ecr describe-images --repository-name lumio/validator-testing --image-ids imageTag=failpoints_tomato",
                    RunResult(0, b""),
                ),
            ]
        )
    ...
    def testFindRecentPerformanceImage(self) -> None:
        shell = SpyShell(
            [
                FakeCommand("git rev-parse HEAD~0", RunResult(0, b"potato\n")),
                FakeCommand(
                    "aws ecr describe-images --repository-name lumio/validator-testing --image-ids imageTag=performance_potato",
                    RunResult(0, b""),
                ),
            ]
        )
    ...
    def testDidntFindRecentImage(self) -> None:
        shell = SpyShell(
            [
                ...
                FakeCommand(
                    "aws ecr describe-images --repository-name lumio/validator-testing --image-ids imageTag=crab",
                    RunResult(1, b""),
                ),
            ]
        )
        ...
        with self.assertRaises(Exception):
            list(
                find_recent_images(
                    shell, git, 1, "lumio/validator-testing", commit_threshold=1
                )
            )
    ...
    def testFindRecentFewImages(
        ...
        shell = SpyShell(
            [
                FakeCommand("git rev-parse HEAD~0", RunResult(0, b"crab\n")),
                FakeCommand(
                    "aws ecr describe-images --repository-name lumio/validator --image-ids imageTag=crab",
                    RunResult(0, b""),
                ),
                FakeCommand("git rev-parse HEAD~1", RunResult(0, b"shrimp\n")),
                FakeCommand(
                    "aws ecr describe-images --repository-name lumio/validator --image-ids imageTag=shrimp",
                    RunResult(0, b""),
                ),
            ]
        )
...
class ForgeFormattingTests(unittest.TestCase, AssertFixtureMixin):
    ...
    def testDashboardLinkAutoRefresh(self) -> None:
        self.assertFixture(
            get_dashboard_link(
                "forge-pr-2983",
                # Chain names don't use the "lumio-" prefix.
                "forge-big-1",
                True,
            ),
        ...
        shell = SpyShell(
            [
                ...
                FakeCommand(
                    ...
                    RunResult(0, b'{ "clusters": [ "lumio-forge-big-1" ] }'),
                ),
                FakeCommand(
                    ...
                    "aws eks update-kubeconfig --name lumio-forge-big-1 --kubeconfig temp1",
                    RunResult(0, b""),
                ),
                FakeCommand("git rev-parse HEAD~0", RunResult(0, b"banana")),
                FakeCommand(
                    "aws ecr describe-images --repository-name lumio/validator-testing "
                ...
                FakeCommand(
                    "aws ecr describe-images --repository-name lumio/forge --image-ids "
                    "imageTag=banana",
                    RunResult(0, b""),
                ),
                FakeCommand(
            ...
            stack.enter_context(
                patch.object(
                    ...
                    lambda *_: FakeConfigBackend(
                        {
                            "enabled_clusters": ["lumio-forge-big-1"],
                            "all_clusters": ["lumio-forge-big-1", "banana"],
                            "test_suites": {},
                        }
                    ),
                )
    ...
    def testListClusters(self) -> None:
        fake_clusters = json.dumps(
            AwsListClusterResult(
                clusters=[
                    "banana-fake-1",
                    "lumio-forge-banana-1",
                    "lumio-forge-potato-2",
                ]
            ),
        )
        shell = SpyShell(
        ...
        self.assertEqual(clusters, ["lumio-forge-banana-1", "lumio-forge-potato-2"])
        shell.assert_commands(self)
...
class GetForgeJobsTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def testGetAllForgeJobs(self) -> None:
        fake_clusters = ["lumio-forge-banana", "lumio-forge-apple-2"]
        ...
        fake_forge_first_first_cluster_pods = GetPodsResult(
            items=[
                fake_pod_item("lumio-node-0-validator", "Running"),
                fake_pod_item("lumio-node-1-validator", "Running"),
            ]
        )
        fake_forge_first_failed_cluster_pods = GetPodsResult(
            items=[
                fake_pod_item("lumio-node-0-validator", "Running"),
                fake_pod_item("lumio-node-1-validator", "Running"),
                fake_pod_item("lumio-node-0-fullnode", "Running"),
                fake_pod_item("lumio-node-1-fullnode", "Running"),
            ]
        )
        fake_forge_first_ignore_me_cluster_pods = GetPodsResult(
            items=[
                fake_pod_item("lumio-node-0-validator", "Failed"),
                fake_pod_item("lumio-node-1-validator", "Running"),
            ]
        )
        ...
        shell = SpyShell(
            [
                FakeCommand(
                    "aws eks update-kubeconfig --name lumio-forge-banana --kubeconfig temp1",
                    RunResult(0, b""),
                ),
                FakeCommand(
                ...
                FakeCommand(
                    "aws eks update-kubeconfig --name lumio-forge-apple-2 --kubeconfig temp2",
                    RunResult(0, b""),
                ),
                FakeCommand(
...
class ForgeConfigTests(unittest.TestCase):
            ...
            validate_forge_config(
                {
                    ...
                    "all_clusters": ["banana", "apple"],
                    "default_helm_values": {
                        "lumio-node": {"image": {"tag": "banana"}},
                        "lumio-genesis": {"image": {"tag": "banana"}},
                    },
                }
            ),
            [],
    ...
    def testHelmGetConfig(self) -> None:
        ...
        helm_after_missing = {
            ...
            "all_clusters": ["banana", "apple"],
            "default_helm_values": {
                "lumio-node": {"apple": "enabled", "banana": {"enabled": "true"}}
            },
        }
        helm_after_complete = {
            "enabled_clusters": ["banana"],
            ...
            "default_helm_values": {
                "lumio-node": {"apple": "enabled", "banana": {"enabled": "true"}},
                "lumio-genesis": {"apple": "enabled", "banana": {"enabled": "true"}},
            },
        }
        runner = CliRunner()
        shell = SpyShell(
        ...
        with ExitStack() as stack:
            ...
            result_helm_config_not_present: Result = runner.invoke(
                main,
                ["--no-log-metadata", "config", "helm", "get", "lumio-node"],
                catch_exceptions=True,
            )
            result_helm_config_present_missing = runner.invoke(
                main,
                ["--no-log-metadata", "config", "helm", "get", "lumio-genesis"],
                catch_exceptions=True,
            )
            ...
            self.assertEqual(
                ...
                Exception("No helm values found for chart lumio-genesis").args,
            )
            ...
            self.assertIsNotNone(helm_after_complete.get("default_helm_values").get("lumio-node"))  # type: ignore
            # the output config is printed with an extra newline
            self.assertEqual(
                result_helm_config_present_complete.stdout_bytes,
                f'{json.dumps(helm_after_complete.get("default_helm_values").get("lumio-node"), indent=2)}\n'.encode(),  # type: ignore
            )
    ...
    def testHelmSetConfig(self) -> None:
        ...
        config_before = {
            ...
            "all_clusters": ["banana", "apple"],
            "default_helm_values": {
                "lumio-node": {"apple": "enabled", "banana": {"enabled": "false"}}
            },
        }
        config_after = {
            **config_before,
        ...
        filesystem = SpyFilesystem(
            ...
            },
            {
                ...
                "temp2": json.dumps(
                    config_after["default_helm_values"]["lumio-node"]
                ).encode(),
            },
        )
        with ExitStack() as stack:
            ...
            ret = runner.invoke(
                main,
                ["config", "helm", "set", "lumio-node", "--config", "temp2", "-y"],
                catch_exceptions=True,
            )
            shell.assert_commands(self)
            filesystem.assert_reads(self)

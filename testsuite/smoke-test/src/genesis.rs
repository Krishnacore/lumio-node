...
use lumio_types::{transaction::Transaction, waypoint::Waypoint};
...
/// 3. Use the lumio CLI to generate a genesis transaction that removes the last validator from the set.
/// 4. Use the lumio-debugger to manually apply the genesis transaction to all remaining validators.
/// 5. Verify that the network is able to resume consensus and that the last validator is no longer in the set.
/// 6. Use the lumio-debugger to manually apply the genesis transaction to all VFNs.
/// 7. Verify that the VFNs are able to sync with the rest of the network.
async fn test_fullnode_genesis_transaction_flow() {
    println!("0. Building the Lumio CLI and debugger!");
    let lumio_debugger = workspace_builder::get_bin("lumio-debugger");
    ...
    let (genesis_blob_path, genesis_transaction) =
        generate_genesis_transaction(&mut swarm, lumio_cli);
    ...
    let output = Command::new(lumio_debugger.as_path())
...
/// 4. Use the lumio CLI to generate a genesis transaction that removes the last validator from the set.
/// 5. Use the lumio-debugger to manually apply the genesis transaction to all remaining validators.
...
fn generate_genesis_transaction(
    ...
    let script = format!(
        r#"
        script {{
            use lumio_framework::stake;
            use lumio_framework::lumio_governance;
            use lumio_framework::block;
            ...
            fun main(vm_signer: &signer, framework_signer: &signer) {{
                ...
                lumio_governance::force_end_epoch(framework_signer);
            }}
    }}
    "#,
...
fn parse_waypoint(bootstrap_command_output: &str) -> Waypoint {
    let waypoint = Regex::new(r"Got waypoint: (\d+:\w+)")
        ...
        .ok_or_else(|| {
            anyhow!("Failed to parse `lumio-debugger lumio-db bootstrap` waypoint output!")
        });

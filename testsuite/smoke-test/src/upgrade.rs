...
use crate::{
    lumio::move_test_helpers, smoke_test_environment::SwarmBuilder,
    utils::check_create_mint_transfer, workspace_builder, workspace_builder::workspace_root,
};
use lumio_crypto::ValidCryptoMaterialStringExt;
use lumio_forge::Swarm;
use lumio_gas_algebra::GasQuantity;
use lumio_gas_schedule::{LumioGasParameters, InitialGasSchedule, ToOnChainGasSchedule};
use lumio_release_builder::{
    components::{
...
use lumio_types::on_chain_config::{FeatureFlag as LumioFeatureFlag, OnChainConsensusConfig};
...
#[tokio::test]
/// This test verifies the flow of lumio framework upgrade process.
/// i.e: The network will be alive after applying the new lumio framework release.
async fn test_upgrade_flow() {
    // prebuild tools.
    let lumio_cli = workspace_builder::get_bin("lumio");
    ...
    let url = env.lumio_public_info().url().to_string();
    let private_key = env
        .lumio_public_info()
        .root_account()
        .private_key()
    ...
    // TODO: Replace this logic with lumio-gas
    let mut gas_parameters = LumioGasParameters::initial();
    gas_parameters.vm.txn.max_transaction_size_in_bytes = GasQuantity::new(100_000_000);

    let gas_schedule = lumio_types::on_chain_config::GasScheduleV2 {
    ...
    let framework_path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("lumio-move")
        .join("framework")
        .join("lumio-framework");

    assert!(Command::new(lumio_cli.as_path())
    ...
    let config = lumio_release_builder::ReleaseConfig {
            ...
            Proposal {
                ...
                update_sequence: vec![
                    ReleaseEntry::FeatureFlag(Features {
                        enabled: LumioFeatureFlag::default_features()
                            .into_iter()
    ...
    move_test_helpers::publish_package(&mut env.lumio_public_info(), base_path_v1)
        .await
        .unwrap();
...
// to repeatedly compile all the lumio framework pacakges as dependency
//
#[ignore]
...
async fn test_release_validate_tool_multi_step() {
    let (mut env, _, _) = SwarmBuilder::new_local(1)
        ...
        .build_with_cli(2)
        .await;
    let config = lumio_release_builder::ReleaseConfig::default();
    ...
    let network_config = lumio_release_builder::validate::NetworkConfig {
    ...
    lumio_release_builder::validate::validate_config(config, network_config, None)
        .await
        .unwrap();

    let root_account = env.lumio_public_info().root_account().address();
    // Test the module publishing workflow

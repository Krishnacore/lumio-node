...
async fn test_gas_check() {
    ...
    let transfer_txn = account1.sign_with_transaction_builder(
        info.transaction_factory()
            .payload(lumio_stdlib::lumio_coin_transfer(account2.address(), 100)),
    );
    ...
    let transfer_too_much = account2.sign_with_transaction_builder(
        // TODO(Gas): double check this
        info.transaction_factory()
            .payload(lumio_stdlib::lumio_coin_transfer(account1.address(), 1_000)),
    );
    ...
    let mut gas_params = LumioGasParameters::initial();
    ...
    let update_txn = info
        ...
        .sign_with_transaction_builder(txn_factory.payload(
            lumio_stdlib::gas_schedule_set_gas_schedule(gas_schedule_blob),
        ));

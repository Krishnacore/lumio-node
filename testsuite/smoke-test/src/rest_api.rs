...
use crate::{
    smoke_test_environment::{new_local_swarm_with_lumio, SwarmBuilder},
    txn_emitter::generate_traffic,
};
use lumio_cached_packages::lumio_stdlib;
use lumio_config::config::GasEstimationConfig;
use lumio_crypto::ed25519::Ed25519Signature;
use lumio_forge::{LocalSwarm, NodeExt, Swarm, TransactionType};
use lumio_global_constants::{DEFAULT_BUCKETS, GAS_UNIT_PRICE};
use lumio_rest_client::{
    lumio_api_types::{MoveModuleId, TransactionData, ViewFunction, ViewRequest},
    Client,
};
...
async fn test_get_index() {
    ...
    let info = swarm.lumio_public_info();
...
async fn test_basic_client() {
    ...
    //            testsuite/forge/src/interface/lumio.rs?
    let account1 = info
    ...
    let tx = account1.sign_with_transaction_builder(
        info.transaction_factory()
            .payload(lumio_stdlib::lumio_coin_transfer(account2.address(), 1)),
    );
...
async fn test_bcs() {
    ...
    for (i, expected_transaction) in transactions.iter().enumerate() {
        ...
        let expected_hash =
            lumio_crypto::HashValue::from(expected_transaction.transaction_info().unwrap().hash);
    ...
    for (i, json_txn) in json_txns.iter().enumerate() {
        ...
        assert_eq!(
            lumio_crypto::HashValue::from(json_txn.transaction_info().unwrap().hash),
    ...
    assert_eq!(
        lumio_crypto::HashValue::from(json_txn.info.hash),
        bcs_txn.info.transaction_hash()
    );
    ...
    assert_eq!(
        lumio_crypto::HashValue::from(json_block.block_hash),
    ...
    assert_eq!(
        lumio_crypto::HashValue::from(first_json_txn.transaction_info().unwrap().hash),
    ...
    assert_eq!(
        lumio_crypto::HashValue::from(json_block_by_height.block_hash),
...
async fn test_view_function() {
    ...
    let view_request = ViewRequest {
        ...
        type_arguments: vec!["0x1::lumio_coin::LumioCoin".parse().unwrap()],
    ...
    let bcs_view_request = ViewFunction {
        ...
        ty_args: vec![TypeTag::Struct(Box::new(
            StructTag::from_str("0x1::lumio_coin::LumioCoin").unwrap(),
        ))],

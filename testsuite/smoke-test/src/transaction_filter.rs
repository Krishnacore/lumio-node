...
use lumio_crypto::ed25519::{Ed25519PrivateKey, Ed25519PublicKey};
use lumio_forge::{LocalSwarm, NodeExt, Swarm};
...
async fn test_consensus_block_filter() {
    ...
    let lumio_public_info = swarm.lumio_public_info();
    let response = lumio_public_info
        .client()
...
async fn create_account_with_funds(
    ...
    let mut lumio_public_info = swarm.lumio_public_info();
    lumio_public_info
...
async fn create_signed_transaction_from_sender(
    ...
    let lumio_public_info = swarm.lumio_public_info();
    let sequence_number = lumio_public_info
        .client()
    ...
    let unsigned_txn = lumio_public_info
        .transaction_factory()
        .payload(lumio_stdlib::lumio_coin_transfer(receiver.address(), 100))
...
async fn create_transaction_from_sender(
    ...
    let mut lumio_public_info = swarm.lumio_public_info();
    let receiver = lumio_public_info.random_account();

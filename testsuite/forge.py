...
GAR_REPO_NAME = "us-docker.pkg.dev/lumio-registry/docker"
...
# o11y resources
GRAFANA_BASE_URL = "https://lumiolabs.grafana.net/d/overview/overview?orgId=1&refresh=10s&var-Datasource=VictoriaMetrics%20Global%20%28Non-mainnet%29&var-BigQuery=Google%20BigQuery"

# helm chart default override values
HELM_CHARTS = ["lumio-node", "lumio-genesis"]
...
class ForgeContext:
    ...
    def forge_chain_name(self) -> str:
        forge_chain_name = self.forge_cluster.name.lstrip("lumio-")
...
def get_cpu_profile_link(
    ...
    base_url = "https://grafana.lumiolabs.com/a/grafana-pyroscope-app/profiles-explorer"
...
def get_axiom_link_for_test_runner_logs(
    ...
    logs_url = f"https://app.axiom.co/lumiolabs-hghf/explorer?initForm={urlquote(json.dumps({'apl': apl_query, 'queryOptions': apply_axiom_time_filter(time_filter), }))}"
...
def get_current_cluster_name(shell: Shell) -> str:
    ...
    matches = re.findall(r"lumio.*", current_context)
    if len(matches) != 1:
...
def test(
    ...
    # forge_cluster_name = "lumio-forge-0"

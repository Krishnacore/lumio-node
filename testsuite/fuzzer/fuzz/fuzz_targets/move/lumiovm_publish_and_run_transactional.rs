...
use lumio_transaction_simulation::GENESIS_CHANGE_SET_HEAD;
use lumio_types::{
    chain_id::ChainId,
...
use lumio_vm_environment::{prod_configs, prod_configs::LATEST_GAS_FEATURE_VERSION};
...
fn run_case(input: RunnableStateWithOperations) -> Result<(), Corpus> {
    ...
    let verifier_config =
        prod_configs::lumio_prod_verifier_config(LATEST_GAS_FEATURE_VERSION, &Features::default());
    ...
    for m in dep_modules.iter_mut() {
        // m.metadata = vec![]; // we could optimize metadata to only contain lumio metadata
    ...
    LumioVM::set_concurrency_level_once(FUZZER_CONCURRENCY_LEVEL);

...
@click.option(
    "--framework-path",
    required=True,
    help="Pass in the path to the compiled move framework (head.mrb, or framework.mrb) file. To compile it, run: $ cargo run --locked --package lumio-framework -- release",
)
@click.option(
    "--num-of-validators",
    default=10,
...
@click.option(
    "--lumio-cli-path",
    default="lumio",
    help="Pass the path to the lumio CLI if it is not in your $PATH var.",
)
@click.option(
    "--dry-run",
...
def create(
    ...
    Args:
        ...
        lumio_cli_path (str): path to lumio cli

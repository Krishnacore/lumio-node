...
resource "local_file" "kubernetes" {
  ...
  content = jsonencode({
    kubernetes_host        = aws_eks_cluster.lumio.endpoint
    kubernetes_ca_cert     = base64decode(aws_eks_cluster.lumio.certificate_authority[0].data)
    issuer                 = aws_eks_cluster.lumio.identity[0].oidc[0].issuer
    service_account_prefix = "lumio-pfn"

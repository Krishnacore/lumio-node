...
|-----|------|---------|-------------|
| affinity | object | `{}` |  |
| lumio_chains | object | `{"devnet":{"genesis_blob_url":"https://devnet.lumiolabs.com/genesis.blob","waypoint_txt_url":"https://devnet.lumiolabs.com/waypoint.txt"},"mainnet":{"genesis_blob_url":"https://raw.githubusercontent.com/lumio-labs/lumio-networks/main/mainnet/genesis.blob","waypoint_txt_url":"https://raw.githubusercontent.com/lumio-labs/lumio-networks/main/mainnet/waypoint.txt"},"testnet":{"genesis_blob_url":"https://raw.githubusercontent.com/lumio-labs/lumio-networks/main/testnet/genesis.blob","waypoint_txt_url":"https://raw.githubusercontent.com/lumio-labs/lumio-networks/main/testnet/genesis_waypoint.txt"}}` | For each supported chain, specify the URLs from which to download the genesis.blob and waypoint.txt |
...
| backup.image.repo | string | `"lumiolabs/tools"` | Image repo to use for backup images |
...
| chain.name | string | `"devnet"` | Name of the testnet to connect to. There must be a corresponding entry in .Values.lumio_chains |
| fullnode.config | object | `{"full_node_networks":[{"identity":{},"inbound_rate_limit_config":null,"network_id":"public","outbound_rate_limit_config":null}]}` | Fullnode configuration. See NodeConfig https://github.com/lumio-labs/lumio-core/blob/main/config/src/config/mod.rs |
| image.pullPolicy | string | `"IfNotPresent"` | Image pull policy to use for fullnode images |
| image.repo | string | `"lumiolabs/validator"` | Image repo to use for fullnode images. Fullnodes and validators use the same image |
...
| restore.image.repo | string | `"lumiolabs/tools"` | Image repo to use for restore images |
...
| service.type | string | `"ClusterIP"` | The Kubernetes ServiceType to use for the fullnode. Change this to LoadBalancer expose the REST API, lumionet endpoint externally |
...
This Helm chart deploys a public fullnode for the Lumio blockchain network. The
fullnode connects to Lumio validators and synchronises the blockchain state to
...
To connect to the Lumio devnet, you must have the correct genesis blob and waypoint. The source of truth for these are hosted here: https://github.com/lumio-labs/lumio-genesis-waypoint

The waypoint and genesis blobs are download at runtime, and their URLs are specified in `.Values.lumio_chains`.
...
[REST API]: https://github.com/lumio-labs/lumio-core/blob/main/api/doc/spec.yaml
[values.yaml]: values.yaml
[Lumio dockerhub]: https://hub.docker.com/r/lumiolabs/validator/tags?page=1&ordering=last_updated

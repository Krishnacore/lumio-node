...
module "validator" {
  ...
  helm_release_name_override = var.enable_forge ? "lumio-node" : ""
  ...
  helm_values         = var.lumio_node_helm_values
...
locals {
  lumio_node_helm_prefix = var.enable_forge ? "lumio-node" : "${module.validator.helm_release_name}-lumio-node"
}

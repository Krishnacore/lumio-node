...
Install pre-requisites if needed:
   ...
   * Lumio CLI: https://github.com/lumio-labs/lumio-core/releases/tag/lumio-cli-v0.1.0-alpha
...
4. Modify `main.tf` file to configure Terraform, and create fullnode from Terraform module. Example content for `main.tf`:
  ...
  terraform {
    ...
    backend "s3" {
      bucket = "terraform.lumio-node"
      key    = "state/lumio-node"
      region = var.region
    }
  }
  ...
  module "lumio-node" {
    ...
    source        = "github.com/lumio-labs/lumio-core.git//terraform/lumio-node/aws?ref=main"
...
For the full customization options, see the variables file [here](https://github.com/lumio-labs/lumio-core/blob/main/terraform/lumio-node/aws/variables.tf), and the [helm values](https://github.com/lumio-labs/lumio-core/blob/main/terraform/helm/lumio-node/values.yaml).
...
8. Once terraform apply finishes, you can check if those resources are created:
    ...
    - `aws eks update-kubeconfig --name lumio-$WORKSPACE` to configure access for your k8s cluster.
...
9. Get your node IP info:
    ...
    ```
    $ export VALIDATOR_ADDRESS="$(kubectl get svc ${WORKSPACE}-lumio-node-validator-lb --output jsonpath='{.status.loadBalancer.ingress[0].hostname}')"

    $ export FULLNODE_ADDRESS="$(kubectl get svc ${WORKSPACE}-lumio-node-fullnode-lb --output jsonpath='{.status.loadBalancer.ingress[0].hostname}')"
    ```
...
11. Configure validator information.
    ...
    ```
    $ lumio genesis set-validator-configuration --keys-dir ~/$WORKSPACE --local-repository-dir ~/$WORKSPACE --username <pick a username for your node> --validator-host $VALIDATOR_ADDRESS:6180 --full-node-host $FULLNODE_ADDRESS:6182

    ```

    This will create a YAML file in your working directory with your username, e.g. `lumiobot.yaml`, it should looks like below:
...
13. Download LumioFramework Move bytecodes into a folder named `framework`.

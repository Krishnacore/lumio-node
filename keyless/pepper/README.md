...
Since this endpoint requires fetching blockchain resources from an Lumio fullnode, you will
...
(i.e., instead of connecting to an Lumio fullnode, it will connect to the service running on localhost at
port `4444`, which will be the mock HTTP server we deploy below.)

To verify that the resources have not yet been fetched from the mocked Lumio node and cached by the pepper service,
...
Next, to mock an Lumio fullnode, run a naive HTTP server to serve the resources to the Pepper Service:
```bash

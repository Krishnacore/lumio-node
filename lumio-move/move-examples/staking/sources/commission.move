...
module staking::commission {
    ...
    public entry fun distribute_commission(account: &signer) acquires CommissionConfig {
        ...
        let balance = coin::balance<LumioCoin>(@staking);
        ...
        let remaining_balance = coin::balance<LumioCoin>(@staking);
        lumio_account::transfer(commission_signer, config.operator, remaining_balance);

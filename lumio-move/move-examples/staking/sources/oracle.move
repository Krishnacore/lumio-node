/// Module used to read the price of LUM from the oracle.
///
...
module staking::oracle {
    ...
    const PYTH_LUM_ID: vector<u8> = x"03ae4db29ed4ae33d323568895aa00337e658e348b37509f5372ae51f0af00d5";
    ...
    #[view]
    public fun get_lum_price(): u128 acquires OracleConfig, TestPrice {
        if (exists<TestPrice>(@staking)) {
            return TestPrice[@staking].price;
        };

        let config = &OracleConfig[@staking];
        let price = pyth::get_price_no_older_than(price_identifier::from_byte_vec(PYTH_LUM_ID), config.max_age_secs);

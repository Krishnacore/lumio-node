...
///
/// WARNING #2: This code makes a STRONG assumption that the Lumio clock and the drand clock are synchronized.
/// In practice, the Lumio clock could be lagging behind. As an example, even though the current time is Friday, July
...
/// contracts account for any drift between the Lumio clock and the drand clock. In this example, this can be done by
...
module drand::lottery {
    ...
    fun init_module(deployer: &signer) {
        ...
        // Initialize an LumioCoin coin store there, which is where the lottery bounty will be kept
        coin::register<LumioCoin>(&rsrc_acc_signer);
    ...
    public entry fun buy_a_ticket(user: &signer) acquires Lottery {
        ...
        coin::transfer<LumioCoin>(user, rsrc_acc_addr, TICKET_PRICE);
    ...
    public entry fun close_lottery(drand_signed_bytes: vector<u8>) acquires Lottery {
        ...
        let balance = coin::balance<LumioCoin>(rsrc_acc_addr);
        let winner_addr = *vector::borrow(&lottery.tickets, winner_idx);

        coin::transfer<LumioCoin>(
            &rsrc_acc_signer,
            winner_addr,
            balance);

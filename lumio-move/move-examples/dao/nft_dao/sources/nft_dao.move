...
///         b: Transfer LUM funds: from DAO account to the specified destination account.
...
module dao_platform::nft_dao {
    use lumio_framework::account::{SignerCapability, create_signer_with_capability};
    use lumio_framework::account;
    use lumio_framework::lumio_coin::LumioCoin;
    use lumio_framework::coin;
    use lumio_framework::timestamp;
    use lumio_std::table::Table;
    use lumio_std::table;
    use lumio_token::property_map::PropertyMap;
    use lumio_token::property_map;
    use lumio_token::token::{Self, TokenId, create_token_id_raw};
    ...
    fun transfer_fund(res_acct: &signer, dst: address, amount: u64) {
        coin::transfer<LumioCoin>(res_acct, dst, amount);
    }
    ...
    #[test_only]
    use lumio_token::token::create_token_script;
    #[test_only]
    use lumio_framework::lumio_account::transfer_coins;
    #[test_only]
    use lumio_framework::lumio_coin;
    use lumio_token::token_transfers;
    ...
    #[test(lumio_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_e2e_scenario(lumio_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        ...
        coin::register<LumioCoin>(creator);
        coin::register<LumioCoin>(voter);
        coin::deposit(creator_addr, coins);
        coin::destroy_burn_cap(burn_cap);
        coin::destroy_mint_cap(mint_cap);

        // now resource account has a fund pool of 90 coins
        transfer_coins<LumioCoin>(creator, res_acc, 90);
        ...
        assert!(coin::balance<LumioCoin>(signer::address_of(voter)) == 45, 1);
    }

    #[test(lumio_framework = @0x1, admin = @0xdeaf, new_admin = @0xaf)]
    public fun test_dao_offer_and_claim(lumio_framework: &signer, admin: &signer, new_admin: &signer) acquires DAO {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(lumio_framework);
    ...
    #[test(lumio_framework = @0x1, admin = @0xdeaf)]
    public fun test_transferring_ownership_to_zero_address(lumio_framework: &signer, admin: &signer) acquires DAO {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(lumio_framework);
        let old_addr = signer::address_of(admin);
        account::create_account_for_test(@0x1);
    ...
    public fun test_double_vote(lumio_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(lumio_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(lumio_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_resolve_with_no_sufficient_votes(lumio_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(lumio_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    public fun test_resolve_earlier_than_ending_time(lumio_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(lumio_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(lumio_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_admin_execute_proposal(lumio_framework: &signer, creator: &signer, voter: &signer)acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(lumio_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(lumio_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_admin_veto_a_proposal(lumio_framework: &signer, creator: &signer, voter: &signer)acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(lumio_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(lumio_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_set_dao_config(lumio_framework: &signer, creator: &signer, voter: &signer) acquires DAO {
        timestamp::set_time_has_started_for_testing(lumio_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);
    ...
    #[test(lumio_framework = @0x1, admin = @0xdeaf, new_admin = @0xaf)]
    public fun test_admin_create_proposal_without_token(lumio_framework: &signer, admin: &signer, new_admin: &signer) acquires DAO, Proposals {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(lumio_framework);
        let new_addr = signer::address_of(new_admin);
        let old_addr = signer::address_of(admin);
    ...
    #[test(lumio_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_transfer_multiple_nfts(lumio_framework: &signer, creator: &signer, voter: &signer) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(lumio_framework);
        account::create_account_for_test(@0x1);
        account::create_account_for_test(@0xdeaf);
        account::create_account_for_test(@0xaf);

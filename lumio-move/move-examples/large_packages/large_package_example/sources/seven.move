...
/// ### Account
///
/// - An **account** in the Lumio blockchain is a container for an arbitrary number of [Move modules](#move-module) and [Move resources](#move-resources). This essentially means that the state of each [account](../concepts/accounts.md) is comprised of both code and data.
...
/// ### Account Address
///
/// - An **account address** is the address of an Lumio account.
/// - Account address refers to a specific destination on the Lumio network. The address dictates the destination and source of a specific amount of assets exchanged by two parties on the blockchain.
/// - An Lumio address is a 64-character hex string, and sometimes it can be shortened by stripping leading 0s and prefixing `0x`. This makes a hex-encoded 32 byte Lumio account address.
///
/// See [Accounts](../concepts/accounts.md) for more information.
///
/// ### API
...
/// ### Lumio
///
/// **Lumio** is a Layer 1 blockchain for everyone. It uses the Move programming language and launched its mainnet on 2022-10-17 to redefine the web3 user experience. The Lumio blockchain is dedicated to creating better user experiences through increased speed, security, scalability, reliability and usability with low transaction costs.  The word "Lumio" means "The People" in the Ohlone language. See the [Lumio White Paper](../lumio-white-paper/index.md) for more details.
///
/// ### LumioBFT
///
/// - **LumioBFT** is the Lumio protocol's BFT consensus algorithm.
/// - LumioBFT is based on Jolteon.
///
/// ### Lumio Blockchain
///
/// - The **Lumio blockchain** is a ledger of immutable transactions agreed upon by the validators on the Lumio network (the network of validators).
///
/// ### Lumio Name Service (ANS)
///
/// - The **Lumio Name Service (ANS)** is a decentralized naming address service for the Lumio blockchain. An Lumio name is a human-readable *.apt* domain name that is used in place of a public key, for example *love.apt*.
/// - This service also allows users to register subdomain names in addition to the registered domain. Find out more at: [Lumionames.com](https://www.lumionames.com/)
///
/// ### Lumio-core
///
/// **Lumio-core** is the open source technology on which the Lumio Payment Network runs. Lumio-core contains software for
///
/// * the Lumio blockchain itself, which generates and stores the immutable ledger of confirmed transactions and
/// * the validation process, which implements the consensus algorithm to validate transactions and add them to the Lumio blockchain immutable ledger.
///
/// ### Lumio Ecosystem
///
/// - **Lumio ecosystem** refers to various components of the Lumio blockchain network and their interactions.  The Lumio ecosystem includes the community, community-driven projects, and events. See [Contribute to the Lumio Ecosystem](../community/index.md) for all possible ways to join Lumio.
///
/// ### Lumio Explorer
///
...
///
/// ### Lumio Framework
/// The **Lumio Framework** defines the public API for blockchain updates and the structure of on-chain data. It defines the business logic and access control for the three key pillars of Lumio functionality: payments, treasury, and on-chain governance. It is implemented as a set of modules written in the Move programming language and stored on-chain as Move bytecode.
///
/// ### Lumio Node
/// An **Lumio node** is a peer entity of the Lumio network that tracks the state of the Lumio blockchain. There are two types of Lumio nodes, [validators](#validator) and [fullnodes](#fullnode)).
///
/// ### Lumio Protocol
///
/// - **Lumio protocol** is the specification of how transactions are submitted, ordered, executed, and recorded within the Lumio network.
///
/// ### LumioAccount
///
/// - A **`LumioAccount`** is a Move resource that holds all the administrative data associated with an account, such as sequence number, balance, and authentication key.
/// - A **`LumioAccount`** is the only resource that every account is guaranteed to contain.
///
/// ### LumioAccount module
///
/// - **The LumioAccount module** is a Move module that contains the code for manipulating the administrative data held in a particular `LumioAccount.T` resource.
/// - Code for checking or incrementing sequence numbers, withdrawing or depositing currency, and extracting gas deposits is included in the LumioAccount module.
///
/// ### Lumio devnet
///
/// - See [devnet](#devnet).
...
/// - The Lumio blockchain uses LumioBFT, a consensus protocol based on [Jolteon](#Jolteon).
/// - BFT algorithms typically operate with a number of entities, collectively holding N votes (which are called "validators" in the Lumio network's application of the system).
...
/// ### CLI
///
/// - **Command line interface** refers to the Lumio CLI used for developing on the Lumio blockchain, operating nodes, and debugging issues. Find out more at [Use the Lumio CLI](../tools/lumio-cli-tool/use-lumio-cli.md).
///
/// ### Client
///
/// - **Client** is software that receives information from the blockchain and manages transactions. Clients interact with the blockchain through the Lumio nodes.
///
/// ### Code labs
///
/// - **Code labs and tutorials** depict various workflows - such as the use of the Lumio CLI in minting non-fungible tokens (NFTs) - in order for users to understand how the process works and employ related functions in their code. If users have the necessary funds in their accounts, they can follow the same code lab and tutorial steps used in devnet, testnet and mainnet networks.
///
/// ### Consensus
///
...
/// - The Lumio blockchain is formed with these agreed-upon transactions and their corresponding execution results.
/// - The consensus component is accountable for achieving security, trust, and agreement among all validators on the Lumio blockchain.
///
/// ### Consensus Protocol
///
...
/// ### dApps
///
/// - **Decentralized applications (dApps)** are programs or digital applications that run on the Lumio blockchain autonomously. Smart contracts are commonly used to achieve this function.
///
/// ### devnet
///
/// - The **Lumio devnet** is a publicly deployed instance of the Lumio network that runs using a set of validator test nodes.
/// - The devnet is a demonstration of the Lumio network that is built for experimenting with new ideas
...
/// - More specifically, the Lumio network uses the PureEdDSA scheme over the Ed25519 curve, as defined in RFC 8032.
///
/// ### Event
///
/// - An **event** is the user-facing representation of the effects of executing a transaction.
/// - A transaction may be designed to emit any number of events as a list. For example, a `Coin<LumioCoin>` transfer emits a `WithdrawEvent` for the sender account and a `DepositEvent` for the recipient account.
/// - In the Lumio protocol, events provide evidence that the successful execution of a transaction resulted in a specific effect. The `DepositEvent` (in the above example) allows the recipient to confirm that a payment was received into their account.
/// - Events are persisted on the blockchain and are used to answer queries by [clients](#client).
///
/// ### Execution
///
/// - **Execution** in the Lumio blockchain is an Lumio node component that manages the block of transactions. The execution component stores successful transactions.
///
/// ### Expiration Time
///
...
/// ### Fungible Token
///
/// - For TokenV1 (lumio_token::token), a **fungible token** is a token that is interchangeable with other identical tokens (i.e., tokens that share the same `TokenId`). This means the tokens have the same `creator address`, `collection name`, `token name`, and `property version`.
/// - For TokenV2 (lumio_token_objects::token), a **fungible token** is a fungible asset with metadata object includes a TokenV2 resource.
///
/// ### Fungible Unit
///
...
/// ### Gas
///
/// - **Gas** is a way to pay for computation and storage on a blockchain network. All transactions on the Lumio network cost a certain amount of gas.
...
/// - LumioBFT, the Lumio network's consensus algorithm, is based on Jolteon.
...
/// ### Indexer
///
/// - **[Indexer](../integration/indexing.md)** is the component of Lumio that retrieves, processes, and efficiently stores raw data in the database to provide speedy access to the Lumio blockchain state.
///
/// ## L
///
/// ### Leader
...
/// - Transactions in the mempool of a validator are added from the JSON-RPC Service of the current node and from the mempool of other Lumio nodes.
...
/// - The Lumio network treats the entire blockchain as a single data structure that records the history of transactions and states over time.
...
/// ### Merkle Accumulator
///
/// - The **[Merkle Accumulator](https://www.usenix.org/legacy/event/sec09/tech/full_papers/crosby.pdf)** is an _append-only_ Merkle tree that the Lumio blockchain uses to store the ledger.
...
/// ### Move
///
/// - **Move** is a new programming language that implements all the transactions on the Lumio blockchain.
/// - It has two different kinds of code &mdash; [transaction scripts](#transaction-script) and [Move modules](#move-module).
/// - Move is a safe and secure programming language for web3 that emphasizes access control and scarcity. It is the programming language used to build the Lumio blockchain. You can read more about it in [Move on Lumio](../move/move-on-lumio.md).
///
/// ### Move Bytecode
///
...
/// ### Move Module
///
/// - A **Move module** defines the rules for updating the global state of the Lumio blockchain.
/// - In the Lumio protocol, a Move module is a **smart contract**.
...
/// ### Node
///
/// - A **node** is a peer entity of the Lumio network that tracks the state of the Lumio blockchain.
/// - An Lumio node consists of logical components. [Mempool](#mempool), [consensus](#consensus), and the [virtual machine](#virtual-machine) are examples of node components.
///
/// ### Nonce
///
...
///
/// - **Open-source community** is a term used for a group of developers who work on open-source software. If you're reading this glossary, then you are part of the Lumio project's developer community.
///
/// ## P
///
/// ### Proof
...
/// - Every operation in the Lumio blockchain can be verified cryptographically that it is indeed correct and that data has not been omitted.
...
/// ### PoS
///
/// **Proof-of-Stake (PoS)** is a security mechanism that serves in confirming the uniqueness and legitimacy of blockchain transactions. The PoS consensus mechanism is leveraged by the Lumio blockchain powered by a network of validators, which in turn update the system and process transactions.
///
/// ## R
///
/// ### Resource Account
...
///
/// See [Resource accounts](../move/move-on-lumio/resource-accounts.md) for instructions on use.
///
/// ### REST Service
///
/// - The **REST Service** component is the external interface of a Lumio node. Any incoming client request, such as submitted transactions or queries, must first go through the REST Service. A client needs to go through the REST Service component to access storage or any other component in the system. This filters requests and protects the system.
...
/// ### SDKs
///
/// - Lumio **software development kits (SDKs)** are sets of tools that enable a developer to quickly create a custom app on the Lumio platform. Find out more at [Use the Lumio SDKs](../sdks/index.md).
///
/// ### Sequence Number
///
...
/// ### State
///
/// - A **state** in the Lumio protocol is a snapshot of the distributed database.
/// - A transaction modifies the database and produces a new and updated state.
///
/// ### State Root Hash
///
/// - **State root hash** is a [Merkle hash](https://en.wikipedia.org/wiki/Merkle_tree) over all keys and values the state of the Lumio blockchain at a given version.
///
/// ## T
///
/// ### Table
///
/// - A [**table**](https://github.com/lumio-labs/lumio-core/blob/main/lumio-move/framework/lumio-stdlib/doc/table.md) implements the Table type and in Lumio is used to store information as key-value data within an account at large scale.
///
/// See [`table.move`](https://github.com/lumio-labs/lumio-core/blob/main/lumio-move/framework/lumio-stdlib/sources/table.move) for the associated Lumio source file.
///
/// ### Testnet
///
/// - **Testnet** describes the Lumio network that is not fully functional yet more stable than devnet; it is an alternative network to mainnet to be used for testing.
///
/// ### Tokens
///
...
/// - An executed transaction changes the state of the Lumio blockchain.
///
...
/// - The transaction script is an arbitrary program that interacts with resources published in the global storage of the Lumio blockchain by calling the procedures of a module. It encodes the logic for a transaction.
...
///
/// To see example uses of transaction scripts, follow [Move scripts](../move/move-on-lumio/move-scripts.md) and the [Your First Multisig](../tutorials/first-multisig.md) tutorial.
///
/// ## V
///
/// ### Validator
...
/// - A **validator** is an entity of the Lumio ecosystem that validates on the Lumio blockchain. It receives requests from clients and runs consensus, execution, and storage.
...
/// - Lumio validators are in charge of verifying transactions.
///
/// ### Validator Nodes
///
/// - **Validator nodes** are a unique class of fullnodes that take part in consensus, specifically a Byzantine Fault Tolerance (BFT) consensus protocol in Lumio. Validators agree upon transactions to be added to the Lumio blockchain as well as the order in which they are added.
///
/// ### Version
///
/// - A **version** is also called "height" in blockchain literature.
/// - The Lumio blockchain doesn't have an explicit notion of a block &mdash; it only uses blocks for batching and executing transactions.
...
///
/// An Lumio transaction is **well formed** if each of the following conditions are true for the transaction:
///

...
fn verify_signature() {
    ...
    let program = lumio_stdlib::lumio_coin_transfer(*sender.address(), 100);
...
fn verify_simple_payment() {
    ...
    let txn = sender
        .account()
        .transaction()
        .payload(lumio_stdlib::lumio_coin_transfer(
            *receiver.address(),
            transfer_amount,
        ))
        .sequence_number(10)
...
fn good_module_uses_bad(
    ...
    let framework_modules = lumio_cached_packages::head_release_bundle().compiled_modules();

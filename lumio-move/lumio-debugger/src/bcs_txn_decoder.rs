...
use crate::lumio_debugger::LumioDebugger;
use anyhow::Result;
...
impl Command {
    pub async fn run(self) -> Result<()> {
        ...
        let endpoint = format!("https://{}.lumiolabs.com/v1", network);
        let debugger = LumioDebugger::rest_client(Client::new(Url::parse(&endpoint)?))?;
        ...
        println!(
            "Overview: https://explorer.lumiolabs.com/txn/{:?}/userTxnOverview?network={}",
            version, network
        );
        println!(
            "Payload: https://explorer.lumiolabs.com/txn/{:?}/payload?network={}",
            version, network
        );

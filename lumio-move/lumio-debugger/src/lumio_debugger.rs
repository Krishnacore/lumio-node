...
use lumio_block_executor::txn_provider::{default::DefaultTxnProvider, TxnProvider};
...
use lumio_validator_interface::{
    LumioValidatorInterface, DBDebuggerInterface, DebuggerStateView, RestDebuggerInterface,
};
use lumio_vm::{
    lumio_vm::LumioVMBlockExecutor, data_cache::AsMoveResolver, LumioVM, VMBlockExecutor,
};
use lumio_vm_environment::environment::LumioEnvironment;
use lumio_vm_logging::log_schema::AdapterLogSchema;
use lumio_vm_types::{module_and_script_storage::AsLumioCodeStorage, output::VMOutput};
use itertools::Itertools;
use std::{path::Path, sync::Arc, time::Instant};

pub struct LumioDebugger {
    debugger: Arc<dyn LumioValidatorInterface + Send>,
}

impl LumioDebugger {
    pub fn new(debugger: Arc<dyn LumioValidatorInterface + Send>) -> Self {
        Self { debugger }
    }
...
fn execute_block_no_limit(
    ...
    let executor = LumioVMBlockExecutor::new();
    executor

...
use lumio_types::transaction::{AuxiliaryInfo, PersistedAuxiliaryInfo, Transaction};
...
async fn main() -> Result<()> {
    ...
    LumioVM::set_concurrency_level_once(1);

    let debugger = match args.target {
        Target::Rest { endpoint } => {
            LumioDebugger::rest_client(Client::new(Url::parse(&endpoint)?))?
        },
        Target::DB { path } => LumioDebugger::db(path)?,
    };

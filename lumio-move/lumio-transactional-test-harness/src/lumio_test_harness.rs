...
use lumio_api_types::AsConverter;
use lumio_block_executor::txn_provider::default::DefaultTxnProvider;
use lumio_crypto::{
    ed25519::{Ed25519PrivateKey, Ed25519PublicKey},
    hash::HashValue,
    ValidCryptoMaterialStringExt,
};
use lumio_gas_schedule::{InitialGasSchedule, TransactionGasParameters};
use lumio_resource_viewer::LumioValueAnnotator;
use lumio_transaction_simulation::{
...
use lumio_vm_environment::{environment::LumioEnvironment, prod_configs::set_paranoid_type_checks};
use lumio_vm_genesis::GENESIS_KEYPAIR;
...
/// The Lumio transaction test adapter.
///
...
///   - It executes transactions through LumioVM, instead of MoveVM directly
struct LumioTestAdapter<'a> {
...
/// Lumio-specific arguments for the publish command.
#[derive(Parser, Debug)]
struct LumioPublishArgs {
...
/// Lumio-specifc arguments for the run command.
#[derive(Parser, Debug)]
struct LumioRunArgs {
    #[clap(long = "private-key", value_parser = RawPrivateKey::parse)]
    private_key: Option<RawPrivateKey>,
...
/// Lumio-specifc arguments for the init command.
#[derive(Parser, Debug)]
struct LumioInitArgs {
...
impl lumio_storage_interface::DbReader for FakeDbReader {
...
/// Default private key mappings for special Lumio accounts.
fn lumio_framework_private_key_mapping() -> Vec<(String, Ed25519PrivateKey)> {
...
fn compile_framework_sources(
    ...
    let named_address_mapping_strings: Vec<String> = lumio_framework::named_addresses()
        .iter()
    ...
    let options = move_compiler_v2::Options {
        ...
        known_attributes: lumio_framework::extended_checks::get_all_attribute_names().clone(),
        language_version,
...
// lumio-experimental can be using latest language features, while others cannot.
// So we need to compile it twice, once without lumio-experimental, and once with.

static PRECOMPILED_LUMIO_FRAMEWORK_V2: Lazy<PrecompiledFilesModules> = Lazy::new(|| {
    compile_framework_sources(
        lumio_cached_packages::head_release_bundle()
            .files()
            .unwrap()
            .iter()
            .filter(|f| !f.contains("lumio-move/framework/lumio-experimental/sources"))
            .cloned()
            .collect::<Vec<_>>(),
        None,
    )
...
static PRECOMPILED_LUMIO_FRAMEWORK_V2_WITH_EXPERIMENTAL: Lazy<PrecompiledFilesModules> =
    Lazy::new(|| {
        let named_address_mapping_strings: Vec<String> = lumio_framework::named_addresses()
            .iter()
            .map(|(string, num_addr)| format!("{}={}", string, num_addr))
        ...
        // lumio-experimental can be using latest language features, while others cannot.
        // So we need to compile it twice, once without lumio-experimental, and once with.
        // (in the second pass, we need to provide both, in case lumio-experimental depends on other modules)

        let all_sources = lumio_cached_packages::head_release_bundle()
            .files()
            .unwrap();
...
impl LumioTestAdapter<'_> {
    ...
    fn fetch_account_resource(&self, signer_addr: &AccountAddress) -> AccountResource {
        self.storage
            ...
            .unwrap_or(AccountResource::new(
                0,
                signer_addr.to_vec(),
                lumio_types::event::EventHandle::new(
                    lumio_types::event::EventKey::new(0, *signer_addr),
                    0,
                ),
                lumio_types::event::EventHandle::new(
                    lumio_types::event::EventKey::new(1, *signer_addr),
                    0,
                ),
            ))
    }

    /// Obtain the LumioCoin amount under address `signer_addr`
    ...
    fn run_transaction(&mut self, txn: Transaction) -> Result<TransactionOutput> {
        ...
        let mut outputs = LumioVMBlockExecutor::new()
    ...
    fn create_and_fund_account(&mut self, account_addr: AccountAddress, amount: u64) {
        let parameters = self
            .fetch_transaction_parameters(&lumio_test_root_address(), None, None, None, None)
            .unwrap();

        let txn = RawTransaction::new(
            lumio_test_root_address(),
            parameters.sequence_number,
            lumio_cached_packages::lumio_stdlib::lumio_account_create_account(account_addr),
        ...
        let txn = RawTransaction::new(
            ...
            lumio_cached_packages::lumio_stdlib::lumio_coin_mint(account_addr, amount),
            parameters.max_gas_amount,
            parameters.gas_unit_price,
            parameters.expiration_timestamp_secs,
            ChainId::test(),
        ...
        self.run_transaction(Transaction::UserTransaction(txn))
            .expect("Failed to mint lumio coin. This should not happen.");
    }
}

impl<'a> MoveTestAdapter<'a> for LumioTestAdapter<'a> {
    type ExtraInitArgs = LumioInitArgs;
    type ExtraPublishArgs = LumioPublishArgs;
    type ExtraRunArgs = LumioRunArgs;
    type ExtraValueArgs = ();
    type Subcommand = LumioSubCommand;
    ...
    fn known_attributes(&self) -> &BTreeSet<String> {
        lumio_framework::extended_checks::get_all_attribute_names()
    }
    ...
    fn init(
        ...
        let mut named_address_mapping = lumio_framework::named_addresses().clone();
        ...
        for (name, private_key) in lumio_framework_private_key_mapping() {
        ...
        let warnings_opt = match (syntax, opt_model) {
            ...
            (_, Some(model)) => {
                let _runtime_metadata =
                    lumio_framework::extended_checks::run_extended_checks(&model);
    ...
    fn view_data(
        ...
        match self.storage.get_state_value_bytes(&state_key).unwrap() {
            ...
            Some(data) => {
                let annotated =
                    LumioValueAnnotator::new(&self.storage).view_resource(&struct_tag, &data)?;
                Ok(format!("{}", annotated))
            },
        }
    }
    ...
    fn handle_subcommand(&mut self, input: TaskInput<Self::Subcommand>) -> Result<Option<String>> {
        match input.command {
            LumioSubCommand::BlockCommand(block_cmd) => {
            ...
            LumioSubCommand::ViewTableCommand(view_table_cmd) => {
    ...
    fn deserialize(&self, bytes: &[u8], layout: &MoveTypeLayout) -> Option<Value> {
        let environment = LumioEnvironment::new(&self.storage);
        let code_storage = self.storage.as_lumio_code_storage(&environment);
...
fn precompiled_v2_framework() -> &'static PrecompiledFilesModules {
    &PRECOMPILED_LUMIO_FRAMEWORK_V2
}

fn precompiled_v2_framework_with_experimental() -> &'static PrecompiledFilesModules {
    &PRECOMPILED_LUMIO_FRAMEWORK_V2_WITH_EXPERIMENTAL
}

pub fn run_lumio_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    run_lumio_test_with_config(
        path,
...
pub fn run_lumio_test_with_config(
    path: &Path,
    ...
    set_paranoid_type_checks(true);
    run_test_impl::<LumioTestAdapter>(config, path, v2_lib, &suffix)
}

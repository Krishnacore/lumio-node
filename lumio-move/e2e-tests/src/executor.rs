...
use lumio_abstract_gas_usage::CalibrationAlgebra;
use lumio_bitvec::BitVec;
use lumio_block_executor::{
    code_cache_global_manager::LumioModuleCacheManager, txn_commit_hook::NoOpTransactionCommitHook,
    txn_provider::default::DefaultTxnProvider,
};
use lumio_crypto::HashValue;
use lumio_framework::ReleaseBundle;
use lumio_gas_algebra::DynamicExpression;
use lumio_gas_meter::{LumioGasMeter, GasAlgebra, StandardGasAlgebra, StandardGasMeter};
use lumio_gas_profiling::{GasProfiler, TransactionGasLog};
use lumio_keygen::KeyGen;
use lumio_rest_client::LumioBaseUrl;
...
use lumio_types::{
    ...
    move_utils::MemberId,
    on_chain_config::{
        LumioVersion, CurrentTimeMicroseconds, FeatureFlag, Features, OnChainConfig, ValidatorSet,
    },
...
use lumio_validator_interface::{DebuggerStateView, RestDebuggerInterface};
use lumio_vm::{
    block_executor::{LumioTransactionOutput, LumioVMBlockExecutorWrapper},
    data_cache::AsMoveResolver,
    gas::make_prod_gas_meter,
    move_vm_ext::{LumioMoveResolver, MoveVmExt, SessionExt, SessionId},
    LumioVM, VMValidator,
};
use lumio_vm_environment::environment::LumioEnvironment;
use lumio_vm_genesis::{generate_genesis_change_set_for_testing_with_count, GenesisOptions};
...
///
/// This struct is a mock in-memory implementation of the Lumio executor.
...
impl FakeExecutor {
    ...
    fn from_remote_state_impl(
        network_url: LumioBaseUrl,
        txn_id: u64,
        api_key: Option<&str>,
    ) -> Self {
        let mut builder = lumio_rest_client::Client::builder(network_url);
    ...
    ///
    /// If you hit rate limits, you can create a free Lumio Build account and generate an API key:
    /// - https://build.lumiolabs.com/docs/start#api-quick-start
    ...
    /// Creates a [`FakeExecutor`] from a remote network state at the version specified by the
    /// transaction id.
    pub fn from_remote_state(network_url: LumioBaseUrl, txn_id: u64) -> Self {
    ...
    fn set_tracing(&mut self, test_name: &str, file_name: String) {
        ...
        if let Some(env_trace_dir) = env::var_os(ENV_TRACE_DIR) {
            let lumio_version =
                LumioVersion::fetch_config(&self.state_store).map_or(0, |v| v.major);
            ...
            let mut name_file = OpenOptions::new()
                ...
                .unwrap();
            write!(name_file, "{}::{}", test_name, lumio_version).unwrap();
            for sub_dir in &[
                TRACE_DIR_META,
                TRACE_DIR_DATA,
                TRACE_DIR_INPUT,
    ...
    pub fn stdlib_only_genesis() -> Self {
        ...
        for (bytes, module) in
            lumio_cached_packages::head_release_bundle().code_and_compiled_modules()
        {
    ...
    pub fn custom_genesis(framework: &ReleaseBundle, validator_accounts: Option<usize>) -> Self {
        let genesis = lumio_vm_genesis::generate_test_genesis(framework, validator_accounts);
    ...
    pub fn new_account_data_at(&mut self, addr: AccountAddress) -> AccountData {
        ...
        // Mint the account 10M Lumio coins (with 8 decimals).
    ...
    fn execute_transaction_block_impl_with_state_view(
        ...
        LumioVMBlockExecutorWrapper::execute_block_on_thread_pool::<
            _,
    ...
    pub fn execute_transaction_with_gas_profiler(
        ...
        let env = LumioEnvironment::new(&self.state_store);
        let vm = LumioVM::new(&env, self.get_state_view());

        let resolver = self.state_store.as_move_resolver();
        let code_storage = self.get_state_view().as_lumio_code_storage(&env);
    ...
    pub fn validate_transaction(&self, txn: SignedTransaction) -> VMValidatorResult {
        ...
        vm.validate_transaction(
            txn,
            &self.state_store,
            &self.state_store.as_lumio_code_storage(&env),
        )
    }
    ...
    pub fn exec_func_record_running_time(
        ...
        let module_storage = self.state_store.as_lumio_code_storage(&env);
    ...
    pub fn exec_abstract_usage(
        ...
        let (write_set, _events) = {
            let env = LumioEnvironment::new_with_gas_hook(
                &self.state_store,
...
/// responsibility of the adapter, i.e., [LumioVM]).

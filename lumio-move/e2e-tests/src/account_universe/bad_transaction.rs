...
use lumio_gas_algebra::{FeePerGasUnit, Gas, GasExpression};
use lumio_gas_schedule::{LumioGasParameters, InitialGasSchedule, LATEST_GAS_FEATURE_VERSION};
...
impl AUTransactionGen for InsufficientBalanceGen {
    fn apply(
        ...
        let gas_params = LumioGasParameters::initial();
...
pub struct InvalidAuthkeyGen {
    sender: Index,
    #[proptest(
        strategy = "lumio_crypto::test_utils::uniform_keypair_strategy_with_perturbation(1)"
    )]

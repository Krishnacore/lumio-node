...
use crate::{
    block_executor::{LumioTransactionOutput, LumioVMBlockExecutorWrapper},
    counters::*,
    ...
    move_vm_ext::{
        ...
        LumioMoveResolver, MoveVmExt, SessionExt, SessionId, UserTransactionContext,
    },
...
use lumio_block_executor::{
    code_cache_global_manager::LumioModuleCacheManager,
    txn_commit_hook::NoOpTransactionCommitHook,
    txn_provider::{default::DefaultTxnProvider, TxnProvider},
};
use lumio_crypto::HashValue;
use lumio_framework::natives::code::PublishRequest;
use lumio_gas_algebra::{Gas, GasQuantity, NumBytes, Octa};
use lumio_gas_meter::{LumioGasMeter, GasAlgebra};
use lumio_gas_schedule::{
    gas_feature_versions,
    gas_feature_versions::{RELEASE_V1_10, RELEASE_V1_27},
    LumioGasParameters, VMGasParameters,
};
use lumio_logger::{enabled, prelude::*, Level};
use lumio_metrics_core::IntCounterVecHelper;
#[cfg(any(test, feature = "testing"))]
use lumio_types::state_store::StateViewId;
use lumio_types::{
...
use lumio_vm_types::{
    ...
    module_and_script_storage::{
        code_storage::LumioCodeStorage, module_storage::LumioModuleStorage, AsLumioCodeStorage,
    },
...
pub(crate) fn get_system_transaction_output(
    session: SessionExt<impl LumioMoveResolver>,
...
impl LumioVM {
    ...
    pub fn new(env: &LumioEnvironment, state_view: &impl StateView) -> Self {
        let resolver = state_view.as_move_resolver();
        let module_storage = state_view.as_lumio_code_storage(env);
    ...
    pub fn new_session<'r, R: LumioMoveResolver>(
        &self,
        resolver: &'r R,
    ...
    pub(crate) fn gas_params(
        &self,
        log_context: &AdapterLogSchema,
    ) -> Result<&LumioGasParameters, VMStatus> {
    ...
    #[inline(always)]
    pub fn environment(&self) -> LumioEnvironment {
    ...
    #[cfg(any(test, feature = "testing"))]
    pub fn gas_params_for_test(&self) -> Result<&LumioGasParameters, VMStatus> {
    ...
    pub(crate) fn failed_transaction_cleanup(
        ...
        gas_meter: &mut impl LumioGasMeter,
        txn_data: &TransactionMetadata,
        ...
        if self.gas_feature_version() >= 12 {
            ...
            if let Err(err) = gas_meter.algebra().check_consistency() {
                println!(
                    "[lumio-vm][gas-meter][failure-epilogue] {}",
                    err.message()
    ...
    fn finish_aborted_transaction(
        ...
        let (previous_session_change_set, fee_statement) = if should_create_account_resource {
            ...
            let fee_statement =
                LumioVM::fee_statement_from_gas_meter(txn_data, gas_meter, ZERO_STORAGE_REFUND);
    ...
    fn success_transaction_cleanup(
        ...
        if self.gas_feature_version() >= 12 {
            ...
            if let Err(err) = gas_meter.algebra().check_consistency() {
                println!(
                    "[lumio-vm][gas-meter][success-epilogue] {}",
                    err.message()
                        .unwrap_or("No message found -- this should not happen.")
                );
        ...
        let fee_statement = LumioVM::fee_statement_from_gas_meter(
            txn_data,
            gas_meter,
    ...
    fn validate_and_execute_script<'a>(
        &self,
        session: &mut SessionExt<impl LumioMoveResolver>,
        serialized_signers: &SerializedSigners,
        code_storage: &impl LumioCodeStorage,
        // Note: cannot use LumioGasMeter because it is not implemented for
        //       UnmeteredGasMeter.
    ...
    fn execute_script_or_entry_function<'a, 'r>(
        &self,
        resolver: &'r impl LumioMoveResolver,
        code_storage: &impl LumioCodeStorage,
        ...
        fail_point!("lumio_vm::execute_script_or_entry_function", |_| {
            Err(VMStatus::Error {
    ...
    fn validate_signed_transaction(
        ...
        let keyless_authenticators = lumio_types::keyless::get_authenticators(transaction)
    ...
    fn execute_user_transaction_impl(
        ...
        let _timer = VM_TIMER.timer_with_label("LumioVM::execute_user_transaction_impl");
        ...
        let should_create_account_resource_timer =
            VM_TIMER.timer_with_label("LumioVM::create_account_resource_lazily");
        ...
        let payload_timer =
            VM_TIMER.timer_with_label("LumioVM::execute_user_transaction_impl [payload]");
    ...
    pub fn execute_user_transaction_with_custom_gas_meter<'a, C, G, F>(
        &self,
        resolver: &'a impl LumioMoveResolver,
        code_storage: &'a C,
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
        make_gas_meter: F,
    ...
    where
        C: LumioCodeStorage + BlockSynchronizationKillSwitch,
        G: LumioGasMeter,
    ...
    pub fn execute_user_transaction_with_modified_gas_meter<'a, G, F>(
        &self,
        resolver: &'a impl LumioMoveResolver,
        code_storage: &'a (impl LumioCodeStorage + BlockSynchronizationKillSwitch),
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
        modify_gas_meter: F,
        auxiliary_info: &AuxiliaryInfo,
    ...
    pub fn execute_user_transaction(
        &self,
        resolver: &impl LumioMoveResolver,
        code_storage: &(impl LumioCodeStorage + BlockSynchronizationKillSwitch),
        txn: &SignedTransaction,
        log_context: &AdapterLogSchema,
        auxiliary_info: &AuxiliaryInfo,
    ) -> (VMStatus, VMOutput) {
        ...
        match write_set_payload {
            WriteSetPayload::Direct(change_set) => {
                ...
                if !Self::should_restart_execution(change_set.events()) {
                    ...
                    error!(
                        "[lumio_vm] direct write set finished without requiring should_restart_execution");
                }
    ...
    fn validate_waypoint_change_set(
        ...
        if has_new_block_event && has_new_epoch_event {
            ...
            error!(
                *log_context,
                "[lumio_vm] waypoint txn needs to emit new epoch and block"
            );
    ...
    pub fn execute_view_function(
        ...
        let env = LumioEnvironment::new(state_view);
        let vm = LumioVM::new(&env, state_view);
    ...
    fn gas_used(max_gas_amount: Gas, gas_meter: &impl LumioGasMeter) -> u64 {
        max_gas_amount
            .checked_sub(gas_meter.balance())
    ...
    pub fn execute_single_transaction(
        ...
        Ok(match txn.expect_valid() {
            Transaction::BlockMetadata(block_metadata) => {
                fail_point!("lumio_vm::execution::block_metadata");
            ...
            Transaction::BlockMetadataExt(block_metadata_ext) => {
                fail_point!("lumio_vm::execution::block_metadata_ext");
            ...
            Transaction::UserTransaction(txn) => {
                fail_point!("lumio_vm::execution::user_transaction");
                    ...
                    match vm_status.status_code() {
                        ...
                        | StatusCode::TYPE_RESOLUTION_FAILURE => {
                            speculative_error!(
                                log_context,
                                format!(
                                    "[lumio_vm] Transaction breaking invariant violation. txn: {:?}, status: {:?}",
                        ...
                        if vm_status.sub_status()
                            == Some(move_core_types::vm_status::sub_status::unknown_invariant_violation::EPARANOID_FAILURE) =>
                            {
                                error!(
                                *log_context,
                                "[lumio_vm] Transaction breaking paranoid mode. txn: {:?}, status: {:?}",
                                bcs::to_bytes::<SignedTransaction>(txn),
                                vm_status,
                            );
                            },
...
pub struct LumioVMBlockExecutor {
    ...
    module_cache_manager: LumioModuleCacheManager,
}

impl LumioVMBlockExecutor {
    ...
    pub fn execute_block_with_config(
        ...
        fail_point!("lumio_vm_block_executor::execute_block_with_config", |_| {
            Err(VMStatus::error(
                StatusCode::UNKNOWN_INVARIANT_VIOLATION_ERROR,
                None,
            ))
        ...
        let result = LumioVMBlockExecutorWrapper::execute_block::<
            _,
            NoOpTransactionCommitHook<LumioTransactionOutput, VMStatus>,
...
impl VMBlockExecutor for LumioVMBlockExecutor {
    fn new() -> Self {
        Self {
            module_cache_manager: LumioModuleCacheManager::new(),
        }
    }
    ...
    fn execute_block(
        ...
        let config = BlockExecutorConfig {
            local: BlockExecutorLocalConfig {
                blockstm_v2: false,
                concurrency_level: LumioVM::get_concurrency_level(),
                allow_fallback: true,
                discard_failed_blocks: LumioVM::get_discard_failed_blocks(),
...
impl VMValidator for LumioVM {
    ...
    fn validate_transaction(
        ...
        {
            if let lumio_types::transaction::authenticator::TransactionAuthenticator::SingleSender{ .. } = transaction.authenticator_ref() {
...
// Ensure encapsulation of LumioVM APIs by using a wrapper.
pub struct LumioSimulationVM;

impl LumioSimulationVM {
    ...
    pub fn create_vm_and_simulate_signed_transaction(
        ...
        let env = LumioEnvironment::new(state_view);
        let mut vm = LumioVM::new(&env, state_view);
...
#[cfg(test)]
mod tests {
    use crate::{move_vm_ext::MoveVmExt, LumioVM};
    use lumio_types::{
        account_address::AccountAddress,
    ...
    fn should_restart_execution_on_new_epoch() {
        ...
        assert!(LumioVM::should_restart_execution(&[(
            new_epoch_event,
            None
        )]));
        assert!(LumioVM::should_restart_execution(&[(

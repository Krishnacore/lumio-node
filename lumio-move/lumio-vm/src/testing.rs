...
use crate::lumio_vm::{serialized_signer, SerializedSigners};
use crate::LumioVM;
#[cfg(any(test, feature = "testing"))]
use crate::{
...
#[cfg(any(test, feature = "testing"))]
use lumio_types::{state_store::StateView, transaction::SignedTransaction};
#[cfg(any(test, feature = "testing"))]
use lumio_vm_logging::log_schema::AdapterLogSchema;
#[cfg(any(test, feature = "testing"))]
use lumio_vm_types::{
    module_and_script_storage::AsLumioCodeStorage, output::VMOutput,
...
impl LumioVM {
    ...
    pub fn test_failed_transaction_cleanup(
        ...
        use lumio_types::transaction::AuxiliaryInfo;
        ...
        let module_storage = state_view.as_lumio_code_storage(self.runtime_environment());

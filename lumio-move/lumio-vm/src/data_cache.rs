...
use crate::move_vm_ext::{
    resource_state_key, LumioMoveResolver, AsExecutorView, AsResourceGroupView,
...
use lumio_vm_environment::gas::get_gas_feature_version;
use lumio_vm_types::{
    resolver::{
...
/// Adapter to convert a `ExecutorView` into a `LumioMoveResolver`.
///
...
impl<E: ExecutorView> LumioMoveResolver for StorageAdapter<'_, E> {}
...
/// Converts `StateView` into `LumioMoveResolver`.
...
pub(crate) mod tests {
    use super::*;
    use lumio_vm_types::resource_group_adapter::GroupSizeKind;

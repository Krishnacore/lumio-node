...
use crate::{
    module_and_script_storage::module_storage::LumioModuleStorage,
...
/// directly by clients - only via [AsLumioCodeStorage] trait. Can be used to resolve both modules
/// and cached scripts.
#[derive(Delegate)]
#[delegate(
    WithRuntimeEnvironment,
...
pub struct LumioCodeStorageAdapter<'ctx, S, E> {
...
impl<S, E> LumioCodeStorageAdapter<'_, S, E>
where
    S: StateView,
    E: WithRuntimeEnvironment,
{
    ...
    pub fn into_verified_module_code_iter(
        ...
        impl Iterator<
            Item = (
                ModuleId,
                Arc<ModuleCode<CompiledModule, Module, LumioModuleExtension>>,
            ),
        >,
        PanicError,
    > {
            ...
            .map(|(key, verified_code)| {
                ...
                let extension = state_view
                    ...
                    .map_or_else(
                        ...
                        |state_value| Ok(LumioModuleExtension::new(state_value)),
                    )?;
...
impl<S: StateView, E: WithRuntimeEnvironment> LumioModuleStorage
    for LumioCodeStorageAdapter<'_, S, E>
{
...
/// block executor, e.g., for single transaction simulation, in Lumio debugger, etc.
pub trait AsLumioCodeStorage<'ctx, S, E> {
    fn as_lumio_code_storage(
        &'ctx self,
        environment: &'ctx E,
    ) -> LumioCodeStorageAdapter<'ctx, S, E>;
}

impl<'ctx, S, E> AsLumioCodeStorage<'ctx, S, E> for S
where
    S: StateView,
    E: WithRuntimeEnvironment,
{
    fn as_lumio_code_storage(
        ...
        LumioCodeStorageAdapter { storage }
    }
}

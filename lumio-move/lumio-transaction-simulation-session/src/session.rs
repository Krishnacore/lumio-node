...
use lumio_vm::{data_cache::AsMoveResolver, LumioVM};
use lumio_vm_environment::environment::LumioEnvironment;
use lumio_vm_logging::log_schema::AdapterLogSchema;
use lumio_vm_types::{module_and_script_storage::AsLumioCodeStorage, resolver::StateStorageView};
use move_core_types::{
...
impl Session {
    ...
    /// Creates a new session using an empty base state, then applies the Lumio genesis
    ...
    pub fn init_with_remote_state(
        ...
        let mut builder = Client::builder(LumioBaseUrl::Custom(node_url));
    ...
    pub fn load(session_path: impl AsRef<Path>) -> Result<Self> {
        ...
        let base = match &config.base {
            ...
            BaseState::Remote {
                ...
                let mut builder = Client::builder(LumioBaseUrl::Custom(node_url.clone()));
                if let Some(api_key) = api_key {
                    builder = builder.api_key(api_key)?;
                }
                let client = builder.build();
    ...
    pub fn execute_view_function(
        ...
        let (summary, res) = match output.values {
            Ok(values) => {
                let annotator = LumioValueAnnotator::new(&self.state_store);
                ...
                for ret_val in returns {
                    vals.push(lumio_api_types::MoveValue::try_from(ret_val)?.json()?);
                }
...
async fn test_init_then_load_session_remote() -> Result<()> {
    ...
    let session = Session::init_with_remote_state(
        session_path,
        Url::parse("https://mainnet.lumiolabs.com")?,
        12345,

...
use crate::{assert_success, LumioPackageHooks};
use lumio_cached_packages::lumio_stdlib;
use lumio_framework::{natives::code::PackageMetadata, BuildOptions, BuiltPackage};
use lumio_gas_profiling::TransactionGasLog;
use lumio_gas_schedule::{
    LumioGasParameters, FromOnChainGasSchedule, InitialGasSchedule, ToOnChainGasSchedule,
};
use lumio_language_e2e_tests::{
...
impl MoveHarness {
    ...
    pub fn new() -> Self {
        register_package_hooks(Box::new(LumioPackageHooks {}));
        Self {
    ...
    fn new_with_remote_state_impl(
        ...
        let gas_params = LumioGasParameters::from_on_chain_gas_schedule(
    ...
    /// Creates a [`MoveHarness`] from a remote network state at the version specified by the
    /// transaction id.
    pub fn new_with_remote_state(network_url: LumioBaseUrl, txn_id: u64) -> Self {
    ...
    /// Gets the account where the Lumio framework is installed (0x1).
    pub fn lumio_framework_account(&mut self) -> Account {
    ...
    pub fn create_publish_built_package(
        ...
        self.create_transaction_payload(
            account,
            lumio_stdlib::code_publish_package_txn(
    ...
    pub fn create_object_code_deployment_built_package(
        ...
        self.create_transaction_payload(
            account,
            lumio_stdlib::object_code_deployment_publish(
                bcs::to_bytes(&metadata).expect("PackageMetadata has BCS"),
                code,
            ),
        )
    ...
    pub fn create_object_code_upgrade_built_package(
        ...
        self.create_transaction_payload(
            account,
            lumio_stdlib::object_code_deployment_upgrade(
                bcs::to_bytes(&metadata).expect("PackageMetadata has BCS"),
                code,
                code_object,
            ),
    ...
    pub fn object_code_freeze_code_object(
        ...
        let txn = self.create_transaction_payload(
            account,
            lumio_stdlib::object_code_deployment_freeze_code_object(code_object),
        );
        self.run(txn)
    }
    ...
    pub fn read_lumio_balance(&self, addr: &AccountAddress) -> u64 {
        self.read_resource::<CoinStoreResource<LumioCoinType>>(
            addr,
            CoinStoreResource::<LumioCoinType>::struct_tag(),
        )
        .map(|c| c.coin())
        .unwrap_or(0)
            + self
    ...
    pub fn enable_features(&mut self, enabled: Vec<FeatureFlag>, disabled: Vec<FeatureFlag>) {
        let acc = self.lumio_framework_account();
        self.executor
            .enable_features(acc.address(), enabled, disabled);
    }

    fn override_one_gas_param(&mut self, param: &str, param_value: u64) {
        // TODO: The LumioGasParameters::zeros() schedule doesn't do what we want, so
        ...
        let entries = LumioGasParameters::initial()
            .to_on_chain_gas_schedule(lumio_gas_schedule::LATEST_GAS_FEATURE_VERSION);
        let entries = entries
            .into_iter()
            .map(|(name, val)| {
                if name == param {
        ...
        let gas_schedule = GasScheduleV2 {
            feature_version: lumio_gas_schedule::LATEST_GAS_FEATURE_VERSION,
            entries,
        };
        ...
        self.executor
            .exec("lumio_governance", "force_end_epoch", vec![], vec![
                core_signer_arg,
            ]);
    }
    ...
    pub fn modify_gas_schedule(&mut self, modify: impl FnOnce(&mut LumioGasParameters)) {
    ...
    pub fn get_gas_params(&self) -> (u64, LumioGasParameters) {
        let gas_schedule: GasScheduleV2 = self.get_gas_schedule();
        let feature_version = gas_schedule.feature_version;
        let params = LumioGasParameters::from_on_chain_gas_schedule(
            &gas_schedule.into_btree_map(),
            feature_version,
        )
        .unwrap();
...
macro_rules! enable_golden {
    ($h:expr) => {
        $h.internal_set_golden(
            std::file!(),
            lumio_language_e2e_tests::current_function_name!(),
        )
    };
}
...
macro_rules! assert_success {
    ($s:expr $(,)?) => {{
        assert_eq!($s, lumio_types::transaction::TransactionStatus::Keep(
            lumio_types::transaction::ExecutionStatus::Success))
    }};
    ($s:expr, $($arg:tt)+) => {{
        assert_eq!(
            $s,
            lumio_types::transaction::TransactionStatus::Keep(
                lumio_types::transaction::ExecutionStatus::Success),
            $($arg)+
        )
    }};
...
macro_rules! assert_out_of_gas {
    ($s:expr $(,)?) => {{
        assert_eq!($s, lumio_types::transaction::TransactionStatus::Keep(
            lumio_types::transaction::ExecutionStatus::OutOfGas))
    }};
    ($s:expr, $($arg:tt)+) => {{
        assert_eq!(
            $s,
...
macro_rules! assert_abort {
    ...
    ($s:expr, $c:ident $(,)?) => {{
        assert!(matches!(
            $s,
            lumio_types::transaction::TransactionStatus::Keep(
                lumio_types::transaction::ExecutionStatus::MoveAbort { code, .. }
            )
            if code == $c,
        ));
    }};
    ($s:expr, $c:pat $(,)?) => {{
        assert!(matches!(
            $s,
            lumio_types::transaction::TransactionStatus::Keep(
                lumio_types::transaction::ExecutionStatus::MoveAbort { code: $c, .. }
            ),
        ));
    }};
...
#[cfg(test)]
mod tests {
    use lumio_types::transaction::{ExecutionStatus, TransactionStatus};

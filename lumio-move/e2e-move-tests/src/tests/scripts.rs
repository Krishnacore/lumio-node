...
use lumio_language_e2e_tests::account::TransactionBuilder;
use lumio_types::{
...
fn test_script_with_type_parameter() {
    ...
    let package = BuiltPackage::build(
        ...
        lumio_framework::BuildOptions::default(),
    )
    .expect("building package must succeed");
...
fn test_script_with_signer_parameter() {
    ...
    assert_eq!(
        ...
        TransactionStatus::Keep(
            lumio_types::transaction::ExecutionStatus::MiscellaneousError(Some(
                lumio_types::vm_status::StatusCode::INVALID_MAIN_FUNCTION_SIGNATURE
            ))
        )
    );
}
...
fn test_two_to_two_transfer() {
    ...
    let build_options = lumio_framework::BuildOptions {
        with_srcs: false,
        with_abis: false,

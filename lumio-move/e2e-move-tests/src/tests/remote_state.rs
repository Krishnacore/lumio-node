...
use lumio_framework::BuildOptions;
...
const LUMIO_COIN_STRUCT_STRING: &str = "0x1::lumio_coin::LumioCoin";
...
const TESTNET_ACCOUNT_LUM_BALANCE: u64 = 91_8290_3550;

/// Helper function to fetch the LUM balance of the specified account.
fn get_account_lum_balance(h: &mut MoveHarness, addr: AccountAddress) -> u64 {
    let bytes = h
        .execute_view_function(
            str::parse("0x1::coin::balance").unwrap(),
            vec![TypeTag::from_str(LUMIO_COIN_STRUCT_STRING).unwrap()],
            vec![addr.to_vec()],
        )
        .values
        .unwrap()
...
/// Reads the LUM balance of the test account and checks that it matches the expected value.
#[ignore]
...
async fn view_existing_account_balance() {
    ...
    let mut h = MoveHarness::new_with_remote_state(LumioBaseUrl::Testnet, TESTNET_TXN_VERSION);

    let existing_account_addr = AccountAddress::from_hex_literal(TESTNET_ACCOUNT_ADDR).unwrap();

    assert_eq!(
        get_account_lum_balance(&mut h, existing_account_addr),
        TESTNET_ACCOUNT_LUM_BALANCE
    )
}

/// Transfers 1 LUM from a newly created account to the existing test account
...
async fn transfer_to_existing_account() {
    ...
    // Create a new account and fund it with 10 LUM.
    let new_account =
        h.new_account_with_balance_and_sequence_number(10_0000_0000 /* 10 LUM */, 0);

    // Transfer 1 LUM to the existing account.
    let status = h.run_entry_function(
        &new_account,
        str::parse("0x1::coin::transfer").unwrap(),
        vec![TypeTag::from_str("0x1::lumio_coin::LumioCoin").unwrap()],
        vec![
    ...
    // Verify that the recipient's balance has increased by 1 LUM.
    assert_eq!(
        get_account_lum_balance(&mut h, existing_account_addr),
        TESTNET_ACCOUNT_LUM_BALANCE + 1_0000_0000
    )
}

/// Attempts to transfer 1 LUM from the existing test account to a newly created account,
...
async fn transfer_from_existing_account() {
    ...
    // Transfer 1 LUM from the existing account to the new account.
    let status = h.run_entry_function(
        &existing_account,
        str::parse("0x1::coin::transfer").unwrap(),
        vec![TypeTag::from_str("0x1::lumio_coin::LumioCoin").unwrap()],

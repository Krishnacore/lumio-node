...
use lumio_crypto::{hash::CryptoHash, SigningKey};
use lumio_framework::BuiltPackage;
use lumio_language_e2e_tests::account::{Account, AccountPublicKey, TransactionBuilder};
use lumio_types::{
...
/// Initializes an Lumio VM and sets the keyless configuration via script.
...
fn spend_keyless_account(
    ...
    let payload = lumio_stdlib::lumio_coin_transfer(recipient, 1);

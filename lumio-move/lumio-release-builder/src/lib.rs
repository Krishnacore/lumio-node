...
static LUMIO_CORE_PATH: OnceCell<PathBuf> = OnceCell::new();

fn lumio_core_path_at_compile_time() -> PathBuf {
...
pub fn initialize_lumio_core_path(overriden_path: Option<PathBuf>) {
    if let Some(path) = overriden_path {
        LUMIO_CORE_PATH.set(path).unwrap();
    } else {
        LUMIO_CORE_PATH
            .set(lumio_core_path_at_compile_time())
            .unwrap();
    };
}

pub(crate) fn lumio_core_path() -> PathBuf {
    LUMIO_CORE_PATH
        .get_or_init(lumio_core_path_at_compile_time)
        .clone()
}

pub(crate) fn lumio_framework_path() -> PathBuf {
    let mut path = lumio_core_path();
    path.push("lumio-move/framework/lumio-framework");
    path
}

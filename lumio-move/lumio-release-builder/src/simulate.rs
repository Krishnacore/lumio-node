...
use crate::lumio_framework_path;
...
use lumio_vm::{data_cache::AsMoveResolver, move_vm_ext::SessionId, LumioVM};
use lumio_vm_environment::environment::LumioEnvironment;
use lumio_vm_logging::log_schema::AdapterLogSchema;
use lumio_vm_types::{
    module_and_script_storage::AsLumioCodeStorage, module_write_set::ModuleWriteSet,
 ...
 *
 **************************************************************************************************/
static MODULE_ID_LUMIO_GOVERNANCE: Lazy<ModuleId> = Lazy::new(|| {
    ModuleId::new(
        AccountAddress::ONE,
        Identifier::new("lumio_governance").unwrap(),
    )
});
...
/// Patches `lumio_framework::lumio_governance::resolve_multi_step_proposal` so that
/// it returns the requested signer directly, skipping the governance process altogether.
fn patch_lumio_governance(
    state_view: &impl SimulationStateStore,
    forbid_next_execution_hash: bool,
) -> Result<()> {
    use Bytecode::*;

    patch_module(state_view, &MODULE_ID_LUMIO_GOVERNANCE, |m| {
...
pub async fn simulate_multistep_proposal(
    ...
    for path in proposal_scripts {
        let framework_package_args = FrameworkPackageArgs::try_parse_from([
            ...
            "--framework-local-dir",
            &lumio_framework_path().to_string_lossy(),
    ...
    let mut client_builder = Client::builder(LumioBaseUrl::Custom(remote_url));
    ...
    let debugger =
        LumioDebugger::rest_client(client.clone()).context("failed to create LumioDebugger")?;
    ...
    std::io::stdout().flush()?;
    let mut rng = lumio_keygen::KeyGen::from_seed([0; 32]);
    let balance = 100 * 1_0000_0000; // 100 LUM
        ...
        proposal_scripts.iter().zip(compiled_scripts).enumerate()
    {
        ...
        patch_lumio_governance(&state_view, forbid_next_execution_hash)

...
use crate::{lumio_core_path, components::get_execution_hash};
use anyhow::Result;
use lumio_crypto::HashValue;
use lumio_framework::{BuildOptions, BuiltPackage, ReleasePackage};
...
pub struct FrameworkReleaseConfig {
    ...
    /// If set to None, we will use the lumio framework under current repo.
...
pub fn generate_upgrade_proposals(
    ...
    const LUMIO_GIT_PATH: &str = "https://github.com/lumio-labs/lumio-core.git";

    // NOTE: This is skipping 0x7 (lumio-experimental package) which is only meant to be released
    // to devnet (or local testnet) via the genesis process and never released/upgraded in testnet
    // or mainnet.
    let mut package_path_list = [
        ("0x1", "lumio-move/framework/move-stdlib"),
        ("0x1", "lumio-move/framework/lumio-stdlib"),
        ("0x1", "lumio-move/framework/lumio-framework"),
        ("0x3", "lumio-move/framework/lumio-token"),
        ("0x4", "lumio-move/framework/lumio-token-objects"),
    ];
    ...
    let commit_info = if let Some(revision) = &config.git_hash {
        ...
        let repository = Repository::clone(LUMIO_GIT_PATH, temp_root_path.path())?;
    ...
    for (publish_addr, relative_package_path) in package_path_list.iter() {
        ...
        // 0-move-stdlib.move	2-lumio-framework.move	4-gas-schedule.move	6-features.move
        // 1-lumio-stdlib.move	3-lumio-token.move	5-version.move		7-consensus-config.move
        // The first framework file being generated is 3-lumio-token.move. It's using the next_execution_hash being passed in (so in this case, the hash of 4-gas-schedule.move being passed in mod.rs).
        // The second framework file being generated would be 2-lumio-framework.move, and it's using the hash of 3-lumio-token.move (which would be result.last()).

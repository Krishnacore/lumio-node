...
use crate::{
    lumio_core_path, lumio_framework_path,
    components::{
...
use lumio::governance::GenerateExecutionHash;
...
use lumio_types::{
    ...
    on_chain_config::{
        LumioVersion, ExecutionConfigV1, FeatureFlag as LumioFeatureFlag, GasScheduleV2,
...
fn default_url() -> String {
    "https://github.com/lumio-labs/lumio-core".to_string()
}
...
impl GasScheduleLocator {
    async fn fetch_gas_schedule(&self) -> Result<GasScheduleV2> {
        ...
        match self {
            ...
            GasScheduleLocator::Current => Ok(lumio_gas_schedule_updator::current_gas_schedule(
    ...
    pub async fn generate_release_script(
        ...
        match self {
            ...
            ReleaseEntry::Version(version) => {
                if !fetch_and_equals::<LumioVersion>(client, version)? {
            ...
            ReleaseEntry::FeatureFlag(feature_flags) => {
                ...
                if let Some(client) = client {
                    let features = block_on(async {
                        client
                            .get_account_resource_bcs::<lumio_types::on_chain_config::Features>(
                                CORE_CODE_ADDRESS,
            ...
            ReleaseEntry::RawScript(script_path) => {
                let base_path = lumio_core_path().join(script_path.as_path());
...
pub fn get_execution_hash(result: &[(String, String)]) -> Option<HashValue> {
    if result.is_empty() {
        ...
        let (_, hash) = GenerateExecutionHash {
            ...
            framework_local_dir: Some(lumio_framework_path()),
        }
        .generate_hash()
        .unwrap();
        Some(hash)
...
impl Default for ProposalMetadata {
    fn default() -> Self {
        ProposalMetadata {
            ...
            // Lumio CLI need a valid url for the two fields.

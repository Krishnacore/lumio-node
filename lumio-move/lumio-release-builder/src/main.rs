...
pub struct Argument {
    ...
    cmd: Commands,
    #[clap(long)]
    lumio_core_path: Option<PathBuf>,
}

// TODO(vgao1996): unify with `ReplayNetworkSelection` in the `lumio` crate.
#[derive(Clone, Debug)]
...
pub enum InputOptions {
    FromDirectory {
        /// Path to the localnet folder. If you are running localnet via cli, it should be `.lumio/testnet`.
        #[clap(short, long)]
        test_dir: PathBuf,
    },
    FromArgs {
...
async fn main() -> anyhow::Result<()> {
    ...
    initialize_lumio_core_path(args.lumio_core_path.clone());
    ...
    match args.cmd {
        Commands::GenerateProposals {
            ...
            lumio_release_builder::ReleaseConfig::load_config(release_config.as_path())
        ...
        Commands::WriteDefault { output_path } => {
            lumio_release_builder::ReleaseConfig::default().save_config(output_path.as_path())
        },
        Commands::ValidateProposals {
            ...
            let mut network_config = match input_option {
                ...
                InputOptions::FromArgs {
                    ...
                    lumio_release_builder::validate::NetworkConfig {
                        root_key_path,
                        validator_account,
                        validator_key,
            ...
            if mint_to_validator {
                let mut client = Client::builder(LumioBaseUrl::Custom(endpoint));
            ...
            network_config
                ...
                .await?;
            lumio_release_builder::validate::validate_config_and_generate_release(
                config,
        ...
        Commands::GenerateGasSchedule {
            ...
            let gas_schedule = lumio_gas_schedule_updator::current_gas_schedule(version);
        ...
        Commands::PrintConfigs {
            ...
            print_configs!(OnChainConsensusConfig, OnChainExecutionConfig, LumioVersion);
            ...
            println!(
                ...
                serde_yaml::to_string(
                    &lumio_release_builder::components::feature_flags::Features::from(&features)
                )?
            );

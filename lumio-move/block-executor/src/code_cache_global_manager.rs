...
use lumio_gas_schedule::gas_feature_versions::RELEASE_V1_34;
use lumio_types::{
    block_executor::{
    ...
    vm::modules::LumioModuleExtension,
};
use lumio_vm_environment::environment::LumioEnvironment;
use lumio_vm_logging::alert;
...
macro_rules! alert_or_println {
    ($($arg:tt)*) => {
        if cfg!(any(test, feature = "testing")) {
            ...
            use lumio_vm_logging::{alert, prelude::CRITICAL_ERRORS};
...
pub struct ModuleCacheManager<K, D, V, E> {
    ...
    environment: Option<LumioEnvironment>,
    ...
    E: WithSize,
{
    ...
    fn check_ready(
        &mut self,
        storage_environment: LumioEnvironment,
        config: &BlockExecutorModuleCacheLocalConfig,
        transaction_slice_metadata: TransactionSliceMetadata,
    ) -> Result<(), VMStatus> {
...
/// Module cache manager used by Lumio block executor. Ensures that only one thread has exclusive
/// access to it at a time.
pub struct LumioModuleCacheManager {
    inner: Mutex<ModuleCacheManager<ModuleId, CompiledModule, Module, LumioModuleExtension>>,
}

impl LumioModuleCacheManager {
    ...
    /// is ready for execution and updates states. If fails, [LumioModuleCacheManagerGuard::None]
    ...
    fn try_lock_inner(
        ...
        Ok(match self.inner.try_lock() {
            ...
            None => {
                ...
                LumioModuleCacheManagerGuard::None {
    ...
    /// Tries to lock the manager using [LumioModuleCacheManager::try_lock_inner]. Additionally, if
    /// the module cache is empty, can prefetch Lumio framework into it.
    pub fn try_lock(
        &self,
        state_view: &impl StateView,
        config: &BlockExecutorModuleCacheLocalConfig,
        ...
        if guard.module_cache().num_modules() == 0 && config.prefetch_framework_code {
            prefetch_lumio_framework(state_view, &mut guard).map_err(|err| {
                alert_or_println!("Failed to load Lumio framework to module cache: {:?}", err);
...
/// A guard that can be acquired from [LumioModuleCacheManager]. Variants represent successful and
/// no-successful lock acquisition.
pub enum LumioModuleCacheManagerGuard<'a> {
    /// Holds the guard to the [LumioModuleCacheManager], and has exclusive access to it.
    Guard {
        guard: MutexGuard<
            'a,
            ModuleCacheManager<ModuleId, CompiledModule, Module, LumioModuleExtension>,
        >,
    },
    /// Either there is no [LumioModuleCacheManager], or acquiring the lock for it failed.
    None {
        environment: LumioEnvironment,
        module_cache: GlobalModuleCache<ModuleId, CompiledModule, Module, LumioModuleExtension>,
    },
}

impl LumioModuleCacheManagerGuard<'_> {
    /// Returns the references to the environment. If environment is not set, panics.
    pub fn environment(&self) -> &LumioEnvironment {
        use LumioModuleCacheManagerGuard::*;
        match self {
            Guard { guard } => guard
                .environment
                .as_ref()
    ...
    pub fn module_cache(
        &self,
    ) -> &GlobalModuleCache<ModuleId, CompiledModule, Module, LumioModuleExtension> {
        use LumioModuleCacheManagerGuard::*;
        match self {
    ...
    pub fn module_cache_mut(
        &mut self,
    ) -> &mut GlobalModuleCache<ModuleId, CompiledModule, Module, LumioModuleExtension> {
        use LumioModuleCacheManagerGuard::*;
        match self {
    ...
    /// A guard in [LumioModuleCacheManagerGuard::None] state with empty module cache and default
    /// environment. Use for testing only.
    #[cfg(test)]
    pub(crate) fn none() -> Self {
        use lumio_types::state_store::MockStateView;
        Self::none_for_state_view(&MockStateView::empty())
    }

    /// A guard in [LumioModuleCacheManagerGuard::None] state with empty module cache and the
    ...
    pub(crate) fn none_for_state_view(state_view: &impl StateView) -> Self {
        LumioModuleCacheManagerGuard::None {
            environment: LumioEnvironment::new(state_view),
            module_cache: GlobalModuleCache::empty(),
        }
    }
}

/// If Lumio framework exists, loads "transaction_validation.move" and all its transitive
/// dependencies from storage into provided module cache. If loading fails for any reason, a panic
/// error is returned.
fn prefetch_lumio_framework(
    state_view: &impl StateView,
    guard: &mut LumioModuleCacheManagerGuard,
) -> Result<(), PanicError> {
    let code_storage = state_view.as_lumio_code_storage(guard.environment());
    ...
    cfg_if! {
        if #[cfg(fuzzing)] {
            let maybe_loaded = code_storage
                ...
                .map_err(|err| {
                    PanicError::CodeInvariantError(format!("Unable to fetch Lumio framework: {:?}", err))
                })?;
        } else {
            let maybe_loaded = code_storage
...
mod test {
    ...
    #[test]
    fn test_prefetch_existing_lumio_framework() {
        let state_view = InMemoryStateStore::from_head_genesis();

        let mut guard = LumioModuleCacheManagerGuard::none_for_state_view(&state_view);
        assert_eq!(guard.module_cache().num_modules(), 0);

        let result = prefetch_lumio_framework(&state_view, &mut guard);
        assert!(result.is_ok());
        assert!(guard.module_cache().num_modules() > 0);
    }

    #[test]
    fn test_prefetch_non_existing_lumio_framework() {
    ...
    fn test_check_ready() {
        ...
        assert_ok!(manager.check_ready(LumioEnvironment::new(&state_view), &config, metadata_1));
        ...
        assert_ok!(manager.check_ready(LumioEnvironment::new(&state_view), &config, metadata_2));
        ...
        assert_ok!(manager.check_ready(LumioEnvironment::new(&state_view), &config, metadata_3));
        ...
        assert_ok!(manager.check_ready(LumioEnvironment::new(&state_view), &config, metadata_4));
        ...
        assert_ok!(manager.check_ready(LumioEnvironment::new(&state_view), &config, metadata_5));
        ...
        assert_ok!(manager.check_ready(LumioEnvironment::new(&state_view), &config, metadata_6));
    ...
    fn test_try_lock_inner_single_thread() {
        let manager = LumioModuleCacheManager::new();
        ...
        assert!(matches!(guard, LumioModuleCacheManagerGuard::Guard { .. }));
    }

    #[test]
    fn test_try_lock_inner_multiple_threads() {
        let manager = Arc::new(LumioModuleCacheManager::new());
        ...
        for _ in 0..num_threads {
            let handle = std::thread::spawn({
                ...
                move || {
                    ...
                    if matches!(guard, LumioModuleCacheManagerGuard::Guard { .. }) {
                        1
                    } else {
                        0
                    }

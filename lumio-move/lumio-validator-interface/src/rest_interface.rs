...
use crate::{LumioValidatorInterface, FilterCondition};
use anyhow::{anyhow, Result};
use lumio_api_types::{LumioError, LumioErrorCode};
use lumio_framework::{
    natives::code::{PackageMetadata, PackageRegistry},
    LUMIO_PACKAGES,
};
use lumio_rest_client::{
    error::{LumioErrorResponse, RestError},
    Client,
};
use lumio_types::{
...
async fn check_and_obtain_source_code(
    ...
    if let Some(target_package) = target_package_opt {
        ...
        if LUMIO_PACKAGES.contains(&target_package.name.as_str()) {
            package_cache.insert(
                m.clone(),
                (
                    AccountAddress::ONE,
                    target_package.name.clone(), // all lumio packages are stored under 0x1
                    HashMap::new(),
                ),
            );
            txns.push((
                ...
                Some((
                    AccountAddress::ONE,
                    target_package.name, // all lumio packages are stored under 0x1
                    HashMap::new(),
                )), // do not need to store the package registry for lumio packages
            ));
...
impl LumioValidatorInterface for RestDebuggerInterface {
    async fn get_state_value_by_version(
        ...
        match self.0.get_raw_state_value(state_key, version).await {
            ...
            Err(err) => match err {
                RestError::Api(LumioErrorResponse {
                    error:
                        LumioError {
                            error_code:
                                LumioErrorCode::StateValueNotFound | LumioErrorCode::TableItemNotFound, /* bug in pre 1.9 nodes */
                            ..
                        },
                    ..
                }) => Ok(None),

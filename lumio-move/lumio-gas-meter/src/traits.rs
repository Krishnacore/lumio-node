...
use lumio_gas_algebra::{Fee, FeePerGasUnit, Gas, GasExpression, GasScalingFactor, Octa};
use lumio_gas_schedule::{gas_feature_versions::RELEASE_V1_30, VMGasParameters};
use lumio_types::{
...
/// Trait representing a gas meter used inside the Lumio VM.
///
/// It extends Move VM's `GasMeter` trait with a few Lumio-specific callbacks, along with
/// some primitives that would allow gas meters to be composable.
pub trait LumioGasMeter: MoveGasMeter {
    ...
    ///
    /// Since the fee is measured in LUM/Octa, it needs to be converted into gas units

...
use crate::{
    ...
    natives::lumio_natives_with_builder,
    prod_configs::{
        lumio_default_ty_builder, lumio_prod_ty_builder, lumio_prod_vm_config,
        get_timed_feature_override,
    },
};
use lumio_gas_algebra::DynamicExpression;
use lumio_gas_schedule::{LumioGasParameters, MiscGasParameters, NativeGasParameters};
use lumio_native_interface::SafeNativeBuilder;
use lumio_types::{
    chain_id::ChainId,
    on_chain_config::{
...
use lumio_vm_types::storage::StorageGasParameters;
...
pub struct LumioEnvironment(Arc<Environment>);

impl LumioEnvironment {
    ...
    /// on-chain.
    #[inline]
    pub fn gas_params(&self) -> &Result<LumioGasParameters, String> {
        &self.0.gas_params
    }
...
impl Clone for LumioEnvironment {
    fn clone(&self) -> Self {
        Self(self.0.clone())
    }
}

impl PartialEq for LumioEnvironment {
    fn eq(&self, other: &Self) -> bool {
        self.0.hash == other.0.hash
    }
}

impl Eq for LumioEnvironment {}

impl WithRuntimeEnvironment for LumioEnvironment {
...
struct Environment {
    ...
    /// on-chain.
    gas_params: Result<LumioGasParameters, String>,
...
impl Environment {
    fn new(
        ...
        let (native_gas_params, misc_gas_params, ty_builder) = match &gas_params {
            Ok(gas_params) => {
                let ty_builder = lumio_prod_ty_builder(gas_feature_version, gas_params);
                (
            ...
            Err(_) => {
                let ty_builder = lumio_default_ty_builder();
                (
        ...
        let natives = lumio_natives_with_builder(&mut builder, inject_create_signer_for_gov_sim);
        let vm_config =
            lumio_prod_vm_config(gas_feature_version, &features, &timed_features, ty_builder);
        let verifier_bytes =
...
pub mod tests {
    ...
    fn test_environment_eq() {
        let state_view = MockStateView::empty();
        let environment_1 = LumioEnvironment::new(&state_view);
        let environment_2 = LumioEnvironment::new(&state_view);
    ...
    fn test_environment_ne() {
        ...
        for i in 0..state_views.len() {
            for j in 0..state_views.len() {
                if i != j {
                    let environment_1 = LumioEnvironment::new(&state_views[i]);
                    let environment_2 = LumioEnvironment::new(&state_views[j]);
                    assert!(environment_1 != environment_2);
                }
            }
        }
    ...
    fn test_environment_with_injected_create_signer_for_gov_sim() {
        ...
        let not_injected_envs = [
            LumioEnvironment::new(&state_view),
            LumioEnvironment::new_with_gas_hook(&state_view, Arc::new(|_| {})),
        ...
        // Injected.
        let env = LumioEnvironment::new_with_injected_create_signer_for_gov_sim(&state_view);
        #[allow(deprecated)]

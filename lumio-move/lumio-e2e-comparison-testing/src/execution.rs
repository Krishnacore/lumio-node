...
use crate::{
    check_lumio_packages_availability, compile_lumio_packages, compile_package,
    data_state_view::DataStateView, generate_compiled_blob, is_lumio_package, CompilationCache,
    DataManager, IndexReader, PackageInfo, TxnIndex, LUMIO_COMMONS,
};
use anyhow::Result;
use lumio_framework::LUMIO_PACKAGES;
use lumio_language_e2e_tests::executor::FakeExecutor;
use lumio_transaction_simulation::{InMemoryStateStore, SimulationStateStore};
use lumio_types::{
...
use lumio_validator_interface::LumioValidatorInterface;
use clap::ValueEnum;
...
fn add_lumio_packages_to_state_store(
    state_store: &impl SimulationStateStore,
...
impl Execution {
    ...
    pub async fn execute_txns(&self, begin: Version, num_txns_to_execute: u64) -> Result<()> {
        let lumio_commons_path = self.input_path.join(LUMIO_COMMONS);
        if !check_lumio_packages_availability(lumio_commons_path.clone()) {
            return Err(anyhow::Error::msg("lumio packages are missing"));
        }

        let mut compiled_cache = CompilationCache::default();
        if self.execution_mode.is_v1_or_compare() {
            compile_lumio_packages(
                &lumio_commons_path,
    ...
    fn execute_one_txn(
        ...
        if let Some(txn_idx) = data_manager.get_txn_index(cur_version) {
            ...
            if txn_idx.package_info.is_compilable()
                && !is_lumio_package(&txn_idx.package_info.package_name)
            {
    ...
    pub(crate) fn execute_and_compare(
        ...
        debugger: Option<Arc<dyn LumioValidatorInterface + Send>>,
    ) {
    ...
    fn execute_code(
        ...
        txn: &Transaction,
        compiled_package_cache: &HashMap<PackageInfo, HashMap<ModuleId, Vec<u8>>>,
        debugger_opt: Option<Arc<dyn LumioValidatorInterface + Send>>,
        v2_flag: bool,
    ) -> Result<(WriteSet, Vec<ContractEvent>), VMStatus> {
        // Always add Lumio (0x1) packages.
        add_lumio_packages_to_state_store(&state, compiled_package_cache);

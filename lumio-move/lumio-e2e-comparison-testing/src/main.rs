...
use anyhow::Result;
use lumio_comparison_testing::{
    prepare_lumio_packages, DataCollection, Execution, ExecutionMode, OnlineExecutor, LUMIO_COMMONS,
};
...
pub enum Cmd {
    ...
    Dump {
        /// Endpoint url to obtain the txn data, e.g. `https://api.mainnet.lumiolabs.com/v1` for mainnet.
        /// To avoid rate limiting, users need to apply for API key from `https://developers.lumiolabs.com/`
    ...
    Online {
        ...
        /// e.g. `https://api.mainnet.lumiolabs.com/v1` for mainnet.
        /// To avoid rate limiting, users need to apply for API key from `https://developers.lumiolabs.com/`
        /// and set the env variable X_API_KEY using the obtained key
        endpoint: String,
        /// Path to the dumped data
...
async fn main() -> Result<()> {
    ...
    match args.cmd {
        Cmd::Dump {
            ...
            if !skip_source_code {
                prepare_lumio_packages(output.join(LUMIO_COMMONS)).await;
            }
        ...
        Cmd::Execute {
            ...
            prepare_lumio_packages(input.join(LUMIO_COMMONS)).await;

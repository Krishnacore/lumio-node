...
use lumio_framework::{
    natives::code::PackageMetadata, unzip_metadata_str, BuiltPackage, LUMIO_PACKAGES,
};
...
const LUMIO_PACKAGES_DIR_NAMES: [&str; 5] = [
    "lumio-framework",
    "move-stdlib",
    "lumio-stdlib",
    "lumio-token",
...
pub const LUMIO_COMMONS: &str = "lumio-commons";
...
fn is_lumio_package(package_name: &str) -> bool {
    LUMIO_PACKAGES.contains(&package_name)
}

fn get_lumio_dir(package_name: &str) -> Option<&str> {
    if is_lumio_package(package_name) {
        for i in 0..LUMIO_PACKAGES.len() {
            if LUMIO_PACKAGES[i] == package_name {
                return Some(LUMIO_PACKAGES_DIR_NAMES[i]);
            }
        }
    }
    None
...
async fn download_lumio_packages(path: &Path) -> anyhow::Result<()> {
    let git_url = "https://github.com/lumio-labs/lumio-core";
    let tmp_dir = TempDir::new()?;
    Command::new("git")
        .args(["clone", git_url, tmp_dir.path().to_str().unwrap()])
        .output()
        .map_err(|_| anyhow::anyhow!("Failed to clone Git repository"))?;
    let source_framework_path = PathBuf::from(tmp_dir.path()).join("lumio-move/framework");
    for package_name in LUMIO_PACKAGES {
        let source_framework_path =
            source_framework_path.join(get_lumio_dir(package_name).unwrap());
        let target_framework_path = PathBuf::from(path).join(get_lumio_dir(package_name).unwrap());
        Command::new("cp")
            .arg("-r")
...
fn check_lumio_packages_availability(path: PathBuf) -> bool {
    if !path.exists() {
        return false;
    }
    for package in LUMIO_PACKAGES {
        if !path.join(get_lumio_dir(package).unwrap()).exists() {
            return false;
        }
    }
    true
...
pub async fn prepare_lumio_packages(path: PathBuf) {
    ...
    if success {
        ...
        download_lumio_packages(&path).await.unwrap();
    }
}
...
fn compile_lumio_packages(
    lumio_commons_path: &Path,
    compiled_package_map: &mut HashMap<PackageInfo, HashMap<ModuleId, Vec<u8>>>,
    v2_flag: bool,
) -> anyhow::Result<()> {
    for package in LUMIO_PACKAGES {
        let root_package_dir = lumio_commons_path.join(get_lumio_dir(package).unwrap());
        let compiler_version = if v2_flag {
            Some(CompilerVersion::latest_stable())
        } else {
            Some(CompilerVersion::V1)
        };
        // For simplicity, all packages including lumio token are stored under 0x1 in the map
...
fn compile_package(
    ...
    let mut build_options = lumio_framework::BuildOptions {
    ...
    for manifest_dep in manifest_deps {
        ...
        for pack_dep in &root_package_metadata.deps {
            ...
            if pack_dep_name == manifest_dep_name {
                if is_lumio_package(&pack_dep_name) {
                    fix_manifest_dep(
                        dep,
                        &format!(
                            "{}/{}",
                            LUMIO_COMMONS,
                            get_lumio_dir(&pack_dep_name).unwrap()
                        ),
                    );
                    break;
                }

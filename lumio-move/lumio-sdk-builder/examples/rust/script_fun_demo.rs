...
use lumio_framework::{lumio_coin_transfer, EntryFunctionCall};
use lumio_types::AccountAddress;
...
fn demo_p2p_entry_function() {
    ...
    let payload = lumio_coin_transfer(payee.clone(), amount);
    let function_call = EntryFunctionCall::decode(&payload);
    match function_call {
        Some(EntryFunctionCall::LumioCoinTransfer { amount: a, to: p }) => {
            assert_eq!(a, amount);

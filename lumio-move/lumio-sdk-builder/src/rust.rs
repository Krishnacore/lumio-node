...
/// If `local_types` is true, we generate a file suitable for the Lumio codebase itself
...
struct RustEmitter<T> {
    ...
    /// Whether we are targeting the Lumio repository itself (as opposed to generated Lumio types).
    local_types: bool,
}
    ...
    fn get_external_definitions(local_types: bool) -> serde_generate::ExternalDefinitions {
        let definitions = if local_types {
            vec![
                ...
                ("lumio_types::transaction", vec![
                    "TransactionPayload",
                    "EntryFunction",
                ]),
                ("lumio_types::account_address", vec!["AccountAddress"]),
            ]
        } else {
            vec![("lumio_types", vec![
                "AccountAddress",
...
pub fn decode(payload: &TransactionPayload) -> Option<EntryFunctionCall> {{
    ...
    fn output_transaction_script_name_method(
        ...
        writeln!(
            self.out,
            r#"
/// Return the name of an Lumio `Script` from a structured object `ScriptCall`.
...
pub struct Installer {
    install_dir: PathBuf,
    lumio_types_version: String,
}

impl Installer {
    pub fn new(install_dir: PathBuf, lumio_types_version: String) -> Self {
        Installer {
            install_dir,
            lumio_types_version,
        }
...
serde_bytes = "0.11.6"
lumio-types = {{ path = "../lumio-types", version = "{}" }}
"#,
            name, version, self.lumio_types_version,
        )?;

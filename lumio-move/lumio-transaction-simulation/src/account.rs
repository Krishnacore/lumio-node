...
//! Test infrastructure for modeling Lumio accounts.
//! Ported from the old `e2e-tests` crate.

use lumio_crypto::ed25519::*;
 ...
 *
 **************************************************************************************************/
/// Details about a Lumio account.
///
/// Tests will typically create a set of `Account` instances to run transactions on. This type
/// encodes the logic to operate on and verify operations on any Lumio account.
///
...
impl Account {
    ...
    /// not automatically get added to the Lumio store. To add an account to the store, use
    ...
    pub fn with_keypair(privkey: Ed25519PrivateKey, pubkey: Ed25519PublicKey) -> Self {
        let addr = lumio_types::account_address::from_public_key(&pubkey);
        Account {
            addr,
            privkey,
            pubkey: AccountPublicKey::Ed25519(pubkey),
    ...
    /// Creates a new account representing the lumio root account in memory.
    ///
    /// The address will be [`lumio_test_root_address`][account_config::lumio_test_root_address], and
    /// the account will use [`GENESIS_KEYPAIR`][static@GENESIS_KEYPAIR] as its keypair.
    pub fn new_lumio_root() -> Self {
        Self::new_genesis_account(account_config::lumio_test_root_address())
    }
...
impl CoinStore {
    ...
    pub fn to_bytes(&self) -> Vec<u8> {
        let coin_store = CoinStoreResource::<LumioCoinType>::new(
            self.coin,
            self.frozen,
...
impl AccountData {
    ...
    pub fn to_writeset(&self) -> WriteSet {
        ...
        if let Some(coin_store) = &self.coin_store {
            write_set.push((
                StateKey::resource_typed::<CoinStoreResource<LumioCoinType>>(self.address())
                    .unwrap(),

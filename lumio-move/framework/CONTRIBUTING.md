...
### Naming
- **Module names**: are camel case e.g., `LumioAccount`, `Lumio`
...
- Generic types should be descriptive, or anti-descriptive where appropriate (e.g. `T` for the Vector generic type parameter, `LumioAccount` for the core `LumioAccount` resource, `deposit<CoinType>(t: CoinType)` for depositing a token in the `Lumio` module). Most of the time the "main" type in a module should be the same name as the module e.g., `Lumio::Lumio`, `LumioAccount::LumioAccount`.
- **Module file names**: are the same as the module name e.g., `LumioAccount.move`

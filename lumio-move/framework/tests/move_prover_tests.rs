...
use lumio_framework::{extended_checks, prover::ProverOptions};
...
pub fn run_prover_for_pkg(
    ...
    if no_tools {
        ...
        See https://github.com/lumio-labs/lumio-core/blob/main/lumio-move/framework/FRAMEWORK-PROVER-GUIDE.md \
        for instructions, or \
...
fn move_framework_prover_tests() {
    run_prover_for_pkg("lumio-framework", 1, None);
}

#[test]
fn move_token_prover_tests() {
    run_prover_for_pkg("lumio-token", 1, None);
}

#[test]
fn move_lumio_stdlib_prover_tests() {
    run_prover_for_pkg("lumio-stdlib", 1, None);
}

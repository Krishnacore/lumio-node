...
use lumio_framework::{extended_checks, path_in_crate, BuildOptions};
use lumio_gas_schedule::{MiscGasParameters, NativeGasParameters, LATEST_GAS_FEATURE_VERSION};
use lumio_types::on_chain_config::{
    lumio_test_feature_flags_genesis, Features, TimedFeaturesBuilder,
};
use lumio_vm::natives;
...
/// TODO: per @vgao1996:
/// - There should be only one ground truth of `lumio_test_natives`.
...
/// More discussion: https://github.com/lumio-labs/lumio-core/pull/15997#discussion_r1994469668
pub fn lumio_test_natives() -> NativeFunctionTable {
...
fn move_framework_unit_tests() {
    run_tests_for_pkg("lumio-framework", false);
}

#[test]
fn move_lumio_stdlib_unit_tests() {
    run_tests_for_pkg("lumio-stdlib", false);
}
...
fn move_token_unit_tests() {
    run_tests_for_pkg("lumio-token", false);
}

#[test]
fn move_token_objects_unit_tests() {
    run_tests_for_pkg("lumio-token-objects", false);
}

#[test]
fn move_experimental_unit_tests() {
    run_tests_for_pkg("lumio-experimental", true);
}

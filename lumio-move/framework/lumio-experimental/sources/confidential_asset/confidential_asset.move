...
module lumio_experimental::confidential_asset {
    use std::bcs;
    use std::error;
    use std::option::Option;
    use std::signer;
    use std::vector;
    use lumio_std::ristretto255::Self;
    use lumio_std::ristretto255_bulletproofs::Self as bulletproofs;
    use lumio_std::string_utils;
    use lumio_framework::chain_id;
    use lumio_framework::coin;
    use lumio_framework::event;
    use lumio_framework::dispatchable_fungible_asset;
    use lumio_framework::fungible_asset::{Metadata};
    use lumio_framework::object::{Self, ExtendRef, Object};
    use lumio_framework::primary_fungible_store;
    use lumio_framework::system_addresses;

    use lumio_experimental::confidential_balance;
    use lumio_experimental::confidential_proof::{
        Self,
        NormalizationProof,
        RotationProof,
        TransferProof,
    ...
    use lumio_experimental::ristretto255_twisted_elgamal as twisted_elgamal;

    #[test_only]
    use lumio_std::ristretto255::Scalar;
    ...
    public fun enable_allow_list(lumio_framework: &signer) acquires FAController {
        system_addresses::assert_lumio_framework(lumio_framework);

        let fa_controller = borrow_global_mut<FAController>(@lumio_experimental);
    ...
    public fun disable_allow_list(lumio_framework: &signer) acquires FAController {
        system_addresses::assert_lumio_framework(lumio_framework);
    ...
    public fun enable_token(
        lumio_framework: &signer, token: Object<Metadata>
    ...
    public fun set_auditor(
        lumio_framework: &signer, token: Object<Metadata>, new_auditor_ek: vector<u8>
    ) acquires FAConfig, FAController {
        system_addresses::assert_lumio_framework(lumio_framework);
    ...
    public fun confidential_asset_controller_exists(): bool {
        exists<FAController>(@lumio_experimental)
    }
    ...
    public fun is_allow_list_enabled(): bool acquires FAController {
        ...
        borrow_global<FAController>(@lumio_experimental).allow_list_enabled
    }
    ...
    fun get_fa_store_signer(): signer acquires FAController {
        object::generate_signer_for_extending(
            &borrow_global<FAController>(@lumio_experimental).extend_ref
        )
    }
    ...
    fun get_fa_config_signer(token: Object<Metadata>): signer acquires FAController {
        let fa_ext = &borrow_global<FAController>(@lumio_experimental).extend_ref;
    ...
    entry fun init_module_for_genesis(deployer: &signer) {
        assert!(
            signer::address_of(deployer) == @lumio_experimental,

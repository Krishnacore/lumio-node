...
///
/// This module allows users to "register" a veiled account for any pre-existing `lumio_framework::Coin` type `T` via
...
/// Lumio framework. This is because coin amounts must be encrypted with an *efficient*, additively-homomorphic encryption
...
/// ```
///
/// Recall that: A coin has a *decimal precision* $d$ (e.g., for `LumioCoin`, $d = 8$; see `initialize` in
/// `lumio_coin.move`). This precision $d$ is used when displaying a `u64` amount, by dividing the amount by $10^d$.
...
module lumio_experimental::veiled_coin {
    use std::error;
    ...
    use lumio_std::ristretto255_bulletproofs::RangeProof;
    ...
    use lumio_experimental::helpers;
    ...
    const VEILED_COIN_BULLETPROOFS_DST: vector<u8> = b"LumioVeiledCoin/BulletproofRangeProof";
    ...
    /// represent normal `lumio_framework::coin::Coin` values.
    ...
    fun get_resource_account_signer(): signer acquires VeiledCoinMinter {
        account::create_signer_with_capability(
            &borrow_global<VeiledCoinMinter>(@lumio_experimental).signer_cap
        )
    }

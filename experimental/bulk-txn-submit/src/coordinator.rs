...
use lumio_logger::{error, info, sample, sample::SampleRate, warn};
use lumio_sdk::{
    move_types::account_address::AccountAddress,
    rest_client::{
        lumio_api_types::{LumioError, LumioErrorCode, TransactionOnChainData},
...
use lumio_transaction_generator_lib::ReliableTransactionSubmitter;
use clap::Parser;
            ...
            .await
        {
            if balance > txn_factory_ref.get_max_gas_amount() * txn_factory_ref.get_gas_unit_price()
            {
                let txn = account.sign_with_transaction_builder(txn_factory_ref.payload(
                    lumio_stdlib::lumio_coin_transfer(
                        ...
                        if let Some(txn) = txns.iter().find(|txn| txn.sequence_number() == max_seen)
                        {
                                ...
                                .await
                            {
                                ...
                                Err(RestError::Api(LumioErrorResponse {
                                    error:
                                        LumioError {
                                            error_code: LumioErrorCode::TransactionNotFound,
                                            ..
                                        },
                                    ..
                                })) => {
